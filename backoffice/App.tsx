...
import { Restaurant, User, SidebarItem, ViewType } from './models/types';
...
const App: React.FC = () => {
    ...
    if (authenticated && !restaurant) {
      ...
      fetchRestaurants()
        ...
        .catch(err => {
          ...
          setError('Falha ao carregar restaurantes. Tente novamente.');
          setIsLoading(false);
        });
    }
  ...
  // Handle login
  const handleLogin = (userData: { email: string; password: string }): void => {
  ...
  const handleRestaurantSelect = (selectedRestaurant: Restaurant): void => {
...
const getBreadcrumbs = (view: ViewType, restaurant: Restaurant): Array<{label: string; href?: string}> => {
