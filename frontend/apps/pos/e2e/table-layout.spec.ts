...
test.describe('Table Layout - Restaurant Management', () => {
  ...
  test('should display table details correctly', async ({ page }) => {
    // The layout should show tables with proper positioning
    // Check if tables are positioned in different areas
    const mainAreaTables = page.locator('[data-area="main"]');
    const vipAreaTables = page.locator('[data-area="vip"]');
    const terraceTables = page.locator('[data-area="terrace"]');
    const barTables = page.locator('[data-area="bar"]');
