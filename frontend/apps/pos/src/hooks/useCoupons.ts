...
export const useCoupons = () => {
  ...
  const updateCoupon = useCallback(
    async (id: string, updates: Partial<CouponCreate>) => {
      setLoading(true);
      setError(null);
  ...
  const validateCoupon = useCallback(
    async (code: string, orderAmount: number, customerId?: string) => {
      setLoading(true);
      setError(null);
      ...
      if (result.is_valid) {
        success(
          `Cupom válido! Desconto: R$ ${result.discount_amount?.toFixed(2)}`
        );
      } else {
  ...
  const applyCoupon = useCallback(
    async (code: string, orderId: string) => {
      setLoading(true);
      setError(null);
  ...
  const getStatistics = useCallback(() => {
    ...
    const sortedByUsage = [...coupons].sort(
      (a: Coupon, b: Coupon) => b.usage_count - a.usage_count
    );
    return {
