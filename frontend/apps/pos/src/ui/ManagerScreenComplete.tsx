...
export default function ManagerScreenComplete() {
  ...
  const handleDeleteEmployee = async (id: string) => {
    if (window.confirm('Confirma a exclusão deste funcionário?')) {
      try {
  ...
  const handleDeleteSupplier = async (id: string) => {
    if (window.confirm('Confirma a exclusão deste fornecedor?')) {
      try {
  ...
  const handleDeleteBill = async (id: string) => {
    if (window.confirm('Confirma a exclusão desta conta?')) {
      try {
        await deleteBill(id);
  ...
  const generatePayroll = async () => {
    try {
      ...
      if (window.confirm('Deseja criar contas individuais para cada funcionário?')) {
