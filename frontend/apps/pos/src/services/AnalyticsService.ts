...
class AnalyticsService {
   ...
   */
  async getDashboardMetrics(_restaurantId: string = '1'): Promise<DashboardMetrics> {
...
// console.log(`Simulando venda de R$ ${amount.toFixed(2)}`);
  ...
  private async getRevenueToday(): Promise<number> {
    try {
      const response = await apiInterceptor.get<{ total: number }>('/api/v1/analytics/revenue/today');
  ...
  private async getOrdersToday(): Promise<number> {
    try {
      const response = await apiInterceptor.get<{ count: number }>('/api/v1/analytics/orders/today');
