...
export class TableLayoutConfigService {
  ...
  // Obter todas as configurações de layout
  ...
  static saveLayoutConfig(
    terminalId: string,
    config: RestaurantLayoutConfig
  ): void {
  ...
  static getLayoutTemplates(): LayoutTemplate[] {
    return [
      ...
      },
      {
        ...
        name: "Fast Food",
        description:
          "Layout otimizado para fast food com mesas pequenas e fluxo rápido",
        type: "fastfood",
  ...
  static applyTemplate(
    ...
    const config: RestaurantLayoutConfig = {
      ...
      metadata: {
        ...
        capacity: template.config.tables.reduce(
  ...
  static validateConfig(config: RestaurantLayoutConfig): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    config.tables?.forEach((table, _index) => {
      if (table.position.x < 0 || table.position.y < 0) {
        errors.push(`Mesa ${table.number}: posição inválida`);
      }

      if (
        table.position.x + table.size.width > config.dimensions.width ||
    ...
    for (let i = 0; i < config.tables.length; i++) {
      for (let j = i + 1; j < config.tables.length; j++) {
        ...
        if (this.tablesOverlap(table1, table2)) {
          errors.push(
            `Mesas ${table1.number} e ${table2.number} estão sobrepostas`
          );
        }
      }
    }
  ...
  static getConfigStats(config: RestaurantLayoutConfig) {
    ...
    const totalSeats = config.tables.reduce(
      (sum, table) => sum + table.seats,
      0
    );
