...
/**
 * Validates terminal ID from URL and loads terminal configuration
 * Similar to POS TerminalValidator
 */
export const TerminalValidator: React.FC<TerminalValidatorProps> = ({ children }) => {
  const { terminalId } = useParams<{ terminalId: string }>();
  const navigate = useNavigate();
  const [isValidating, setIsValidating] = useState(true);
  const [_terminalConfig, setTerminalConfig] = useState<TerminalConfig | null>(null);
  ...
  // List of available terminals (could be loaded from API in production)
    ...
    const validateTerminal = async () => {
      try {
        ...
        if (!AVAILABLE_TERMINALS.includes(id)) {
          ...
          setTerminalNotFound(true);
          setIsValidating(false);
          offlineStorage.log('Terminal not in available list', { terminalId: id });
          return;
        }
        ...
        try {
          const response = await fetch(`/src/config/kiosk/${id}.json`);
          
          if (!response.ok) {
            // Terminal config doesn't exist
            setTerminalNotFound(true);
            setIsValidating(false);
            offlineStorage.log('Terminal config not found', { terminalId: id });
            return;
          }
          ...
          // This prevents issues where server might return wrong file
          if (config.terminalId !== id) {
            offlineStorage.error('Terminal ID mismatch', { 
              requested: id, 
          ...
          if (!config.active) {
            setError(`Terminal ${id} está desativado`);
            setIsValidating(false);
            return;
          }

          // Store terminal config in localStorage for quick access
          localStorage.setItem('kiosk-terminal-config', JSON.stringify(config));
          localStorage.setItem('kiosk-terminal-id', id.toString());
          
          // Store in window for global access
          (window as unknown as Record<string, unknown>)['KIOSK_TERMINAL_CONFIG'] = config;
          
          setTerminalConfig(config);
          offlineStorage.log('Terminal validated', { 
            terminalId: id, 
          ...
          setTerminalNotFound(true);
          setIsValidating(false);
          offlineStorage.error('Failed to load terminal config', { 
            terminalId: id, 
            error: fetchError 
          });
        }
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'Erro ao validar terminal';
        setError(errorMessage);
        offlineStorage.error('Terminal validation failed', { error: errorMessage, terminalId });
        setIsValidating(false);
      }
    };
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-900 dark:to-gray-800">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 dark:text-gray-400 font-medium">
            Validando terminal {requestedTerminal || ''}...
          </p>
        </div>
      </div>
    );
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4">
          <div className="text-center">
            ...
            <h3 className="mt-4 text-xl font-bold text-gray-900 dark:text-white">
              Terminal {requestedTerminal} não encontrado
            </h3>
            <p className="mt-3 text-gray-600 dark:text-gray-400">
              O terminal que você está tentando acessar não existe ou não está configurado.
            </p>
            
            <div className="mt-6 space-y-3">
              <p className="text-sm text-gray-500 dark:text-gray-500">
                Terminais disponíveis: 1, 2, 3
              </p>
              
              <button
                onClick={() => navigate('/kiosk/1')}
