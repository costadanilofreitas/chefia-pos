...
class SupportService:
    ...
    async def get_support_dashboard(self, restaurant_id: str, user_id: str) -> Dict[str, Any]:
        ...
        recent_tickets = await self.ticket_service.get_recent_tickets(restaurant_id, limit=5)
        
        # Obter artigos populares da base de conhecimento
        popular_articles = await self.knowledge_service.get_popular_articles(restaurant_id, limit=5)
        
        # Verificar se há conversas de chatbot ativas
        active_chats = await self.chatbot_service.get_active_chats(restaurant_id, user_id)
    ...
    async def search_support_resources(self, restaurant_id: str, query: str) -> Dict[str, Any]:
...
class TicketService:
    ...
    async def update_ticket(self, ticket_id: str, update_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        protected_fields = ["id", "restaurant_id", "created_by", "created_at", "messages"]
        ...
        await self.db_service.update("support_tickets", {"id": ticket_id}, {"$set": update_data})
    ...
    async def add_message(self, ticket_id: str, message_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await self.event_bus.publish("ticket.message.created", {
            "ticket_id": ticket_id,
    ...
    async def get_recent_tickets(self, restaurant_id: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    async def search_tickets(self, restaurant_id: str, query: str) -> List[Dict[str, Any]]:
    ...
    async def _analyze_and_adjust_priority(self, ticket: Dict[str, Any], message_content: Optional[str] = None) -> None:
        ...
        try:
            ...
            if sentiment_result.get("sentiment") == "negative" and sentiment_result.get("score", 0) < -0.5:
    ...
    async def _analyze_sentiment(self, text: str) -> Dict[str, Any]:
        ...
        negative_words = ["problema", "erro", "falha", "bug", "ruim", "péssimo", "terrível", "horrível", "insatisfeito"]
        positive_words = ["bom", "ótimo", "excelente", "funciona", "satisfeito", "grato", "obrigado", "resolvido"]
        ...
        if total_words > 0:
            sentiment_score = (positive_count - negative_count) / min(total_words, 20)  # Normalizar
        else:
            sentiment_score = 0
...
class ChatbotService:
    ...
    async def start_chat(self, chat_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await self.event_bus.publish("chat.started", {
            "chat_id": chat_id,
            "restaurant_id": chat_data.get("restaurant_id"),
            "user_id": chat_data.get("user_id"),
        ...
        await self.add_message(chat_id, {
    ...
    async def add_message(self, chat_id: str, message_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Publicar evento de nova mensagem
        await self.event_bus.publish("chat.message.created", {
            "chat_id": chat_id,
            "message_id": message_id,
            "sender_id": message_data.get("sender_id"),
            "sender_type": message_data.get("sender_type")
    ...
    async def end_chat(self, chat_id: str, reason: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Atualizar conversa no banco de dados
        await self.db_service.update("support_chats", {"id": chat_id}, {"$set": update_data})
        
        # Publicar evento de encerramento de conversa
        await self.event_bus.publish("chat.ended", {
            "chat_id": chat_id,
    ...
    async def get_active_chats(self, restaurant_id: str, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    async def search_chats(self, restaurant_id: str, query: str) -> List[Dict[str, Any]]:
    ...
    async def transfer_to_human(self, chat_id: str, agent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        await self.event_bus.publish("chat.transferred", {
            "chat_id": chat_id,
            "agent_id": agent_id
        })
    ...
    async def _generate_welcome_message(self, chat: Dict[str, Any]) -> str:
        ...
        restaurant_config = await self.config_service.get_restaurant_config(restaurant_id)
    ...
    async def _generate_bot_response(self, chat: Dict[str, Any], user_message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if parsed_response.get("identified_issue"):
                new_context["identified_issues"].append(parsed_response.get("identified_issue"))
            
            # Adicionar artigos sugeridos, se houver
            if parsed_response.get("suggested_articles"):
                new_context["suggested_articles"].extend(parsed_response.get("suggested_articles"))
            ...
            await self.add_message(chat["id"], {
    ...
    def _prepare_prompt(self, chat: Dict[str, Any], user_message: Dict[str, Any]) -> str:
...
"""
    ...
    def _format_chat_for_ticket(self, chat: Dict[str, Any]) -> str:
        ...
        # Formatar histórico de mensagens
        chat_history = f"Histórico de conversa com {chat.get('user_name', 'Usuário')}:\n\n"
        
        for msg in messages:
            sender_type = msg.get("sender_type", "user")
            sender = "Usuário" if sender_type == "user" else "Bot"
            timestamp = datetime.fromisoformat(msg.get("created_at").replace('Z', '+00:00')).strftime("%d/%m/%Y %H:%M:%S")
...
class KnowledgeBaseService:
    ...
    async def get_article(self, article_id: str) -> Dict[str, Any]:
        ...
        article = await self.db_service.find_one("knowledge_articles", {"id": article_id})
    ...
    async def update_article(self, article_id: str, update_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Campos que não podem ser atualizados diretamente
        protected_fields = ["id", "restaurant_id", "created_by", "created_at", "view_count", "helpful_count", "not_helpful_count"]
        ...
        await self.db_service.update("knowledge_articles", {"id": article_id}, {"$set": update_data})
    ...
    async def get_popular_articles(self, restaurant_id: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    async def search_articles(self, restaurant_id: str, query: str) -> List[Dict[str, Any]]:
...
class SupportAnalyticsService:
    ...
    async def get_support_metrics(self, restaurant_id: str) -> Dict[str, Any]:
        ...
        tickets_open = await self.db_service.count(
            "support_tickets", 
            {"restaurant_id": restaurant_id, "status": "open"}
        )
        ...
        for ticket in resolved_tickets:
            ...
            for message in ticket.get("messages", []):
                if message.get("update_status") == "closed":
                    closed_at = datetime.fromisoformat(message["created_at"].replace('Z', '+00:00'))
            
            # Se não encontrar, usar updated_at
            if not closed_at and "updated_at" in ticket:
                closed_at = datetime.fromisoformat(ticket["updated_at"].replace('Z', '+00:00'))
            
            if closed_at:
                resolution_time = (closed_at - created_at).total_seconds() / 3600  # Converter para horas
...
class BedrockService:
    ...
    async def generate_response(self, prompt: str) -> str:
        ...
        if "problema" in prompt.lower() or "erro" in prompt.lower() or "não funciona" in prompt.lower():
            ...
            """
        elif "preço" in prompt.lower() or "custo" in prompt.lower() or "valor" in prompt.lower():
            ...
            """
        elif "lento" in prompt.lower() or "travando" in prompt.lower() or "péssimo" in prompt.lower() or "horrível" in prompt.lower():
...
async def list_tickets(
    ...
    return await support_service.ticket_service.get_tickets(
        restaurant_id, status, priority, category, assigned_to, created_by, page, page_size
    )
...
async def transfer_chat(
    chat_id: str = Path(..., description="ID da conversa"),
    agent_id: Optional[str] = Query(None, description="ID do agente para transferência"),
    current_user: User = Depends(get_current_user),
    support_service: SupportService = Depends(get_support_service)
):
    """Transfere uma conversa de chatbot para um atendente humano."""
...
async def update_article(
    ...
    return await support_service.knowledge_service.update_article(article_id, update_data)
...
async def whatsapp_webhook(
    ...
    processed_message = await support_service.chatbot_service.twilio_service.receive_message(message_data)
    
    # Processar mensagem em background
    background_tasks.add_task(process_whatsapp_message, processed_message, support_service)
...
async def send_whatsapp_message(
    ...
    """Envia uma mensagem via WhatsApp usando Twilio."""
    return await support_service.chatbot_service.twilio_service.send_message(to, message)

# Função para processar mensagem do WhatsApp em background
async def process_whatsapp_message(message: Dict[str, Any], support_service: SupportService):
    ...
    try:
        ...
        if active_chats and len(active_chats) > 0:
            ...
            # Adicionar mensagem à conversa
            await support_service.chatbot_service.add_message(chat["id"], {
