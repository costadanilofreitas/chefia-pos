...
class DeliveryOrderStatusChangedHandler(EventHandler):
    ...
    async def handle(self, event: Event) -> None:
        ...
        if (
            event.type == EventType.ORDER_STATUS_CHANGED
...
async def register_delivery_event_handlers():
    ...
    await event_bus.subscribe(
        EventType.ORDER_STATUS_CHANGED, OrderStatusChangedHandler()
    )
    await event_bus.subscribe(
        EventType.ORDER_STATUS_CHANGED, DeliveryOrderStatusChangedHandler()
    )
