...
class GoogleMapsService:
    ...
    async def geocode_address(self, address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if data["status"] != "OK":
                logger.error(
                    f"Geocoding failed: {data['status']} - {data.get('error_message', 'No error message')}"
                )
    ...
    async def calculate_route(
        ...
        if waypoints:
            params["waypoints"] = "optimize:true|" + "|".join(
        ...
        try:
            ...
            if data["status"] != "OK":
                logger.error(
                    f"Route calculation failed: {data['status']} - {data.get('error_message', 'No error message')}"
                )
    ...
    async def calculate_distance_matrix(
        ...
        params = {
            "origins": "|".join(
                [f"{origin['lat']},{origin['lng']}" for origin in origins]
            ),
            "destinations": "|".join(
        ...
        try:
            ...
            if data["status"] != "OK":
                logger.error(
                    f"Distance matrix calculation failed: {data['status']} - {data.get('error_message', 'No error message')}"
                )
                raise ValueError(
                    f"Failed to calculate distance matrix: {data['status']}"
                )
    ...
    async def get_place_details(self, place_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if data["status"] != "OK":
                logger.error(
                    f"Place details request failed: {data['status']} - {data.get('error_message', 'No error message')}"
                )
    ...
    async def snap_to_roads(
        ...
        params = {"path": path_str, "interpolate": "true", "key": self.api_key}
        ...
        try:
            ...
            if "snappedPoints" not in data:
                ...
                raise ValueError(
                    "Failed to snap points to roads: No snapped points returned"
                )
    ...
    async def find_nearest_addresses(
        ...
        try:
            ...
            if data["status"] != "OK" and data["status"] != "ZERO_RESULTS":
                logger.error(
                    f"Nearby search failed: {data['status']} - {data.get('error_message', 'No error message')}"
                )
    ...
    async def calculate_delivery_zone(
        ...
        for i in range(directions):
            ...
            lng = center["lng"] + (
                radius_km / (111.32 * math.cos(center["lat"] * 3.14159 / 180))
            ) * math.sin(angle)
        ...
        for point in points:
            try:
                ...
                route = await self.calculate_route(
                    origin=center, destination=point, mode=mode
                )
