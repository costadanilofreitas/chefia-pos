...
class DeliveryService:
    ...
    async def get_tracking_history(
        ...
        if order.status.value in [
            s.value
            for s in [DeliveryOrderStatus.IN_TRANSIT, DeliveryOrderStatus.DELIVERED]
        ]:
            # Em trânsito
            history.append(
                DeliveryTracking(
        ...
        if order.status == DeliveryOrderStatus.DELIVERED:
            ...
            history.append(
                DeliveryTracking(
                    ...
                    or (order.created_at + timedelta(minutes=40)),
    ...
    async def optimize_routes(self) -> List[DeliveryRoute]:
        ...
        pending_orders = await self.list_delivery_orders(
            status=DeliveryOrderStatus.PENDING
        )
...
class CourierService:
    ...
    async def update_courier(
        self, courier_id: str, data: Dict[str, Any]
    ) -> DeliveryCourier:
    ...
    async def update_courier_status(
        self, courier_id: str, status: CourierStatus
    ) -> DeliveryCourier:
    ...
    async def update_courier_location(
        self, courier_id: str, location: Dict[str, float]
    ) -> DeliveryCourier:
    ...
    async def get_courier_current_deliveries(
        self, courier_id: str
    ) -> List[DeliveryOrder]:
        ...
        orders = await delivery_service.list_delivery_orders(
            courier_id=courier_id, status=DeliveryOrderStatus.IN_TRANSIT
        )
