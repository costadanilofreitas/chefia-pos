...
async def create_delivery_order(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao criar pedido de delivery: {str(e)}")
...
async def assign_courier_to_order(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao atribuir entregador: {str(e)}")
...
async def create_courier(
    courier_data: CreateCourierRequest,
    current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao criar entregador: {str(e)}")
...
async def get_courier(
    ...
    if not courier:
        raise HTTPException(status_code=404, detail=f"Entregador {courier_id} não encontrado")
    return courier
...
async def update_courier_status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao atualizar status do entregador: {str(e)}")
...
async def update_courier_location(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao atualizar localização do entregador: {str(e)}")
...
async def list_couriers(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao listar entregadores: {str(e)}")

@router.get("/delivery/couriers/{courier_id}/deliveries", response_model=List[DeliveryOrder])
async def get_courier_deliveries(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter entregas do entregador: {str(e)}")

@router.get("/delivery/couriers/{courier_id}/performance", response_model=Dict[str, Any])
async def get_courier_performance(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter desempenho do entregador: {str(e)}")
...
async def create_zone(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao criar zona de entrega: {str(e)}")
...
async def get_zone(
    ...
    if not zone:
        raise HTTPException(status_code=404, detail=f"Zona de entrega {zone_id} não encontrada")
    return zone
...
async def list_zones(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao listar zonas de entrega: {str(e)}")

@router.post("/delivery/check-address", response_model=Dict[str, Any])
async def check_address_deliverable(
    address_data: CheckAddressRequest,
    current_user: User = Depends(get_current_user)
):
    """Verifica se um endereço está em uma zona de entrega."""
    try:
        is_deliverable = await zone_service.check_address_deliverable(address_data.address_id)
        ...
        if is_deliverable and address_data.order_value:
            ...
            delivery_fee = await delivery_service.calculate_delivery_fee(
                address_id=address_data.address_id,
                order_value=address_data.order_value
            )
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao verificar endereço: {str(e)}")
...
async def get_tracking_history(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter histórico de rastreamento: {str(e)}")
...
async def create_tracking_event(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao criar evento de rastreamento: {str(e)}")
...
async def public_track_order(
    ...
    try:
        ...
        delivery_order = await delivery_service.get_delivery_order_by_tracking(tracking_code)
        
        if not delivery_order:
            raise HTTPException(status_code=404, detail=f"Pedido com código de rastreamento {tracking_code} não encontrado")
        
        # Obter histórico de rastreamento
        tracking_history = await delivery_service.get_tracking_history(delivery_order.id)
        ...
        response = {
            ...
            "actual_delivery_time": delivery_order.actual_delivery_time.isoformat() if delivery_order.actual_delivery_time else None,
            "events": [
                {
