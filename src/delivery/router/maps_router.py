...
async def optimize_route(
    ...
    try:
        ...
        route_result = await maps_service.calculate_route(
            ...
            departure_time=(
                int(datetime.now().timestamp()) if request.consider_traffic else None
            ),
...
async def calculate_distance_matrix(
    origins: str = Query(
        ...,
        description="Lista de coordenadas de origem no formato 'lat1,lng1|lat2,lng2'",
    ),
    destinations: str = Query(
        ...,
        description="Lista de coordenadas de destino no formato 'lat1,lng1|lat2,lng2'",
    ),
