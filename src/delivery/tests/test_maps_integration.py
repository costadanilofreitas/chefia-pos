...
from delivery.router.maps_router import router, get_maps_service
...
class TestGoogleMapsIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.distance_matrix_response = {
            ...
            "rows": [
                {"elements": [{"distance": {"value": 1000}, "duration": {"value": 300}}]},
                {"elements": [{"distance": {"value": 2000}, "duration": {"value": 600}}]}
            ],
            "full_response": {}
        }
        ...
        async def mock_calculate_delivery_zone(center, **kwargs):
            return [
                ...
                {"lat": center["lat"] - 0.01, "lng": center["lng"] - 0.01},
                {"lat": center["lat"] + 0.01, "lng": center["lng"] - 0.01},
                {"lat": center["lat"] + 0.01, "lng": center["lng"] + 0.01}  # Fechar o polígono
            ]
        ...
        self.maps_service_mock.calculate_distance_matrix = mock_calculate_distance_matrix
    ...
    def test_geocode_address(self, mock_get_maps_service):
        ...
        async def run_test():
            ...
            data = response.json()
            self.assertEqual(data["formatted_address"], self.geocode_response["formatted_address"])
    ...
    def test_optimize_route(self, mock_get_maps_service):
        ...
        async def run_test():
            ...
            self.assertEqual(data["waypoint_order"], self.route_response["waypoint_order"])
            self.assertEqual(data["estimated_duration"], self.route_response["duration"]["value"])
            self.assertEqual(data["estimated_distance"], self.route_response["distance"]["value"])
    ...
    def test_calculate_delivery_zone(self, mock_get_maps_service):
        ...
        async def run_test():
            ...
            self.assertEqual(len(data["polygon"]), 5)  # 4 pontos + 1 para fechar o polígono
