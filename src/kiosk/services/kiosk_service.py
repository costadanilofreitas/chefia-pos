...
class KioskService:
    ...
    async def create_kiosk_config(self, config_data: KioskConfig) -> KioskConfig:
        ...
        await event_bus.publish(Event(
            event_type=EventType.KIOSK_CONFIG_CREATED,
            data=config_dict
        ))
    ...
    async def update_kiosk_config(self, kiosk_id: str, config_data: Dict[str, Any]) -> Optional[KioskConfig]:
        """Updates a kiosk configuration."""
        configs = self._load_data(KIOSK_CONFIG_FILE)
        config_index = next((i for i, c in enumerate(configs) if c["id"] == kiosk_id), None)
        ...
        event_bus = get_event_bus()
        await event_bus.publish(Event(
            event_type=EventType.KIOSK_CONFIG_UPDATED,
    ...
    async def delete_kiosk_config(self, kiosk_id: str) -> bool:
        ...
        event_bus = get_event_bus()
        await event_bus.publish(Event(
            event_type=EventType.KIOSK_CONFIG_DELETED,
    ...
    async def start_kiosk_session(self, kiosk_id: str) -> KioskSession:
        ...
        event_bus = get_event_bus()
        await event_bus.publish(Event(
            event_type=EventType.KIOSK_SESSION_STARTED,
            data=session_dict
        ))
    ...
    async def end_kiosk_session(self, session_id: str, order_id: Optional[str] = None, completed: bool = False) -> Optional[KioskSession]:
        ...
        event_bus = get_event_bus()
        await event_bus.publish(Event(
            event_type=EventType.KIOSK_SESSION_ENDED,
            data=ended_session.dict()
        ))
    ...
    async def _update_analytics(self, session: KioskSession) -> None:
        ...
        analytics_index = next((
            i for i, a in enumerate(analytics_list) 
        ...
        if analytics_index is None:
            ...
            if session.ended_at:
                ...
                current_avg = analytics_list[analytics_index]["average_session_time_seconds"]
                current_total = analytics_list[analytics_index]["total_sessions"] - 1  # Exclude current session
                
                if current_total > 0:
                    new_avg = ((current_avg * current_total) + session_time) / (current_total + 1)
                    analytics_list[analytics_index]["average_session_time_seconds"] = new_avg
                else:
                    analytics_list[analytics_index]["average_session_time_seconds"] = session_time
        
        self._save_data(KIOSK_ANALYTICS_FILE, analytics_list)

    async def get_kiosk_analytics(self, kiosk_id: str, start_date: Optional[str] = None, end_date: Optional[str] = None) -> List[KioskAnalytics]:
        ...
        if start_date:
            filtered_analytics = [a for a in filtered_analytics if a["date"] >= start_date]
        
        if end_date:
            filtered_analytics = [a for a in filtered_analytics if a["date"] <= end_date]
        
        return [KioskAnalytics(**a) for a in filtered_analytics]

    async def create_order_from_kiosk(self, session_id: str, order_data: KioskOrder) -> Dict[str, Any]:
        ...
        from src.product.models.product import OrderCreate, OrderItemCreate, OrderItemCustomization, OrderType
        
        # Create order items
        order_items = []
        for item in order_data.items:
            customizations = [
                OrderItemCustomization(name=c["name"], price_adjustment=c["price_adjustment"])
