...
async def create_employee(
    employee_data: EmployeeCreate, current_user: User = Depends(get_current_user)
):
...
async def query_employees(
    ...
    employment_type: Optional[EmploymentType] = Query(
        None, description="Tipo de vínculo"
    ),
    is_active: Optional[bool] = Query(None, description="Status ativo"),
    hire_date_start: Optional[date] = Query(
        None, description="Data de contratação inicial"
    ),
    hire_date_end: Optional[date] = Query(
        None, description="Data de contratação final"
    ),
...
async def update_delivery_assignment(
    ...
    try:
        ...
        if not assignment:
            raise HTTPException(
                status_code=404, detail="Atribuição de entrega não encontrada"
            )
        return assignment
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
...
@router.get(
    "/employees/{employee_id}/delivery-assignments",
    response_model=List[DeliveryAssignment],
)
...
async def update_attendance(
    ...
    try:
        ...
        if not attendance:
            raise HTTPException(
                status_code=404, detail="Registro de ponto não encontrado"
            )
        return attendance
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
...
@router.get(
    "/employees/{employee_id}/attendance", response_model=List[EmployeeAttendance]
)
async def get_employee_attendance(
    ...
    _check_permissions(current_user, ["attendance.read"])
    return await employee_service.get_employee_attendance(
...
async def update_performance_evaluation(
    ...
    try:
        ...
        if not evaluation:
            raise HTTPException(
                status_code=404, detail="Avaliação de desempenho não encontrada"
            )
        return evaluation
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
...
@router.get(
    "/employees/{employee_id}/performance", response_model=List[EmployeePerformance]
)
