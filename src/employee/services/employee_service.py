...
DELIVERY_ASSIGNMENTS_DATA_FILE = os.path.join(
    "/home/ubuntu/pos-modern/data", "delivery_assignments.json"
)
ATTENDANCE_DATA_FILE = os.path.join(
    "/home/ubuntu/pos-modern/data", "employee_attendance.json"
)
PERFORMANCE_DATA_FILE = os.path.join(
    "/home/ubuntu/pos-modern/data", "employee_performance.json"
)
...
class EmployeeService:
    ...
    async def create_employee(
        self, employee_data: EmployeeCreate, user_id: str, user_name: str
    ) -> Employee:
        """
    ...
    async def delete_employee(
        self, employee_id: str, user_id: str, user_name: str
    ) -> bool:
        """
    ...
    def _employee_matches_query(
        self, employee_data: Dict[str, Any], query: EmployeeQuery
    ) -> bool:
        """
        ...
        if (
            query.name
            and query.name.lower() not in employee_data.get("name", "").lower()
        ):
            return False
        ...
        if query.employment_type and query.employment_type != employee_data.get(
            "employment_type"
        ):
            return False

        # Verifica status ativo
        if query.is_active is not None and query.is_active != employee_data.get(
            "is_active", True
        ):
            return False
    ...
    async def create_delivery_assignment(
        self, assignment_data: DeliveryAssignmentCreate, user_id: str, user_name: str
    ) -> DeliveryAssignment:
        """
        ...
        if not employee:
            raise ValueError(
                f"Funcionário não encontrado: {assignment_data.employee_id}"
            )
            ...
            if assignment.get("id") == assignment_id:
                ...
                if "status" in update_dict:
                    ...
                    if (
                        ...
                        and not current_assignment.started_at
                    ):
                        update_dict["started_at"] = datetime.now()

                    if (
                        new_status == "completed"
                        and not current_assignment.completed_at
                    ):
                        ...
                        if current_assignment.status != "completed":
                            await self._process_delivery_payment(
                                current_assignment, update_dict.get("tip_amount")
                            )
    ...
    async def _process_delivery_payment(
        self, assignment: DeliveryAssignment, tip_amount: Optional[float] = None
    ) -> None:
        """
        ...
        if employee.payment_frequency == "per_delivery":
            try:
                ...
                await accounts_service.create_payable_for_employee(
                    ...
                    due_date=date.today()
                    + timedelta(days=1),  # Pagamento no dia seguinte
                    created_by="system",
                )
    ...
    async def record_attendance(
        self, attendance_data: EmployeeAttendanceCreate, user_id: str, user_name: str
    ) -> EmployeeAttendance:
        """
        ...
        if not employee:
            raise ValueError(
                f"Funcionário não encontrado: {attendance_data.employee_id}"
            )
    ...
    async def update_attendance(
        self,
        attendance_id: str,
        update_data: EmployeeAttendanceUpdate,
        user_id: str,
        user_name: str,
            ...
            if attendance.get("id") == attendance_id:
                ...
                if "clock_out" in update_dict and update_dict["clock_out"]:
                    ...
                    if current_attendance.break_start and current_attendance.break_end:
                        break_seconds = (
                            current_attendance.break_end
                            - current_attendance.break_start
                        ).total_seconds()
    ...
    async def get_employee_attendance(
        ...
        for attendance_data in self.attendance_records:
            if attendance_data.get("employee_id") == employee_id:
                attendance_date = (
                    datetime.strptime(attendance_data.get("date"), "%Y-%m-%d").date()
                    if isinstance(attendance_data.get("date"), str)
                    else attendance_data.get("date")
                )
    ...
    async def create_performance_evaluation(
        self, evaluation_data: EmployeePerformanceCreate, user_id: str, user_name: str
    ) -> EmployeePerformance:
        """
        ...
        if not employee:
            raise ValueError(
                f"Funcionário não encontrado: {evaluation_data.employee_id}"
            )
    ...
    async def update_performance_evaluation(
        self,
        evaluation_id: str,
        update_data: EmployeePerformanceUpdate,
        user_id: str,
        user_name: str,
    ) -> Optional[EmployeePerformance]:
        """
    ...
    async def generate_salary_payables(
        self, payment_date: date, user_id: str, user_name: str
    ) -> List[str]:
        """
    ...
    async def get_employee_statistics(self) -> Dict[str, Any]:
        ...
        recent_deliveries = [
            d
            for d in self.delivery_assignments
            if (
                datetime.fromisoformat(d.get("assigned_at"))
                if isinstance(d.get("assigned_at"), str)
                else d.get("assigned_at")
            )
            >= thirty_days_ago
        ]
        ...
        completed_deliveries = sum(
            1 for d in recent_deliveries if d.get("status") == "completed"
        )
        cancelled_deliveries = sum(
            1 for d in recent_deliveries if d.get("status") == "cancelled"
        )
        ...
        recent_attendance = [
            ...
            if (
                datetime.fromisoformat(a.get("date"))
        ...
        overtime_count = sum(
            1 for a in recent_attendance if a.get("is_overtime", False)
        )

        overtime_rate = (
            (overtime_count / total_attendance * 100) if total_attendance > 0 else 0
        )
    ...
    async def _publish_employee_event(
        ...
        """
        event = EmployeeEvent(
            type=event_type, employee_id=employee_id, user_id=user_id, data=data
        )
