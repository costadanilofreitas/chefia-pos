    ...
    async def process_request(self, request: ApiRequest) -> ApiRequest:
        ...
        if count >= self.rate_limit:
            raise RateLimitExceededError(
                f"Limite de {self.rate_limit} requisições por {self.time_window} segundos excedido"
            )
...
class LoggingMiddleware(Middleware):
    ...
    async def process_request(self, request: ApiRequest) -> ApiRequest:
        ...
        self.logger.info(
            f"Requisição recebida: {request.method} {request.path} de {request.client_ip}"
        )
        return request
    ...
    async def process_response(
        ...
        self.logger.info(
            f"Resposta enviada: {response.status_code} para {request.method} {request.path}"
        )
        return response
...
class GatewayRouter:
    ...
    def add_route(
        self, path: str, target_service: str, methods: List[RequestMethod] = None
    ):
    ...
    async def route_request(self, request: ApiRequest) -> ApiResponse:
        ...
        try:
            ...
            response = ApiResponse(
                status_code=(
                    ...
                    else 429 if isinstance(e, RateLimitExceededError) else 400
                ),
                body={"error": str(e)},
            )
        ...
        for route_path, route_info in self.routes.items():
            if (
                request.path.startswith(route_path)
        ...
        if not target_service:
            response = ApiResponse(
                status_code=404, body={"error": f"Rota não encontrada: {request.path}"}
            )
            return await self._process_response_middlewares(request, response)
        ...
        try:
            ...
            response = ApiResponse(
                status_code=503, body={"error": f"Serviço indisponível: {str(e)}"}
            )
            return await self._process_response_middlewares(request, response)
    ...
    async def _process_response_middlewares(
        self, request: ApiRequest, response: ApiResponse
    ) -> ApiResponse:
    ...
    async def _call_product_service(self, request: ApiRequest) -> ApiResponse:
        ...
        if (
            request.path == "/api/external/products"
            ...
            request.path.startswith("/api/external/products/")
            and request.method == RequestMethod.GET
        ):
            ...
            return ApiResponse(
                status_code=404, body={"error": "Produto não encontrado"}
            )
    ...
    async def _call_order_service(self, request: ApiRequest) -> ApiResponse:
        ...
        # Simulação de resposta
        if (
            request.path == "/api/external/orders"
            ...
            request.path.startswith("/api/external/orders/")
            and request.method == RequestMethod.GET
        ):
            ...
            return ApiResponse(status_code=404, body={"error": "Pedido não encontrado"})

    async def _call_delivery_service(self, request: ApiRequest) -> ApiResponse:
        """Chama o serviço de delivery."""
        # Simulação de resposta
        if (
            request.path == "/api/external/delivery/track"
            and request.method == RequestMethod.POST
        ):
            ...
            return ApiResponse(
                status_code=404, body={"error": "Informação de delivery não encontrada"}
            )
...
class AuthService:
    ...
    async def create_token(self, credentials: Dict[str, str]) -> str:
        ...
        if (
            credentials.get("client_id") == "test_client"
            and credentials.get("client_secret") == "test_secret"
        ):
...
class ApiGateway:
    ...
    def __init__(self):
        ...
        self.router.add_middleware(
            RateLimitingMiddleware(rate_limit=100, time_window=60)
        )

        # Configurar transformadores
        request_transformers = {"/api/external/orders": self._transform_order_request}
        ...
        self.router.add_route(
            "/api/external/products", "product_service", [RequestMethod.GET]
        )
        self.router.add_route(
            "/api/external/orders",
            "order_service",
    ...
    def _transform_product_response(
