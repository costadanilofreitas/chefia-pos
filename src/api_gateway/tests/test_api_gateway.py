...
class TestApiGateway(unittest.TestCase):
    ...
    async def test_authentication_middleware(self, mock_validate_token):
        ...
        self.assertEqual(
            processed_request.authenticated_user,
    ...
    async def test_logging_middleware(self, mock_get_logger):
        ...
        response = ApiResponse(status_code=200, body={"message": "Success"})
    ...
    async def test_route_request_to_product_service(self, mock_call_product_service):
        ...
        mock_response = ApiResponse(
            status_code=200, body=[{"id": "1", "name": "Test Product"}]
        )
    ...
    async def test_route_request_to_order_service(self, mock_call_order_service):
        ...
        # Configurar mock
        mock_response = ApiResponse(
            status_code=201, body={"id": "1", "status": "created"}
        )
    ...
    async def test_route_request_to_delivery_service(self, mock_call_delivery_service):
        ...
        # Configurar mock
        mock_response = ApiResponse(
            status_code=200, body={"tracking_code": "ABC123", "status": "in_transit"}
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.body, {"tracking_code": "ABC123", "status": "in_transit"}
        )
    ...
    async def test_handle_request(self):
        ...
        with patch.object(
            self.api_gateway.router, "route_request"
