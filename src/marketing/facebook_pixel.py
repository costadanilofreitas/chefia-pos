...
class FacebookPixelIntegration:
    ...
    def __init__(self, pixel_id: str = None, access_token: str = None):
        ...
        if not self.pixel_id:
            logger.warning("ID do pixel não configurado. A integração não funcionará corretamente.")
        
        if not self.access_token:
            logger.warning("Token de acesso não configurado. A integração não funcionará corretamente.")
        
        logger.info(f"Integração com Facebook Pixel inicializada. Pixel ID: {self.pixel_id}")
    ...
    def track_event(self, event_name: str, user_data: Dict[str, Any], custom_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Erro ao rastrear evento '{event_name}': {response.status_code}, {response.text}")
                return {
                    "success": False,
                    "event_name": event_name,
