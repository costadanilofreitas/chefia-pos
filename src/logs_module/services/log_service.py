...
class LogService:
    ...
    async def log(self, entry: LogEntry) -> LogEntry:
        ...
        if (
            entry.source in self.log_config.excluded_sources
            or entry.level in self.log_config.excluded_levels
        ):
            return entry
    ...
    async def query_logs(self, query: LogQuery) -> List[LogEntry]:
        ...
        if query.sort_by:
            ...
            filtered_results.sort(
                key=lambda x: x.get(query.sort_by, ""), reverse=reverse
            )
    ...
    async def _read_logs_from_file(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(self._log_files) > 10:
                ...
                oldest_file = min(
                    self._log_files.keys(), key=lambda k: os.path.getmtime(k)
                )
    ...
    def _matches_query(self, log_dict: Dict[str, Any], query: LogQuery) -> bool:
        ...
        # Check level
        if query.levels and log_dict.get("level") not in [
            level.value for level in query.levels
        ]:
            return False

        # Check source
        if query.sources and log_dict.get("source") not in [
            source.value for source in query.sources
        ]:
            return False
    ...
    async def get_log_stats(self, query: Optional[LogQuery] = None) -> LogStats:
        ...
        return LogStats(
            ...
            entries_by_level={
                level.value: entries_by_level.get(level, 0) for level in LogLevel
            },
            entries_by_source={
                source.value: entries_by_source.get(source, 0) for source in LogSource
            },
    ...
    async def export_logs(self, query: LogQuery, format: str = "json") -> str:
        ...
        if format == "json":
            ...
            with open(export_path, "w") as f:
                for log in logs:
                    f.write(
                        f"[{log.timestamp.isoformat()}] [{log.level.upper()}] [{log.source}] [{log.module}] {log.message}\n"
                    )
                    if log.details:
                        f.write(f"  Details: {json.dumps(log.details, default=str)}\n")
                    if log.user_id or log.user_name:
                        f.write(
                            f"  User: {log.user_name or ''} ({log.user_id or ''})\n"
                        )
                    if log.ip_address:
...
async def log_debug(
    message: str, source: Union[LogSource, str], module: str, **kwargs
) -> LogEntry:
