...
class InventoryService:
    ...
    async def create_inventory_item(
        self, item_create: InventoryItemCreate
    ) -> InventoryItem:
    ...
    async def list_inventory_items(
        ...
        if search:
            ...
            items = [
                ...
                or (item.sku and search_lower in item.sku.lower())
                or (item.barcode and search_lower in item.barcode.lower())
    ...
    async def update_inventory_item(
        self, item_id: uuid.UUID, item_update: InventoryItemUpdate
    ...
    async def create_transaction(
        self, transaction_create: InventoryTransactionCreate
    ) -> InventoryTransaction:
        ...
        unit_cost = (
            transaction_create.unit_cost
            if transaction_create.unit_cost is not None
    ...
    async def approve_transaction(
        self, transaction_id: uuid.UUID, approver_id: uuid.UUID
    ) -> InventoryTransaction:
        ...
        if transaction.transaction_type == TransactionType.PURCHASE:
            ...
            and transaction.value_change != 0
        ):
            await self._create_financial_entry(
    ...
    async def approve_loss(
        self, loss_id: uuid.UUID, approver_id: uuid.UUID
    ) -> InventoryLoss:
    ...
    async def create_inventory_count(
        self, count_create: InventoryCountCreate
    ) -> InventoryCount:
        ...
        for item_count in count_create.items:
            ...
            variance_percentage = (
                (variance / item_count.expected_quantity * 100)
    ...
    async def approve_inventory_count(
        self, count_id: uuid.UUID, approver_id: uuid.UUID
    ) -> InventoryCount:
    ...
    async def _update_item_stock(
        self, item: InventoryItem, new_stock: float, unit_cost: float
    ) -> InventoryItem:
        ...
        if item.current_stock <= item.reorder_point:
            logger.warning(
                f"Item {item.name} (ID: {item.id}) is below reorder point. Current stock: {item.current_stock}, Reorder point: {item.reorder_point}"
            )
    ...
    async def _create_financial_entry(
        ...
        if is_posted and self.accounts_service:
            try:
                ...
                logger.info(
                    f"Financial entry {entry_id} posted successfully: {description}"
                )
