...
class KDSKeyboardHandler:
    ...
    def __init__(self, event_bus: EventBus):
        ...
        self.event_bus.subscribe(PeripheralEventType.KEYBOARD_COMMAND, self._handle_keyboard_command)
    ...
    async def _handle_keyboard_command(self, event: Event):
        ...
        try:
            ...
            logger.info(f"Comando de teclado recebido: {command} do dispositivo {device_id}")
    ...
    async def _next_order(self):
        ...
        logger.info(f"Navegado para o pedido {self.current_order_index + 1} de {len(self.orders)}")
    ...
    async def _next_item(self):
        ...
        logger.info(f"Navegado para o item {self.current_item_index + 1} de {len(items)}")
    ...
    async def _advance_status(self):
        ...
        if next_status:
            ...
            logger.info(f"Status do item {item_id} atualizado: {current_status} -> {next_status}")
        else:
            logger.info(f"Não é possível avançar o status do item {item_id} (status atual: {current_status})")
    ...
    async def _mark_all_ready(self):
        ...
        logger.info(f"Todos os itens do pedido {current_order.get('id')} marcados como prontos")
    ...
    async def _highlight_current_selection(self):
        ...
        item_id = None
        if current_order.get("items") and self.current_item_index < len(current_order.get("items")):
