...
class KDSIntelligenceService:
    ...
    def register_event_handlers(self):
        ...
        self.event_bus.subscribe(
            "order_item.status_changed", self.handle_item_status_change
        )
    ...
    async def calculate_preparation_times(self, order: Dict[str, Any]) -> None:
        ...
        if not items:
            logger.warning(
                f"KDS Intelligence: Pedido {order.get('order_id')} não possui itens"
            )
            return
        ...
        longest_prep_time, longest_item_id = self.find_longest_preparation_time(
            item_prep_times
        )
    ...
    def adjust_time_for_customizations(
        self, base_time: int, item: Dict[str, Any]
    ) -> int:
        """
    ...
    def find_longest_preparation_time(
        self, prep_times: Dict[str, int]
    ) -> Tuple[int, str]:
        """
    ...
    def calculate_start_times(
        self, prep_times: Dict[str, int], longest_time: int
    ) -> Dict[str, int]:
        """
    ...
    async def adjust_start_times(
        self, start_times: Dict[str, int], items: List[Dict[str, Any]]
    ) -> Dict[str, int]:
        """
    ...
    async def save_preparation_schedule(
    ...
    async def record_preparation_start(self, order_id: str, item_id: str) -> None:
        ...
        # Log para simulação
        logger.info(
            f"KDS Intelligence: Registrando início de preparo do item {item_id} do pedido {order_id}"
        )
    ...
    async def record_preparation_end(self, order_id: str, item_id: str) -> None:
        ...
        # Log para simulação
        logger.info(
            f"KDS Intelligence: Registrando conclusão de preparo do item {item_id} do pedido {order_id}"
        )
    ...
    async def update_preparation_times(
        ...
        pending_items = [
            ...
            if item.get("item_id") != changed_item_id
            and item.get("status") not in ["ready", "delivered"]
        ]

        if not pending_items:
            logger.info(
                f"KDS Intelligence: Todos os itens do pedido {order.get('order_id')} estão prontos ou em preparo"
            )
            return
        ...
        # Log para simulação
        logger.info(
            f"KDS Intelligence: Atualizando tempos de preparo para {len(pending_items)} itens pendentes do pedido {order.get('order_id')}"
        )

    async def prioritize_orders(
        ...
        for order in active_orders:
            ...
            prioritized_orders.append(
                {"order": order, "priority_score": priority_score}
            )
    ...
    async def get_preparation_metrics(
...
def get_kds_intelligence_service(
    order_service=None, event_bus=None
) -> KDSIntelligenceService:
    """
