...
class KDSService:
    ...
    async def process_new_order(self, order_data: Dict[str, Any]) -> KDSOrder:
        ...
        kds_order.estimated_completion_time = datetime.now() + timedelta(
            minutes=adjusted_prep_time
        )
    ...
    async def update_order_status(
        self, order_id: str, update_data: KDSOrderUpdate
    ) -> Optional[KDSOrder]:
    ...
    async def update_item_status(
        self, order_id: str, item_id: str, update_data: KDSOrderItemUpdate
        ...
        if update_data.status is not None:
            ...
            if all(s == update_data.status for s in all_items_status):
                ...
                if (
                    update_data.status == KDSOrderStatus.PREPARING
                    and not order.started_at
                ):
                    order.started_at = datetime.now()
                elif (
                    update_data.status == KDSOrderStatus.READY
    ...
    async def create_session(self, session_data: KDSSessionCreate) -> KDSSession:
        ...
        session = KDSSession(
            name=session_data.name, station_type=session_data.station_type
        )
    ...
    async def update_session(
        self, session_id: str, update_data: KDSSessionUpdate
    ) -> Optional[KDSSession]:
        """Atualiza uma sessão."""
        sessions = await self._load_sessions()
        session_index = next(
    ...
    async def _update_stats(self) -> None:
        ...
        preparing_orders = [
            o for o in orders if o["status"] == KDSOrderStatus.PREPARING
        ]
        ready_orders = [o for o in orders if o["status"] == KDSOrderStatus.READY]
        delivered_orders = [
            o for o in orders if o["status"] == KDSOrderStatus.DELIVERED
        ]
        cancelled_orders = [
            o for o in orders if o["status"] == KDSOrderStatus.CANCELLED
        ]
        ...
        completed_orders = [
            o
            for o in orders
            if o["status"] in [KDSOrderStatus.READY, KDSOrderStatus.DELIVERED]
        ]
        ...
        if len(pending_orders) + len(preparing_orders) > 0:
            ...
            current_load = min(
                100, (len(pending_orders) + len(preparing_orders) * 2) * 10
            )
    ...
    async def _save_order(self, order: KDSOrder) -> None:
        ...
        order_index = next(
            (i for i, o in enumerate(orders) if o["id"] == order.id), None
        )
