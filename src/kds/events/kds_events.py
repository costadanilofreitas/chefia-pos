...
class KDSEventPublisher:
    ...
    async def publish_order_received(self, kds_order: KDSOrder) -> None:
        ...
        event = Event(
            event_type=EventType.KDS_ORDERS_UPDATED,  # Using core EventType
            metadata={"source": "kds", "event_subtype": KDSEventType.KDS_ORDER_RECEIVED},
            data=kds_order.dict(),
        )
    ...
    async def publish_order_status_changed(
        ...
        event = Event(
            event_type=EventType.KDS_ORDERS_UPDATED,  # Using core EventType
            metadata={"source": "kds", "event_subtype": KDSEventType.KDS_ORDER_STATUS_CHANGED},
            data={
                "order_id": order_id,
                "status": status,
    ...
    async def publish_item_status_changed(
        ...
        event = Event(
            event_type=EventType.KDS_ORDERS_UPDATED,  # Using core EventType
            metadata={"source": "kds", "event_subtype": KDSEventType.KDS_ITEM_STATUS_CHANGED},
            data={
                "order_id": order_id,
                "item_id": item_id,
                "status": status,
    ...
    async def publish_session_created(self, session_data: Dict[str, Any]) -> None:
        ...
        event = Event(
            event_type=EventType.KDS_ORDERS_UPDATED,  # Using core EventType
            metadata={"source": "kds", "event_subtype": KDSEventType.KDS_SESSION_CREATED},
            data=session_data,
        )
        await self.event_bus.publish(event)
    ...
    async def publish_session_updated(self, session_data: Dict[str, Any]) -> None:
        ...
        event = Event(
            event_type=EventType.KDS_ORDERS_UPDATED,  # Using core EventType
            metadata={"source": "kds", "event_subtype": KDSEventType.KDS_SESSION_UPDATED},
            data=session_data,
        )
        await self.event_bus.publish(event)
    ...
    async def publish_stats_updated(self, stats_data: Dict[str, Any]) -> None:
        ...
        event = Event(
            event_type=EventType.KDS_ORDERS_UPDATED,  # Using core EventType
            metadata={"source": "kds", "event_subtype": KDSEventType.KDS_STATS_UPDATED},
            data=stats_data,
        )
        await self.event_bus.publish(event)
