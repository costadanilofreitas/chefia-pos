...
async def create_stock_item_endpoint(
    item_create: StockItemCreate, service: StockService = Depends(lambda: stock_service)
):
...
async def get_stock_item_endpoint(
    item_id: uuid.UUID, service: StockService = Depends(lambda: stock_service)
):
...
async def update_stock_item_endpoint(
    ...
    updated_item = await service.update_stock_item(
        item_id, item_update.dict(exclude_unset=True)
    )
    if not updated_item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Stock item not found"
...
@router.post(
    "/movements/", response_model=StockMovement, status_code=status.HTTP_201_CREATED
)
...
async def get_stock_levels_endpoint(
    low_stock_only: bool = False, service: StockService = Depends(lambda: stock_service)
):
