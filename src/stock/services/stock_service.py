...
class StockService:
    ...
    async def update_stock_item(self, item_id: uuid.UUID, item_update_data: dict) -> Optional[StockItem]:
    ...
    async def record_movement(self, movement_create: StockMovementCreate) -> StockMovement:
        ...
        item.current_quantity += movement.quantity # Positive for entry, negative for exit/sale/adjustment
    ...
    async def get_stock_levels(self, low_stock_only: bool = False) -> List[StockLevel]:
        ...
        for item in _stock_items_db.values():
            is_low = False
            if item.low_stock_threshold is not None and item.current_quantity < item.low_stock_threshold:
                is_low = True
    ...
    async def get_movement_history(self, item_id: Optional[uuid.UUID] = None) -> List[StockMovement]:
