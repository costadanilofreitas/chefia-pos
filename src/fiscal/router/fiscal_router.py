...
async def update_nfce(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao atualizar NFC-e: {str(e)}")
...
async def register_mfe_equipment(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao registrar equipamento MFE: {str(e)}")
...
async def update_mfe_equipment(
    ...
    try:
        equipment = mfe_service.update_equipment(equipment_id, request.dict(exclude_unset=True))
        if not equipment:
            raise HTTPException(status_code=404, detail="Equipamento MFE não encontrado")
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao atualizar equipamento MFE: {str(e)}")
...
async def activate_mfe_equipment(
    ...
    success, message = mfe_service.activate_equipment(equipment_id, request.activation_code)
...
@router.post("/mfe/equipment/{equipment_id}/configuration", response_model=Dict[str, Any])
...
@router.get("/mfe/equipment/{equipment_id}/configuration", response_model=List[MFEConfiguration])
...
async def complete_mfe_maintenance(
    ...
    success, message = mfe_service.complete_maintenance(schedule_id, request.completion_notes)
...
async def register_accounting_provider(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao registrar provedor: {str(e)}")
...
async def update_accounting_provider(
    ...
    try:
        provider = accounting_service.update_provider(provider_id, request.dict(exclude_unset=True))
        if not provider:
            raise HTTPException(status_code=404, detail="Provedor não encontrado")
...
async def update_accounting_mapping(
    ...
    try:
        mapping = accounting_service.update_mapping(mapping_id, request.dict(exclude_unset=True))
        if not mapping:
            raise HTTPException(status_code=404, detail="Mapeamento não encontrado")
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao atualizar mapeamento: {str(e)}")

@router.post("/accounting/export", response_model=AccountingExportBatch, status_code=201)
async def create_export_batch(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao criar lote de exportação: {str(e)}")
...
async def update_accounting_schedule(
    ...
    try:
        schedule = accounting_service.update_schedule(schedule_id, request.dict(exclude_unset=True))
        if not schedule:
            raise HTTPException(status_code=404, detail="Agendamento não encontrado")
