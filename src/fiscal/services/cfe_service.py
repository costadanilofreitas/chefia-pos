...
class CFeService:
    ...
    def get_state_rule(self, state_code: str) -> CFeStateRule:
        ...
        rule = self.db_service.find_one(
            "fiscal_cfe_state_rules", {"state_code": state_code}
        )
    ...
    def create_cfe(self, cfe_data: Dict[str, Any]) -> CFeDocument:
        ...
        if state_rule.requires_mfe:
            ...
            if mfe_equipment:
                ...
                raise ValueError(
                    f"Nenhum equipamento MFE disponível para o estado {cfe_data['state_code']}"
                )
        else:
    ...
    def _validate_cfe_data(self, cfe_data: Dict[str, Any]) -> None:
        ...
        if (
            abs(total_payments - cfe_data["total_value"]) > 0.01
        ):  # Tolerância de 1 centavo
            raise ValueError(
                f"O total dos pagamentos ({total_payments}) não corresponde ao valor total ({cfe_data['total_value']})"
            )
    ...
    def update_cfe(
        self, cfe_id: str, update_data: Dict[str, Any]
    ) -> Optional[CFeDocument]:
        """
        Atualiza um CF-e
        ...
        if cfe_data["status"] not in [CFeStatus.DRAFT, CFeStatus.ERROR]:
            raise ValueError(
                f"CF-e com status {cfe_data['status']} não pode ser atualizado"
            )
        ...
        self.db_service.update_one(
            "fiscal_cfe_documents", {"id": cfe_id}, {"$set": update_data}
        )
    ...
    def send_cfe(self, cfe_id: str) -> Tuple[bool, str, Optional[CFeResponse]]:
        ...
        try:
            ...
            self.update_cfe(
                cfe_id, {"status": CFeStatus.PENDING, "xml_content": xml_content}
            )
            ...
            new_status = (
                CFeStatus.AUTHORIZED if response_data["success"] else CFeStatus.REJECTED
            )
            self.update_cfe(cfe_id, {"status": new_status, "response": response.dict()})
    ...
    def _simulate_mfe_response(
        self, cfe: CFeDocument, state_rule: CFeStateRule
    ) -> Dict[str, Any]:
        """
    ...
    def cancel_cfe(
        self, cfe_id: str, reason: str
    ) -> Tuple[bool, str, Optional[CFeEvent]]:
        """
        Cancela um CF-e
        ...
        try:
            ...
            self.db_service.update_one(
                "fiscal_cfe_events", {"id": event_id}, {"$set": event_data}
            )
    ...
    def _simulate_sat_event_response(
        self, cfe: CFeDocument, event_type: str
    ) -> Dict[str, Any]:
        """
    ...
    def _simulate_mfe_event_response(
        self, cfe: CFeDocument, event_type: str
    ) -> Dict[str, Any]:
        """
    ...
    def get_cfe_for_accounting(
        ...
        # Busca os documentos no banco de dados
        documents = self.db_service.find(
            "fiscal_cfe_documents", filters, sort=[("issue_date", 1)]
        )
