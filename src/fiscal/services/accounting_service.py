...
from src.fiscal.models.accounting_models import (
    ...
    DocumentType, AccountingMapping, AccountingProvider, AccountingSchedule
)
...
class AccountingService:
    ...
    def _validate_provider_data(self, provider_data: Dict[str, Any]) -> None:
        ...
        """
        required_fields = ["name", "provider_type", "api_url", "auth_method", "export_format"]
        ...
        existing = self.db_service.find_one("fiscal_accounting_providers", {
            "name": provider_data["name"]
        })
        
        if existing:
            raise ValueError(f"Já existe um provedor com o nome {provider_data['name']}")
        
        # Verifica se o formato de exportação é suportado
        supported_formats = ["json", "xml", "csv"]
        if provider_data["export_format"] not in supported_formats:
            raise ValueError(f"Formato de exportação não suportado: {provider_data['export_format']}. Formatos suportados: {', '.join(supported_formats)}")
    ...
    def get_provider(self, provider_id: str) -> Optional[AccountingProvider]:
        ...
        """
        provider_data = self.db_service.find_one("fiscal_accounting_providers", {"id": provider_id})
    ...
    def update_provider(self, provider_id: str, update_data: Dict[str, Any]) -> Optional[AccountingProvider]:
        """
        ...
        self.db_service.update_one("fiscal_accounting_providers", {"id": provider_id}, {"$set": update_data})
    ...
    def _validate_mapping_data(self, mapping_data: Dict[str, Any]) -> None:
        ...
        existing = self.db_service.find_one("fiscal_accounting_mappings", {
            "source_code": mapping_data["source_code"]
        })
        
        if existing:
            raise ValueError(f"Já existe um mapeamento para o código de origem {mapping_data['source_code']}")
    ...
    def get_mapping(self, mapping_id: str) -> Optional[AccountingMapping]:
        ...
        """
        mapping_data = self.db_service.find_one("fiscal_accounting_mappings", {"id": mapping_id})
    ...
    def update_mapping(self, mapping_id: str, update_data: Dict[str, Any]) -> Optional[AccountingMapping]:
        """
        ...
        # Atualiza no banco de dados
        self.db_service.update_one("fiscal_accounting_mappings", {"id": mapping_id}, {"$set": update_data})
        
        # Retorna o mapeamento atualizado
        return AccountingMapping(**mapping_data)
    
    def create_export_batch(self, reference_period: str, export_destination: str, 
                           created_by: str, notes: Optional[str] = None) -> AccountingExportBatch:
        """
    ...
    def get_export_batch(self, batch_id: str) -> Optional[AccountingExportBatch]:
        ...
        """
        batch_data = self.db_service.find_one("fiscal_accounting_export_batches", {"id": batch_id})
    ...
    def update_export_batch(self, batch_id: str, update_data: Dict[str, Any]) -> Optional[AccountingExportBatch]:
        """
        ...
        # Atualiza no banco de dados
        self.db_service.update_one("fiscal_accounting_export_batches", {"id": batch_id}, {"$set": update_data})
    ...
    def process_export_batch(self, batch_id: str) -> Tuple[bool, str, Optional[str]]:
        ...
        if batch.status not in [AccountingExportStatus.PENDING, AccountingExportStatus.FAILED]:
            return False, f"Lote com status {batch.status} não pode ser processado", None
        
        try:
            # Atualiza o status para PROCESSING
            self.update_export_batch(batch_id, {"status": AccountingExportStatus.PROCESSING})
            ...
            nfce_documents = self.nfce_service.get_nfce_for_accounting(start_date, end_date)
            cfe_documents = self.cfe_service.get_cfe_for_accounting(start_date, end_date)
            ...
            if not export_items:
                self.update_export_batch(batch_id, {
            ...
            for item in export_items:
                ...
                self.db_service.update_one("fiscal_accounting_export_items", 
                                          {"id": item.id}, 
            ...
            return True, f"Exportação concluída com sucesso. {len(export_items)} documentos exportados.", export_file_path
    ...
    def _prepare_nfce_export_data(self, nfce) -> Dict[str, Any]:
        ...
        export_data = {
            ...
            "issuer": {
                "name": nfce.issuer.name,
                "cnpj": nfce.issuer.cnpj
            },
            "total_value": nfce.total_value,
    ...
    def _prepare_cfe_export_data(self, cfe) -> Dict[str, Any]:
        ...
        export_data = {
            ...
            "issuer": {
                "name": cfe.issuer.name,
                "cnpj": cfe.issuer.cnpj
            },
            "total_value": cfe.total_value,
    ...
    def _generate_export_file(self, batch: AccountingExportBatch, items: List[AccountingExportItem], 
                             provider: AccountingProvider) -> str:
        """
        Gera o arquivo de exportação
        ...
        filename = f"export_{batch.reference_period}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
        ...
        if provider.export_format == "json":
            ...
            raise ValueError(f"Formato de exportação não suportado: {provider.export_format}")
    
    def _generate_json_export(self, batch: AccountingExportBatch, items: List[AccountingExportItem], 
                             export_dir: str, filename: str) -> str:
        """
    ...
    def _generate_xml_export(self, batch: AccountingExportBatch, items: List[AccountingExportItem], 
                            export_dir: str, filename: str) -> str:
        """
        ...
        ET.SubElement(batch_elem, "TotalValue").text = str(sum(item.document_value for item in items))
    ...
    def _generate_csv_export(self, batch: AccountingExportBatch, items: List[AccountingExportItem], 
                            export_dir: str, filename: str) -> str:
        """
        ...
        with open(file_path, "w", newline="", encoding="utf-8") as f:
            ...
            # Escreve o cabeçalho
            writer.writerow([
                "Tipo", "Número", "Data", "CNPJ Emissor", "Nome Emissor", 
                "Valor Total", "Impostos", "Chave de Acesso", "Protocolo"
            ])
            ...
            for item in items:
                ...
                # Escreve a linha
                writer.writerow([
                    document_type, document_number, issue_date, issuer_cnpj, issuer_name,
                    total_value, total_taxes, access_key, protocol
                ])
            
            # Escreve um resumo no final
            writer.writerow([])
            writer.writerow(["Resumo", "", "", "", "", "", "", "", ""])
            writer.writerow(["Período", batch.reference_period, "", "", "", "", "", "", ""])
            writer.writerow(["Total de Documentos", len(items), "", "", "", "", "", "", ""])
            writer.writerow(["Valor Total", sum(item.document_value for item in items), "", "", "", "", "", "", ""])
            writer.writerow(["Data de Exportação", datetime.now().isoformat(), "", "", "", "", "", "", ""])
    ...
    def _validate_schedule_data(self, schedule_data: Dict[str, Any]) -> None:
        ...
        if schedule_data["frequency"] not in ["daily", "weekly", "monthly"]:
            raise ValueError(f"Frequência inválida: {schedule_data['frequency']}. Valores válidos: daily, weekly, monthly")
        ...
        if "day_of_week" in schedule_data and schedule_data["day_of_week"] not in range(7):
            raise ValueError("Dia da semana deve ser um número entre 0 (segunda-feira) e 6 (domingo)")
        ...
        if "day_of_month" in schedule_data and schedule_data["day_of_month"] not in range(1, 32):
    ...
    def _calculate_next_run(self, schedule_data: Dict[str, Any]) -> datetime:
        ...
        if schedule_data["frequency"] == "daily":
            ...
            try:
                next_date = datetime(base_date.year, base_date.month, day_of_month, hour, minute)
                
                # Se a data já passou, avança para o próximo mês
                if next_date <= now:
                    if base_date.month == 12:
                        next_date = datetime(base_date.year + 1, 1, day_of_month, hour, minute)
                    else:
                        next_date = datetime(base_date.year, base_date.month + 1, day_of_month, hour, minute)
                ...
                if base_date.month == 12:
                    ...
                    return datetime(base_date.year, base_date.month + 1, 1, hour, minute)
    ...
    def get_schedule(self, schedule_id: str) -> Optional[AccountingSchedule]:
        ...
        """
        schedule_data = self.db_service.find_one("fiscal_accounting_schedules", {"id": schedule_id})
    ...
    def update_schedule(self, schedule_id: str, update_data: Dict[str, Any]) -> Optional[AccountingSchedule]:
        """
        ...
        if "frequency" in update_data or "day_of_week" in update_data or "day_of_month" in update_data or "time_of_day" in update_data:
            schedule_data["next_run"] = self._calculate_next_run(schedule_data)
            update_data["next_run"] = schedule_data["next_run"]
        
        # Atualiza no banco de dados
        self.db_service.update_one("fiscal_accounting_schedules", {"id": schedule_id}, {"$set": update_data})
    ...
    def execute_scheduled_exports(self) -> List[Tuple[str, bool, str]]:
        ...
        schedules = self.db_service.find("fiscal_accounting_schedules", {
            "is_active": True,
            "next_run": {"$lte": now}
        })
        ...
        for schedule in schedules:
            ...
            try:
                ...
                logger.error(f"Erro ao executar exportação agendada {schedule_id}: {str(e)}")
    ...
    def _get_default_provider_id(self) -> str:
        ...
        provider = self.db_service.find_one("fiscal_accounting_providers", {"is_active": True})
    ...
    def list_export_batches(self, filters: Dict[str, Any], page: int = 1, page_size: int = 20) -> Tuple[List[AccountingExportBatch], int]:
        """
    ...
    def list_export_items(self, batch_id: str, page: int = 1, page_size: int = 20) -> Tuple[List[AccountingExportItem], int]:
        """
        ...
        total = self.db_service.count("fiscal_accounting_export_items", {"batch_id": batch_id})
