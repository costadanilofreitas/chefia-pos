...
class NFCeService:
    ...
    def get_state_rule(self, state_code: str) -> NFCeStateRule:
        ...
        rule = self.db_service.find_one("fiscal_nfce_state_rules", {"state_code": state_code})
    ...
    def _validate_nfce_data(self, nfce_data: Dict[str, Any]) -> None:
        ...
        """
        required_fields = ["number", "series", "issue_date", "items", "payments", 
        ...
        if abs(total_payments - nfce_data["total_value"]) > 0.01:  # Tolerância de 1 centavo
            raise ValueError(f"O total dos pagamentos ({total_payments}) não corresponde ao valor total ({nfce_data['total_value']})")
    ...
    def update_nfce(self, nfce_id: str, update_data: Dict[str, Any]) -> Optional[NFCeDocument]:
        """
        Atualiza uma NFC-e
        ...
        if nfce_data["status"] not in [NFCeStatus.DRAFT, NFCeStatus.ERROR]:
            raise ValueError(f"NFC-e com status {nfce_data['status']} não pode ser atualizada")
        ...
        self.db_service.update_one("fiscal_nfce_documents", {"id": nfce_id}, {"$set": update_data})
    ...
    def send_nfce(self, nfce_id: str) -> Tuple[bool, str, Optional[NFCeResponse]]:
        ...
        try:
            ...
            self.update_nfce(nfce_id, {"status": NFCeStatus.PENDING, "xml_content": xml_content})
            ...
            new_status = NFCeStatus.AUTHORIZED if response_data["success"] else NFCeStatus.REJECTED
            self.update_nfce(nfce_id, {
                "status": new_status,
    ...
    def _simulate_sefaz_response(self, nfce: NFCeDocument, state_rule: NFCeStateRule) -> Dict[str, Any]:
        """
    ...
    def cancel_nfce(self, nfce_id: str, reason: str) -> Tuple[bool, str, Optional[NFCeEvent]]:
        """
        Cancela uma NFC-e
        ...
        try:
            ...
            response_data = self._simulate_sefaz_event_response(nfce, "cancellation", state_rule)
            ...
            self.db_service.update_one("fiscal_nfce_events", {"id": event_id}, {"$set": event_data})
            
            # Se o cancelamento foi bem-sucedido, atualiza o status da NFC-e
            if response_data["success"]:
                self.update_nfce(nfce_id, {"status": NFCeStatus.CANCELLED})
            
            return response_data["success"], response_data["status_message"], NFCeEvent(**event_data)
            
        except Exception as e:
            logger.error(f"Erro ao cancelar NFC-e {nfce_id}: {str(e)}")
            return False, f"Erro ao cancelar NFC-e: {str(e)}", None
    
    def _simulate_sefaz_event_response(self, nfce: NFCeDocument, event_type: str, state_rule: NFCeStateRule) -> Dict[str, Any]:
        """
    ...
    def list_nfce(self, filters: Dict[str, Any], page: int = 1, page_size: int = 20) -> Tuple[List[NFCeDocument], int]:
        """
    ...
    def get_nfce_for_accounting(self, start_date: datetime, end_date: datetime) -> List[NFCeDocument]:
        """
