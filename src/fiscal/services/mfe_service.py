...
class MFEService:
    ...
    def get_state_rule(self, state_code: str) -> MFEStateRule:
        ...
        rule = self.db_service.find_one(
            "fiscal_mfe_state_rules", {"state_code": state_code}
        )
    ...
    def _get_default_webservice_url(self, state_code: str) -> str:
        ...
        return webservice_urls.get(
            state_code, f"https://mfe.sefaz.{state_code.lower()}.gov.br/mfe/services"
        )
    ...
    def _validate_equipment_data(self, equipment_data: Dict[str, Any]) -> None:
        ...
        existing = self.db_service.find_one(
            "fiscal_mfe_equipment", {"serial_number": equipment_data["serial_number"]}
        )

        if existing:
            raise ValueError(
                f"Já existe um equipamento MFE com o número de série {equipment_data['serial_number']}"
            )
        ...
        if not state_rule.active:
            raise ValueError(
                f"O estado {state_rule.state_name} não está configurado para MFE"
            )
    ...
    def get_equipment(self, equipment_id: str) -> Optional[MFEEquipment]:
        ...
        """
        equipment_data = self.db_service.find_one(
            "fiscal_mfe_equipment", {"id": equipment_id}
        )
    ...
    def update_equipment(
        self, equipment_id: str, update_data: Dict[str, Any]
    ) -> Optional[MFEEquipment]:
        """
        ...
        self.db_service.update_one(
            "fiscal_mfe_equipment", {"id": equipment_id}, {"$set": update_data}
        )
    ...
    def activate_equipment(
        self, equipment_id: str, activation_code: str
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            success, message = self._simulate_activation(
                equipment, activation_code, state_rule
            )
    ...
    def _simulate_activation(
        self, equipment: MFEEquipment, activation_code: str, state_rule: MFEStateRule
    ) -> Tuple[bool, str]:
        """
        ...
        if len(activation_code) < 8:
            return (
                False,
                "Código de ativação inválido. Deve ter pelo menos 8 caracteres.",
            )
    ...
    def deactivate_equipment(self, equipment_id: str, reason: str) -> Tuple[bool, str]:
        ...
        if equipment.status != MFEStatus.ACTIVE:
            return (
                False,
                f"Equipamento MFE com status {equipment.status} não pode ser desativado",
            )
        ...
        try:
            ...
            # Em um ambiente real, isso seria uma chamada ao webservice da SEFAZ
            success, message = self._simulate_deactivation(
                equipment, reason, state_rule
            )
            ...
            if success:
                ...
                self._register_operation(
                    equipment_id=equipment_id,
                    operation_type=MFEOperationType.CONFIGURATION,
                    request_data={"action": "deactivation", "reason": reason},
                    response_data={"success": True, "message": message},
                )
    ...
    def _simulate_deactivation(
        self, equipment: MFEEquipment, reason: str, state_rule: MFEStateRule
    ) -> Tuple[bool, str]:
        """
        ...
        if len(reason) < 10:
            return (
                False,
                "Motivo da desativação muito curto. Forneça uma descrição mais detalhada.",
            )
    ...
    def check_equipment_status(
        ...
        try:
            ...
            success, message, status_data = self._simulate_status_check(
                equipment, state_rule
            )

            # Atualiza o último contato do equipamento
            self.update_equipment(equipment_id, {"last_communication": datetime.now()})

            # Registra a operação
            self._register_operation(
                equipment_id=equipment_id,
                operation_type=MFEOperationType.CONSULTATION,
                request_data={"action": "status_check"},
                response_data={
                    "success": success,
                    "message": message,
            ...
            logger.error(
                f"Erro ao verificar status do equipamento MFE {equipment_id}: {str(e)}"
            )
    ...
    def _simulate_status_check(
        self, equipment: MFEEquipment, state_rule: MFEStateRule
    ...
    def set_configuration(
        ...
        try:
            ...
            existing = self.db_service.find_one(
                "fiscal_mfe_configurations",
                {"equipment_id": equipment_id, "parameter_name": parameter_name},
            )
            ...
            logger.error(
                f"Erro ao definir configuração para equipamento MFE {equipment_id}: {str(e)}"
            )
    ...
    def get_configuration(
        self, equipment_id: str, parameter_name: str
    ) -> Optional[MFEConfiguration]:
        """
    ...
    def get_all_configurations(self, equipment_id: str) -> List[MFEConfiguration]:
        ...
        configs = self.db_service.find(
            "fiscal_mfe_configurations", {"equipment_id": equipment_id}
        )
    ...
    def schedule_maintenance(
        ...
        try:
            ...
            self.db_service.insert_one(
                "fiscal_mfe_maintenance_schedules", schedule_data
            )
            ...
            logger.error(
                f"Erro ao agendar manutenção para equipamento MFE {equipment_id}: {str(e)}"
            )
    ...
    def complete_maintenance(
        self, schedule_id: str, completion_notes: str
    ) -> Tuple[bool, str]:
        """
        ...
        schedule_data = self.db_service.find_one(
            "fiscal_mfe_maintenance_schedules", {"id": schedule_id}
        )
        ...
        if schedule_data["status"] != "scheduled":
            return (
                False,
                f"Agendamento com status {schedule_data['status']} não pode ser concluído",
            )

        try:
            # Atualiza o agendamento
            self.db_service.update_one(
                "fiscal_mfe_maintenance_schedules",
                {"id": schedule_id},
                {
                    "$set": {
                        "status": "completed",
            ...
            self.update_equipment(
                schedule_data["equipment_id"], {"last_maintenance": datetime.now()}
            )
    ...
    def list_operations(
        self,
        equipment_id: str,
        operation_type: Optional[MFEOperationType] = None,
        start_date: Optional[datetime] = None,
        end_date: Optional[datetime] = None,
        page: int = 1,
        page_size: int = 20,
    ) -> Tuple[List[MFEOperation], int]:
        """
