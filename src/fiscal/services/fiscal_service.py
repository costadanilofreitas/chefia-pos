...
class FiscalService:
    ...
    def _load_configurations(self):
        ...
        for filename in os.listdir(regional_dir):
            if filename.endswith(".json"):
                try:
                    with open(os.path.join(regional_dir, filename), 'r', encoding='utf-8') as f:
                        config_data = json.load(f)
                        config = ConfiguracaoRegional(**config_data)
                        self.regional_configs[config.id] = config
                        logger.info(f"Configuração regional carregada: {config.uf} - {config.municipio or 'Todos os municípios'}")
                except Exception as e:
                    logger.error(f"Erro ao carregar configuração regional {filename}: {str(e)}")
            ...
            if filename.endswith(".json"):
                try:
                    with open(os.path.join(groups_dir, filename), 'r', encoding='utf-8') as f:
                        ...
                        logger.info(f"Grupo fiscal carregado: {group.id} - {group.descricao}")
                except Exception as e:
    ...
    def get_regional_config(self, uf: str, municipio: Optional[str] = None) -> Optional[ConfiguracaoRegional]:
        ...
        if municipio:
            for config in self.regional_configs.values():
                if config.uf.upper() == uf.upper() and config.municipio and config.municipio.upper() == municipio.upper() and config.ativo:
                    return config
        
        # Se não encontrar, busca uma configuração para a UF sem município específico
        for config in self.regional_configs.values():
            if config.uf.upper() == uf.upper() and not config.municipio and config.ativo:
                return config
    ...
    def get_ncm_rule(self, ncm: str, regional_config: ConfiguracaoRegional) -> Optional[RegraNcm]:
    ...
    def get_applicable_benefits(self, 
                               ...
                               data: datetime = None) -> List[BeneficioFiscal]:
    ...
    async def calculate_item_taxes(self,
                                  item: Dict[str, Any],
                                  product_fiscal_info: ProductFiscalInfo,
                                  regional_config: ConfiguracaoRegional) -> ItemCalculoFiscal:
        ...
        if not fiscal_group:
            logger.warning(f"Grupo fiscal não encontrado: {product_fiscal_info.fiscal_group_id}")
        ...
        if fiscal_group.tipo_item in [TipoItem.SERVICO, TipoItem.PRODUTO_SERVICO] and fiscal_group.iss:
    ...
    async def _calculate_icms(self,
                             valor: float,
                             fiscal_group: GrupoFiscal,
                             ncm_rule: Optional[RegraNcm],
                             regional_config: ConfiguracaoRegional,
                             ncm: Optional[str] = None) -> ImpostoCalculado:
        ...
        aliquota = fiscal_group.icms.aliquota if fiscal_group.icms else regional_config.aliquota_icms_padrao
        base_calculo_percentual = fiscal_group.icms.base_calculo if fiscal_group.icms else 100.0
        reducao_base = fiscal_group.icms.percentual_reducao_base_calculo if fiscal_group.icms else 0.0
        ...
        if ncm:
            benefits = self.get_applicable_benefits(ncm, TipoImposto.ICMS, regional_config)
        ...
        return ImpostoCalculado(
            ...
            valor=valor_imposto,
            observacoes="Redução de base de cálculo aplicada" if reducao_base > 0 else None
        )
    
    async def _calculate_pis(self,
                            valor: float,
                            fiscal_group: GrupoFiscal,
                            ncm_rule: Optional[RegraNcm],
                            regional_config: ConfiguracaoRegional,
        ...
        base_calculo_percentual = fiscal_group.pis.base_calculo if fiscal_group.pis else 100.0
        ...
        reducao_base = 0.0
        if ncm:
            benefits = self.get_applicable_benefits(ncm, TipoImposto.PIS, regional_config)
            for benefit in benefits:
                reducao_base = max(reducao_base, benefit.percentual_reducao)
    ...
    async def _calculate_cofins(self,
                               valor: float,
                               fiscal_group: GrupoFiscal,
                               ncm_rule: Optional[RegraNcm],
                               regional_config: ConfiguracaoRegional,
        ...
        base_calculo_percentual = fiscal_group.cofins.base_calculo if fiscal_group.cofins else 100.0
        ...
        reducao_base = 0.0
        if ncm:
            benefits = self.get_applicable_benefits(ncm, TipoImposto.COFINS, regional_config)
            for benefit in benefits:
                reducao_base = max(reducao_base, benefit.percentual_reducao)
    ...
    async def _calculate_iss(self,
                            valor: float,
                            fiscal_group: GrupoFiscal,
                            regional_config: ConfiguracaoRegional) -> ImpostoCalculado:
        ...
        """
        # Define valores padrão
        aliquota = fiscal_group.iss.aliquota if fiscal_group.iss else regional_config.aliquota_iss_padrao
        base_calculo_percentual = fiscal_group.iss.base_calculo if fiscal_group.iss else 100.0
    ...
    async def _calculate_ipi(self,
        ...
        base_calculo_percentual = fiscal_group.ipi.base_calculo if fiscal_group.ipi else 100.0
        
        # Aplica benefícios fiscais se disponíveis
        reducao_base = 0.0
        if ncm:
            benefits = self.get_applicable_benefits(ncm, TipoImposto.IPI, regional_config)
            for benefit in benefits:
                reducao_base = max(reducao_base, benefit.percentual_reducao)
    ...
    async def calculate_order_taxes(self,
                                   order: Dict[str, Any],
                                   regional_config: ConfiguracaoRegional,
                                   product_service: Any) -> ResultadoCalculoFiscal:
        ...
        result = ResultadoCalculoFiscal(
            ...
            descontos=order.get("discount", 0.0) + order.get("coupon_discount", 0.0) + order.get("points_discount", 0.0),
        ...
        for item in order.get("items", []):
            ...
            product_fiscal_info = await self._get_product_fiscal_info(product_id, product_service)
    ...
    async def _get_product_fiscal_info(self, product_id: str, product_service: Any) -> ProductFiscalInfo:
        ...
        try:
            ...
            logger.error(f"Erro ao obter informações fiscais do produto {product_id}: {str(e)}")
            # Retorna informações padrão em caso de erro
            return ProductFiscalInfo(
                fiscal_group_id="DEFAULT",
                origem=OrigemProduto.NACIONAL
            )
    
    def _log_tax_calculation(self, order_id: str, region_id: str, result: ResultadoCalculoFiscal):
    ...
    def save_regional_config(self, config: ConfiguracaoRegional):
        ...
        logger.info(f"Configuração regional salva: {config.uf} - {config.municipio or 'Todos os municípios'}")
