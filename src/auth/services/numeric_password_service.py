...
class NumericPasswordService:
    ...
    async def create_credential(
        ...
        if existing:
            raise ValueError(
                f"Operador {credential_data.operator_id} já possui credencial"
            )
    ...
    async def update_password(
        self, operator_id: str, update_data: OperatorCredentialUpdate
    ) -> OperatorCredential:
        """
        ...
        if not self._verify_password(
            update_data.current_password, credential.password_hash, credential.salt
        ):
            raise ValueError("Senha atual incorreta")

        # Verificar se a nova senha é igual à atual
        if self._verify_password(
            update_data.new_password, credential.password_hash, credential.salt
        ):
    ...
    async def reset_password(
        self, reset_data: OperatorCredentialReset
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "temporary_password": (
                new_password if reset_data.generate_temporary else None
            ),
        }
    ...
    async def authenticate(self, login_data: LoginRequest) -> LoginResponse:
        ...
        if credential.locked_until:
            if credential.locked_until > datetime.utcnow():
                minutes_remaining = (
                    credential.locked_until - datetime.utcnow()
                ).total_seconds() / 60
                raise ValueError(
                    f"Conta bloqueada. Tente novamente em {int(minutes_remaining)} minutos"
                )
            else:
        ...
        # Verificar senha
        if not self.repository.verify_password(
            login_data.operator_id, login_data.password
        ):
            ...
            if credential.failed_attempts >= self.config.max_failed_attempts:
                credential.locked_until = datetime.utcnow() + timedelta(
                    minutes=self.config.lock_duration_minutes
                )
                self.repository.update_credentials(credential)
                raise ValueError(
                    f"Conta bloqueada por {self.config.lock_duration_minutes} minutos devido a múltiplas tentativas falhas"
                )
        ...
        token_expiry = datetime.utcnow() + timedelta(
            minutes=self.config.session_expiry_minutes
        )
        token_payload = {
