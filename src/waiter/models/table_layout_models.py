...
class TableModel(BaseModel):
    ...
    status: TableStatus = Field(TableStatus.AVAILABLE, description="Status atual da mesa")
    capacity: int = Field(4, description="Capacidade de pessoas na mesa")
    current_order_id: Optional[str] = Field(None, description="ID do pedido atual, se houver")
    waiter_id: Optional[str] = Field(None, description="ID do garçom responsável, se atribuído")
    custom_shape_path: Optional[str] = Field(None, description="Caminho SVG para formas personalizadas")
...
class LayoutSection(BaseModel):
    ...
    background_image: Optional[str] = Field(None, description="URL ou caminho da imagem de fundo")
    background_color: str = Field("#FFFFFF", description="Cor de fundo da seção")
    position: Dict[str, float] = Field({}, description="Posição e dimensões da seção no layout")
...
class TableLayoutConfig(BaseModel):
    ...
    auto_assign_waiter: bool = Field(False, description="Atribuir garçom automaticamente")
