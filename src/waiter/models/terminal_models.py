...
class TerminalCapabilities(BaseModel):
    ...
    barcode_scanner: bool = Field(False, description="Se possui leitor de código de barras")
    wifi: bool = Field(True, description="Se possui conectividade Wi-Fi")
    mobile_data: bool = Field(False, description="Se possui conectividade de dados móveis")
...
class TerminalConfig(BaseModel):
    ...
    api_secret: Optional[str] = Field(None, description="Segredo de API para integração")
    merchant_id: Optional[str] = Field(None, description="ID do comerciante na operadora")
    ...
    offline_mode_enabled: bool = Field(True, description="Se o modo offline está habilitado")
...
class TerminalSession(BaseModel):
    ...
    status: TerminalStatus = Field(TerminalStatus.ONLINE, description="Status atual do terminal")
    started_at: datetime = Field(default_factory=datetime.now, description="Data de início da sessão")
    last_activity: datetime = Field(default_factory=datetime.now, description="Data da última atividade")
    last_sync: datetime = Field(default_factory=datetime.now, description="Data da última sincronização")
    pending_orders: List[str] = Field([], description="IDs de pedidos pendentes de sincronização")
    battery_level: Optional[int] = Field(None, description="Nível de bateria em percentual")
    signal_strength: Optional[int] = Field(None, description="Força do sinal em percentual")
...
class OfflineOrder(BaseModel):
    ...
    server_order_id: Optional[str] = Field(None, description="ID do pedido no servidor após sincronização")
