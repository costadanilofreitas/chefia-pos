...
async def update_terminal_config(
    ...
    if terminal_id != config.id:
        raise HTTPException(status_code=400, detail="ID do terminal na URL não corresponde ao ID no corpo")
...
async def get_offline_orders(
    terminal_id: str = Query(..., description="ID do terminal"),
    synced: Optional[bool] = Query(None, description="Filtrar por status de sincronização"),
    terminal_service: TerminalService = Depends()
):
