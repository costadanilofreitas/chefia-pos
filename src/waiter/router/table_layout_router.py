...
async def delete_layout(
    ...
    if not success:
        raise HTTPException(status_code=400, detail="Não foi possível excluir o layout. Verifique se não é o layout ativo.")
...
async def set_active_layout(
    ...
    if not success:
        raise HTTPException(status_code=400, detail="Não foi possível definir o layout como ativo")
    
    return {"success": True}

# Rotas para gerenciamento de mesas
@router.put("/layouts/{layout_id}/tables/{table_id}/status", response_model=Dict[str, bool])
...
async def get_table_with_order(
    ...
    if not table:
        raise HTTPException(status_code=404, detail="Mesa não encontrada para este pedido")
