...
async def list_orders(
    ...
    waiter_id_filter: Optional[int] = Query(
        None, alias="waiter_id", description="Filtrar por ID da instância licenciada"
    ),
    ...
    if waiter_id_filter is not None:
        config_file_path = os.path.join(
            CONFIG_DIR, "waiter", f"{waiter_id_filter}.json"
        )
...
async def get_order(
    ...
    if not os.path.exists(config_file_path):
        raise HTTPException(
            status_code=403,
            detail=f"Instance ID {order.waiter_id} associated with this order is not licensed.",
        )
    return order
...
async def update_order(
    ...
    # Check license for the associated waiter_id
    config_file_path = os.path.join(
        CONFIG_DIR, "waiter", f"{existing_order.waiter_id}.json"
    )
    if not os.path.exists(config_file_path):
        raise HTTPException(
            status_code=403,
            detail=f"Instance ID {existing_order.waiter_id} associated with this order is not licensed.",
        )
    order = await waiter_service.update_order(order_id, update_data)
    if not order:
        raise HTTPException(
            status_code=404, detail="Pedido não encontrado após atualização"
        )
    return order
...
async def cancel_order(
    ...
    """
    _check_permissions(
        current_user, [Permission.ORDER_UPDATE]
    )  # Changed from ORDER_DELETE
    waiter_service = get_waiter_service()
    existing_order = await waiter_service.get_order(order_id)
    if not existing_order:
        raise HTTPException(status_code=404, detail="Pedido não encontrado")
    ...
    if not order:
        raise HTTPException(
            status_code=404, detail="Pedido não encontrado após cancelamento"
        )
    return order
...
async def get_session(
    ...
    if not os.path.exists(config_file_path):
        raise HTTPException(
            status_code=403,
            detail=f"Instance ID {session.waiter_id} associated with this session is not licensed.",
        )
    return session
...
async def update_session(
    ...
    # Check license for the associated waiter_id
    config_file_path = os.path.join(
        CONFIG_DIR, "waiter", f"{existing_session.waiter_id}.json"
    )
    if not os.path.exists(config_file_path):
        raise HTTPException(
            status_code=403,
            detail=f"Instance ID {existing_session.waiter_id} associated with this session is not licensed.",
        )
    session = await waiter_service.update_session(session_id, update_data)
    if not session:
        raise HTTPException(
            status_code=404, detail="Sessão não encontrada após atualização"
        )
    return session
...
async def sync_device_data(
    ...
    if not os.path.exists(config_file_path):
        raise HTTPException(
            status_code=403,
            detail=f"Instance ID {session.waiter_id} associated with this device is not licensed.",
        )
    # --- END LICENSE CHECK ---
