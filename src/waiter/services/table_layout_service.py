...
class TableLayoutService:
    ...
    async def create_layout(self, layout_data: Dict[str, Any]) -> TableLayout:
        ...
        if not config:
            ...
            if layout.id not in config.available_layouts:
                ...
                await self.update_layout_config(config.restaurant_id, config.store_id, config.dict())
    ...
    async def get_layout(self, layout_id: str) -> Optional[TableLayout]:
        ...
        """
        layout_data = await self.db_service.find_one(self.collection_name, {"id": layout_id})
    ...
    async def get_layouts_by_restaurant(self, restaurant_id: str, store_id: str) -> List[TableLayout]:
        """
    ...
    async def get_active_layout(self, restaurant_id: str, store_id: str) -> Optional[TableLayout]:
        """
    ...
    async def update_layout(self, layout_id: str, layout_data: Dict[str, Any]) -> Optional[TableLayout]:
        """
    ...
    async def delete_layout(self, layout_id: str) -> bool:
        ...
        result = await self.db_service.delete_one(self.collection_name, {"id": layout_id})
    ...
    async def set_active_layout(self, restaurant_id: str, store_id: str, layout_id: str) -> bool:
        """
    ...
    async def update_table_status(self, layout_id: str, table_id: str, status: TableStatus, 
                                  order_id: Optional[str] = None, waiter_id: Optional[str] = None) -> bool:
        """
        ...
        # Encontrar a mesa
        table_index = next((i for i, t in enumerate(layout.tables) if t.id == table_id), -1)
        if table_index == -1:
            return False
        ...
        await self.db_service.update_one(
            self.collection_name,
            {"id": layout_id},
            {"$set": {"tables": [t.dict() for t in layout.tables], "updated_at": datetime.now()}}
        )
        
        return True
    
    async def get_tables_by_status(self, restaurant_id: str, store_id: str, 
                                  status: Optional[TableStatus] = None) -> List[Dict[str, Any]]:
        """
    ...
    async def create_layout_config(self, config_data: Dict[str, Any]) -> TableLayoutConfig:
        """
    ...
    async def get_layout_config(self, restaurant_id: str, store_id: str) -> Optional[TableLayoutConfig]:
        """
    ...
    async def update_layout_config(self, restaurant_id: str, store_id: str, 
                                  config_data: Dict[str, Any]) -> Optional[TableLayoutConfig]:
        """
