...
class WaiterService:
    ...
    async def create_order(self, order_data: WaiterOrderCreate) -> WaiterOrder:
        ...
        await self._update_table_status(
            order.table_number, "occupied", order.id, order.customer_count
        )
    ...
    async def update_order(
        self, order_id: str, update_data: WaiterOrderUpdate
        ...
        if orders[order_index]["status"] not in [
            ...
            raise ValueError(
                "Não é possível modificar um pedido que já foi entregue ou cancelado"
            )
        ...
        if update_data.status == WaiterOrderStatus.SENT and not orders[order_index].get(
            "sent_at"
        ):
            orders[order_index]["sent_at"] = datetime.now().isoformat()

        # Se o status for alterado para DELIVERED, definir completed_at
        if update_data.status == WaiterOrderStatus.DELIVERED and not orders[
            order_index
        ].get("completed_at"):
            ...
            await self._update_table_status(
                orders[order_index]["table_number"], "cleaning", None, None
            )
    ...
    async def send_order_to_kitchen(self, order_id: str) -> Optional[WaiterOrder]:
        ...
        if order.status != WaiterOrderStatus.DRAFT:
            raise ValueError(
                "Apenas pedidos em rascunho podem ser enviados para a cozinha"
            )
    ...
    async def cancel_order(
        ...
        if order.status in [WaiterOrderStatus.DELIVERED, WaiterOrderStatus.CANCELLED]:
            raise ValueError(
                "Não é possível cancelar um pedido que já foi entregue ou cancelado"
            )
        ...
        if updated_order:
            ...
            if order.table_number:
                await self._update_table_status(
                    order.table_number, "available", None, None
                )

        return updated_order

    async def add_order_item(
        self, order_id: str, item_data: WaiterOrderItemCreate
        ...
        if order.status not in [WaiterOrderStatus.DRAFT, WaiterOrderStatus.SENT]:
            raise ValueError(
                "Não é possível adicionar itens a um pedido que já foi entregue ou cancelado"
            )
        ...
        if product.type == "composite" and item_data.sections:
            section_product_ids = {
                section.section_id: section.product_id for section in item_data.sections
            }
            unit_price = await product_service.calculate_composite_product_price(
                product.id, section_product_ids
            )
        ...
        if order_index is not None:
            ...
            orders[order_index]["total"] = (
                orders[order_index]["subtotal"]
                - orders[order_index]["discount"]
                + orders[order_index]["tax"]
            )
            orders[order_index]["updated_at"] = datetime.now().isoformat()
            ...
            await self.event_publisher.publish_order_updated(
                order_id,
                {
                    "items": orders[order_index]["items"],
                    "subtotal": orders[order_index]["subtotal"],
                    "total": orders[order_index]["total"],
                },
            )
    ...
    async def update_order_item(
        self, order_id: str, item_id: str, update_data: WaiterOrderItemUpdate
    ) -> Optional[WaiterOrderItem]:
        ...
        if order.status not in [WaiterOrderStatus.DRAFT, WaiterOrderStatus.SENT]:
            raise ValueError(
                "Não é possível modificar itens de um pedido que já foi entregue ou cancelado"
            )

        # Encontrar o item
        item_index = next(
            (i for i, item in enumerate(order.items) if item.id == item_id), None
        )
    ...
    async def remove_order_item(self, order_id: str, item_id: str) -> bool:
        ...
        if order.status not in [WaiterOrderStatus.DRAFT, WaiterOrderStatus.SENT]:
            raise ValueError(
                "Não é possível remover itens de um pedido que já foi entregue ou cancelado"
            )
    ...
    async def get_session_by_device(self, device_id: str) -> Optional[WaiterSession]:
        ...
        sessions = await self._load_sessions()
        session_dict = next(
            (s for s in sessions if s["device_id"] == device_id and s["active"]), None
        )
    ...
    async def update_session(
        self, session_id: str, update_data: WaiterSessionUpdate
    ...
    async def create_table(self, table_data: WaiterTableCreate) -> WaiterTable:
        """Cria uma nova mesa."""
        table = WaiterTable(
            number=table_data.number, name=table_data.name, capacity=table_data.capacity
        )
    ...
    async def update_table(
        self, table_id: str, update_data: WaiterTableUpdate
    ...
    async def _update_table_status(
        ...
        update_data = WaiterTableUpdate(
            status=status, current_order_id=order_id, customer_count=customer_count
        )
    ...
    async def _update_stats(self) -> None:
        ...
        preparing_orders = [
            o for o in orders if o["status"] == WaiterOrderStatus.PREPARING
        ]
        ready_orders = [o for o in orders if o["status"] == WaiterOrderStatus.READY]
        delivered_orders = [
            o for o in orders if o["status"] == WaiterOrderStatus.DELIVERED
        ]
        cancelled_orders = [
            o for o in orders if o["status"] == WaiterOrderStatus.CANCELLED
        ]
        ...
        pending_sync_orders = [
            o for o in orders if o.get("sync_status") == "pending_sync"
        ]
    ...
    async def update_order_status_from_event(
        self, order_id: str, status: WaiterOrderStatus
    ) -> Optional[WaiterOrder]:
    ...
    async def update_order_from_event(
    ...
    async def _generate_order_number(self) -> str:
        ...
        orders = await self._load_orders()
        today_orders = [
            o for o in orders if o.get("order_number", "").startswith(today)
        ]
