...
class TerminalService:
    ...
    async def get_terminal_configs(self, restaurant_id: str, store_id: str) -> List[TerminalConfig]:
    ...
    async def update_terminal_config(self, config: TerminalConfig) -> Optional[TerminalConfig]:
    ...
    async def create_terminal_session(self, terminal_id: str, waiter_id: str) -> Optional[TerminalSession]:
    ...
    async def get_offline_orders(self, terminal_id: str, synced: Optional[bool] = None) -> List[OfflineOrder]:
        """Obtém pedidos offline de um terminal."""
        orders = [
            order for order in self._offline_orders.values()
    ...
    async def sync_terminal_data(self, terminal_id: str, session_id: str) -> Dict[str, Any]:
        ...
        session.pending_orders = [
            order_id for order_id in session.pending_orders
            if order_id in self._offline_orders and not self._offline_orders[order_id].synced
        ]
