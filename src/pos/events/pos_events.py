...
class POSEventPublisher:
    ...
    async def publish_payment_failed(
        self, payment_data: Dict[str, Any], error_message: str
    ):
    ...
    async def subscribe_to_events(self):
        ...
        self.subscriptions.append(
            await self.event_bus.subscribe(
                "business_day.opened", self.handle_business_day_opened
            )
        )
        self.subscriptions.append(
            await self.event_bus.subscribe(
                "business_day.closed", self.handle_business_day_closed
            )
        )
        ...
        self.subscriptions.append(
            await self.event_bus.subscribe(
                "cashier.operation", self.handle_cashier_operation
            )
        )
        ...
        self.subscriptions.append(
            await self.event_bus.subscribe(
                "order.payment_status_changed", self.handle_order_payment_status_changed
            )
        )
        ...
        self.subscriptions.append(
            await self.event_bus.subscribe(
                "external_order.received", self.handle_external_order_received
            )
        )
        self.subscriptions.append(
            await self.event_bus.subscribe(
                "ifood.order_status_changed", self.handle_ifood_order_status_changed
            )
        )
