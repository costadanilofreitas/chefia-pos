              ...
              <Grid container spacing={1} sx={{ mb: 3 }}>
                ...
                <Grid item xs={6}>
                  <Typography variant="body1" align="right">
                    {formatCurrency(
                      currentOrder?.items.reduce((sum, item) => sum + item.total_price, 0) || 0
                    )}
                  </Typography>
                </Grid>
                <Grid item xs={6}>
                      ...
                      <List>
                        {customizations.map((item) => (
                          <ListItem key={item.id} disablePadding sx={{ mb: 1 }}>
                            ...
                            <ListItemSecondaryAction>
                              <Button
                                variant={item.action === 'keep' ? 'outlined' : 'contained'}
                                color={item.action === 'keep' ? 'primary' : item.action === 'remove' ? 'error' : 'success'}
                                size="small"
                                onClick={() => handleCustomizationChange(
                                  item.id, 
                                  item.action === 'keep' ? 'remove' : 
                                  item.action === 'remove' ? 'extra' : 'keep'
                                )}
                              >
                                {item.action === 'keep' ? 'Normal' : 
                                 item.action === 'remove' ? 'Remover' : 'Extra'}
                              </Button>
                            </ListItemSecondaryAction>
                          </ListItem>
                        ))}
                    ...
                    <Box sx={{ bgcolor: 'background.default', p: 2, borderRadius: '8px' }}>
                      <Grid container spacing={1}>
                        <Grid item xs={8}>
                          ...
                          {customizations.filter(c => c.action === 'extra').map(item => (
                            <Typography key={item.id} variant="body2" color="text.secondary">
                              Extra {item.name}: +{formatCurrency(item.price * quantity)}
                            </Typography>
                          ))}
                        </Grid>
                        <Grid item xs={4}>
