...
from src.core.dependencies import (
    ...
    CONFIG_DIR,
)  # Import the dependency and config dir
...
async def get_pos_session(
    ...
    if not os.path.exists(config_file_path):
        raise HTTPException(
            status_code=403,
            detail=f"Instance ID {session.pos_id} associated with this session is no longer licensed.",
        )
...
async def list_pos_sessions(
    pos_id: Optional[int] = Query(
        None, description="Filtrar por ID da instância POS licenciada"
    ),
    ...
    business_day_id: Optional[str] = Query(
        None, description="Filtrar por dia de operação"
    ),
...
async def update_pos_session(
    ...
    try:
        ...
        config_file_path = os.path.join(
            CONFIG_DIR, "pos", f"{existing_session.pos_id}.json"
        )
        if not os.path.exists(config_file_path):
            raise HTTPException(
                status_code=403,
                detail=f"Instance ID {existing_session.pos_id} associated with this session is no longer licensed.",
            )

        session = await pos_service.update_session(session_id, session_update)
        if not session:
            raise HTTPException(
                status_code=404, detail="Sessão POS não encontrada após atualização"
            )
        return session
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def close_pos_session(
    ...
    try:
        ...
        if not session:
            raise HTTPException(
                status_code=404, detail="Sessão POS não encontrada após fechamento"
            )
        return session
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def suspend_pos_session(
    ...
    try:
        ...
        if not session:
            raise HTTPException(
                status_code=404, detail="Sessão POS não encontrada após suspensão"
            )
        return session
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_pos_config(
    ...
    if not config:
        raise HTTPException(
            status_code=404, detail=f"Configuração para POS ID {pos_id} não encontrada"
        )
    return config
...
async def refund_payment(
    ...
    _check_permissions(
        current_user, [Permission.SALE_UPDATE]
    )  # Corrected permission (assuming update covers refund)
    raise HTTPException(status_code=501, detail="Not Implemented")


@router.post("/pos/cash-operations", response_model=CashOperation)
async def create_cash_operation(
    operation_data: CashOperationCreate, current_user: User = Depends(get_current_user)
):
    _check_permissions(
        current_user, [Permission.CASHIER_WITHDRAW]
...
async def get_cash_operation(
    operation_id: str, current_user: User = Depends(get_current_user)
):
    _check_permissions(current_user, [Permission.SALE_READ])  # Corrected permission
    raise HTTPException(status_code=501, detail="Not Implemented")
...
async def create_receipt(
    receipt_data: ReceiptCreate, current_user: User = Depends(get_current_user)
):
    _check_permissions(
        current_user, [Permission.SALE_READ]
    )  # Corrected permission (assuming read allows printing)
    raise HTTPException(status_code=501, detail="Not Implemented")
