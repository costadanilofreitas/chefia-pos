...
class POSService:
    ...
    async def get_instance_config(self, pos_id: int) -> Optional[POSConfig]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to load configuration for POS ID {pos_id}")
    
    # Métodos para Sessões POS
    async def create_session(self, session_data: POSSessionCreate, user_id: str, pos_id: int) -> POSSession:
        """Cria uma nova sessão POS."""
        # Verificar se já existe uma sessão aberta para o mesmo caixa
        for session in self.sessions.values():
            if (session.cashier_id == session_data.cashier_id and 
                session.status == POSSessionStatus.OPEN):
                raise ValueError(f"Já existe uma sessão aberta para o caixa {session_data.cashier_id}")
    ...
    async def update_session(self, session_id: str, session_update: POSSessionUpdate) -> Optional[POSSession]:
    ...
    async def close_session(self, session_id: str, closing_balance: float, notes: Optional[str] = None) -> Optional[POSSession]:
    ...
    async def suspend_session(self, session_id: str, reason: str) -> Optional[POSSession]:
    ...
    async def create_payment(self, payment_data: PaymentTransactionCreate) -> PaymentTransaction:
    ...
    async def update_payment(self, payment_id: str, payment_update: PaymentTransactionUpdate) -> Optional[PaymentTransaction]:
    ...
    async def refund_payment(self, payment_id: str, amount: Optional[float] = None, reason: str = "") -> Optional[PaymentTransaction]:
        ...
        if refund_amount > payment.amount:
            raise ValueError("Valor do reembolso não pode ser maior que o valor do pagamento")
    ...
    async def create_cash_operation(self, operation_data: CashOperationCreate) -> CashOperation:
        ...
        if session.status != POSSessionStatus.OPEN:
            raise ValueError("Operações de caixa só podem ser realizadas em sessões abertas")
