...
async def create_split_config(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Erro interno ao criar configuração de rateio")
...
async def get_split_config(
    ...
    if not config:
        raise HTTPException(status_code=404, detail="Configuração de rateio não encontrada")
...
async def update_split_config(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Erro interno ao atualizar configuração de rateio")
...
async def create_payment_with_split(
    ...
    try:
        split_service = get_split_payment_service()
        result = await split_service.create_payment_with_split(payment_data, split_config_id)
        return result
    except ValueError as e:
        ...
        raise HTTPException(status_code=500, detail="Erro interno ao criar pagamento com rateio")
...
async def get_split_payment(
    ...
    if not record:
        raise HTTPException(status_code=404, detail="Registro de pagamento com rateio não encontrado")
