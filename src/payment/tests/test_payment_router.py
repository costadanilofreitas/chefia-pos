...
for route in router.routes:
    if hasattr(route, "dependencies"):
        ...
        for dependency in route.dependencies:
            if getattr(dependency, "dependency", None) and "get_current_user" in str(
...
class TestPaymentRouter(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.service_patch = patch(
            "src.payment.router.payment_router.get_payment_service",
    ...
    async def test_create_payment(self):
        ...
        response = self.client.post(
            "/api/v1/payments/", json=payment_data, headers=self.auth_headers
        )
    ...
    async def test_get_payment(self):
        ...
        response = self.client.get(
            "/api/v1/payments/payment_id_123", headers=self.auth_headers
        )
    ...
    async def test_get_payment_not_found(self):
        ...
        # Fazer requisição
        response = self.client.get(
            "/api/v1/payments/nonexistent_id", headers=self.auth_headers
        )
    ...
    async def test_get_payments_by_order(self):
        ...
        # Fazer requisição
        response = self.client.get(
            "/api/v1/payments/order/order_123", headers=self.auth_headers
        )
    ...
    async def test_check_payment_status(self):
        ...
        # Fazer requisição
        response = self.client.get(
            "/api/v1/payments/payment_id_123/check", headers=self.auth_headers
        )
