...
from src.payment.models.payment_models import (
    PaymentMethod, PaymentStatus, PaymentTransaction, AsaasConfig
)
from src.payment.models.split_models import (
    SplitConfig, SplitRecipient, SplitType, RetentionConfig,
    SplitTransaction, RetentionTransaction, SplitPaymentRecord
)
...
class AsaasAdapter:
    ...
    def __init__(self, config: AsaasConfig = None):
        ...
        if config:
            ...
            self.base_url = "https://sandbox.asaas.com/api/v3" if config.environment == "sandbox" else "https://www.asaas.com/api/v3"
            self.headers = {
    ...
    async def create_customer(self, customer_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(endpoint, headers=self.headers, json=customer_data) as response:
                response_data = await response.json()
                
                if response.status >= 400:
                    logger.error(f"Erro ao criar cliente no Asaas: {response_data}")
                    raise Exception(f"Erro ao criar cliente no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint, headers=self.headers) as response:
                ...
                if response.status >= 400:
                    ...
                    raise Exception(f"Erro ao obter cliente no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint, headers=self.headers) as response:
                ...
                if response.status >= 400:
                    ...
                    raise Exception(f"Erro ao buscar cliente no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
    ...
    async def create_payment(self, payment_data: Dict[str, Any], split_config: Optional[SplitConfig] = None) -> Dict[str, Any]:
        """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(endpoint, headers=self.headers, json=payment_data) as response:
                response_data = await response.json()
                
                if response.status >= 400:
                    logger.error(f"Erro ao criar pagamento no Asaas: {response_data}")
                    raise Exception(f"Erro ao criar pagamento no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint, headers=self.headers) as response:
                ...
                if response.status >= 400:
                    ...
                    raise Exception(f"Erro ao obter pagamento no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
    ...
    async def cancel_payment(self, payment_id: str) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.delete(endpoint, headers=self.headers) as response:
                ...
                if response.status >= 400:
                    logger.error(f"Erro ao cancelar pagamento no Asaas: {response_data}")
                    raise Exception(f"Erro ao cancelar pagamento no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
                
                return response_data
    
    async def refund_payment(self, payment_id: str, value: Optional[float] = None) -> Dict[str, Any]:
        """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(endpoint, headers=self.headers, json=data) as response:
                ...
                if response.status >= 400:
                    logger.error(f"Erro ao reembolsar pagamento no Asaas: {response_data}")
                    raise Exception(f"Erro ao reembolsar pagamento no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
                
                return response_data
    
    async def create_pix_payment(self, customer_id: str, value: float, description: str, 
                               due_date: str, split_config: Optional[SplitConfig] = None,
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint, headers=self.headers) as response:
                ...
                if response.status >= 400:
                    ...
                    raise Exception(f"Erro ao obter QR Code PIX no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
                
                return response_data
    
    async def create_credit_card_payment(self, customer_id: str, value: float, description: str,
                                       credit_card_data: Dict[str, Any], split_config: Optional[SplitConfig] = None,
                                       external_reference: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def create_debit_card_payment(self, customer_id: str, value: float, description: str,
                                      debit_card_data: Dict[str, Any], split_config: Optional[SplitConfig] = None,
                                      external_reference: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def get_transfers(self, payment_id: str) -> List[Dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint, headers=self.headers) as response:
                ...
                if response.status >= 400:
                    logger.error(f"Erro ao obter transferências no Asaas: {response_data}")
                    raise Exception(f"Erro ao obter transferências no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
                
                return response_data.get('data', [])
    
    async def calculate_split_values(self, total_value: float, split_config: SplitConfig) -> Dict[str, Any]:
        """
        ...
        if split_config.retention_config and split_config.retention_config.type == SplitType.FIXED:
            retention = split_config.retention_config
        ...
        if split_config.retention_config and split_config.retention_config.type == SplitType.PERCENTAGE:
            retention = split_config.retention_config
        ...
        if result["retention"]:
            result["retention"]["calculated_value"] = round(result["retention"]["calculated_value"], 2)
        
        return result
    
    async def create_split_payment_record(self, payment_id: str, provider_payment_id: str, 
                                        split_config: SplitConfig, total_value: float) -> SplitPaymentRecord:
        """
        ...
        for recipient_data in split_values["recipients"]:
            split_transactions.append(SplitTransaction(
    ...
    async def update_split_transactions_status(self, split_payment_record: SplitPaymentRecord) -> SplitPaymentRecord:
        """
        ...
        for split in split_payment_record.splits:
            if split.wallet_id in transfers_by_wallet:
                ...
                split.status = "transferred" if transfer.get("status") == "CONFIRMED" else "pending"
                split.transfer_id = transfer.get("id")
                split.transferred_at = datetime.fromisoformat(transfer.get("transferDate").replace("Z", "+00:00")) if transfer.get("transferDate") else None
                split.updated_at = datetime.utcnow()
        
        # Atualizar status da transação de retenção
        if split_payment_record.retention and split_payment_record.retention.wallet_id in transfers_by_wallet:
