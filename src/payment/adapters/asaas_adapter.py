    ...
    async def create_split_config(self, split_config: SplitConfig) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(endpoint, headers=self.headers, json=data) as response:
                ...
                if response.status >= 400:
                    logger.error(f"Erro ao criar configuração de split no Asaas: {response_data}")
                    raise Exception(f"Erro ao criar configuração de split no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
    ...
    async def get_split_config(self, split_id: str) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint, headers=self.headers) as response:
                ...
                if response.status >= 400:
                    logger.error(f"Erro ao obter configuração de split no Asaas: {response_data}")
                    raise Exception(f"Erro ao obter configuração de split no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
                
                return response_data
    
    async def update_split_config(self, split_id: str, split_config: SplitConfig) -> Dict[str, Any]:
        """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.put(endpoint, headers=self.headers, json=data) as response:
                response_data = await response.json()
                
                if response.status >= 400:
                    logger.error(f"Erro ao atualizar configuração de split no Asaas: {response_data}")
                    raise Exception(f"Erro ao atualizar configuração de split no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
    ...
    async def delete_split_config(self, split_id: str) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.delete(endpoint, headers=self.headers) as response:
                ...
                if response.status >= 400:
                    logger.error(f"Erro ao excluir configuração de split no Asaas: {response_data}")
                    raise Exception(f"Erro ao excluir configuração de split no Asaas: {response_data.get('errors', [{'description': 'Erro desconhecido'}])[0].get('description')}")
    ...
    def _format_split_config(self, split_config: SplitConfig) -> List[Dict[str, Any]]:
        ...
        for recipient in split_config.recipients:
            recipient_data = {
                ...
                "percentualValue": recipient.percentage if recipient.percentage else None,
