...
class SplitPaymentService:
    ...
    async def create_split_config(self, config_data: SplitConfigCreate) -> SplitConfig:
        ...
        if config_data.retention_config:
            ...
            if not await self.asaas_adapter.validate_wallet(retention_config.wallet_id):
                raise ValueError(
                    f"Carteira de retenção inválida: {retention_config.wallet_id}"
                )
        ...
        await event_bus.publish(
            Event(event_type=EventType.SPLIT_CONFIG_CREATED, data=split_config.dict())
        )

        return split_config

    async def update_split_config(
        self, config_id: str, config_data: SplitConfigUpdate
        ...
        configs = self._load_split_configs()
        config_index = next(
            (i for i, c in enumerate(configs) if c.get("id") == config_id), None
        )
        ...
        if config_data.retention_config is not None:
            if config_data.retention_config:
                ...
                # Validar carteira de retenção no Asaas
                if not await self.asaas_adapter.validate_wallet(
                    retention_config.wallet_id
                ):
                    raise ValueError(
                        f"Carteira de retenção inválida: {retention_config.wallet_id}"
                    )
        ...
        await event_bus.publish(
            Event(
                event_type=EventType.SPLIT_CONFIG_UPDATED, data=existing_config.dict()
            )
        )
    ...
    async def get_split_configs_by_restaurant(
        self, restaurant_id: str, store_id: Optional[str] = None
    ) -> List[SplitConfig]:
        """
        ...
        if store_id:
            config_dicts = [
                c
                for c in configs
                if c.get("restaurant_id") == restaurant_id
                and c.get("store_id") == store_id
            ]
        else:
            config_dicts = [
                c for c in configs if c.get("restaurant_id") == restaurant_id
            ]
    ...
    async def delete_split_config(self, config_id: str) -> bool:
        ...
        await event_bus.publish(
            Event(
                event_type=EventType.SPLIT_CONFIG_DELETED, data={"config_id": config_id}
            )
        )
    ...
    async def create_payment_with_split(
        self, payment_data: PaymentCreate, split_config_id: str
    ) -> Dict[str, Any]:
        """
        ...
        if not split_config:
            raise ValueError(
                f"Configuração de rateio não encontrada: {split_config_id}"
            )
        ...
        if split_config.valid_from and split_config.valid_from > now:
            raise ValueError(
                f"Configuração de rateio ainda não válida: {split_config_id}"
            )
        ...
        payment = await payment_service.create_payment_with_split(
            payment_data, split_config
        )
    ...
    async def get_split_payment_record(
        self, payment_id: str
    ) -> Optional[SplitPaymentRecord]:
        """
        ...
        split_payments = self._load_split_payments()
        record_dict = next(
            (p for p in split_payments if p.get("payment_id") == payment_id), None
        )
    ...
    async def update_split_payment_status(
        ...
        updated_record = await self.asaas_adapter.update_split_transactions_status(
            split_payment_record
        )
        ...
        record_index = next(
            (
                i
                for i, p in enumerate(split_payments)
    ...
    async def get_split_payments_by_config(
        self, split_config_id: str
    ) -> List[SplitPaymentRecord]:
        """
        ...
        split_payments = self._load_split_payments()
        record_dicts = [
            p for p in split_payments if p.get("split_config_id") == split_config_id
        ]
...
class PaymentService:
    ...
    def __init__(self):
        self.adapters = {PaymentProvider.ASAAS: AsaasAdapter()}
        self._load_provider_configs()
    ...
    def _save_provider_config(self, config: ProviderConfig) -> None:
        ...
        config_index = next(
            (i for i, c in enumerate(configs) if c.get("provider") == config.provider),
            None,
        )
    ...
    def _save_payment(self, payment: Payment) -> None:
        ...
        payment_index = next(
            (i for i, p in enumerate(payments) if p.get("id") == payment.id), None
        )
    ...
    async def create_provider_config(
        self, config_data: ProviderConfigCreate
    ) -> ProviderConfig:
    ...
    async def update_provider_config(
        self, provider: PaymentProvider, config_data: ProviderConfigUpdate
    ) -> Optional[ProviderConfig]:
    ...
    async def create_payment(self, payment_data: PaymentCreate) -> Payment:
        ...
        payment = Payment(
            ...
            or config.default_notification,
        ...
        event_bus = get_event_bus()
        await event_bus.publish(
            Event(event_type=EventType.PAYMENT_CREATED, data=payment.dict())
        )

        return payment

    async def create_payment_with_split(
        self, payment_data: PaymentCreate, split_config: SplitConfig
    ) -> Payment:
        """
        ...
        adapter = self.adapters[provider]
        provider_payment = await adapter.create_payment(
            payment_data.dict(), split_config
        )
        ...
        payment = Payment(
            ...
            description=payment_data.description,
            external_reference=payment_data.external_reference,
            metadata={**payment_data.metadata, "split_config_id": split_config.id},
        )
    ...
    async def get_payment_by_provider_id(
        ...
        payments = self._load_payments()
        payment_dict = next(
            (p for p in payments if p["provider_payment_id"] == provider_payment_id),
            None,
        )
        if not payment_dict:
            return None
    ...
    async def update_payment_status(
        self, payment_id: str, status: PaymentStatus, paid_at: Optional[datetime] = None
    ) -> Optional[Payment]:
    ...
    async def process_webhook(
        self, provider: PaymentProvider, data: Dict[str, Any]
    ) -> Optional[Payment]:
        ...
        payment = await self.get_payment_by_provider_id(
        ...
        return await self.update_payment_status(
            payment.id, PaymentStatus(webhook_data.status), webhook_data.payment_date
        )
    ...
    async def check_payment_status(self, payment_id: str) -> Optional[Payment]:
        ...
        status_data = await adapter.get_payment_status(
            payment.provider_payment_id, config
        )

        # Atualizar status se necessário
        if status_data["status"] != payment.status:
            return await self.update_payment_status(
                payment.id, status_data["status"], status_data.get("payment_date")
            )
