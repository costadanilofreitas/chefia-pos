...
class SeatService:
    ...
    async def get_seat(self, seat_id: str) -> Seat:
        ...
        if seat_id not in self.seats:
            ...
            raise HTTPException(status_code=404, detail=f"Assento não encontrado: {seat_id}")
    ...
    async def delete_seat(self, seat_id: str) -> None:
        ...
        if seat_id in self.seat_order_items and self.seat_order_items[seat_id]:
            logger.error(f"Não é possível remover assento com itens associados: {seat_id}")
            raise HTTPException(
                status_code=400, 
        ...
        if seat_id in self.seat_payments and self.seat_payments[seat_id]:
            logger.error(f"Não é possível remover assento com pagamentos associados: {seat_id}")
            raise HTTPException(
                status_code=400, 
        ...
        for group_id, group in self.seat_groups.items():
            if seat_id in group.seat_ids:
                logger.error(f"Não é possível remover assento que pertence a um grupo: {seat_id}")
                raise HTTPException(
                    status_code=400, 
...
class SeatOrderService:
    ...
    async def assign_item_to_seat(self, item_data: SeatOrderItemCreate) -> SeatOrderItem:
        """
        ...
        logger.info(f"Item associado ao assento: {item_data.order_item_id} -> {item_data.seat_id}")
    ...
    async def remove_item_from_seat(self, seat_id: str, order_item_id: str) -> None:
        ...
        if seat_id not in self.seat_order_items:
            ...
            raise HTTPException(status_code=404, detail="Assento não tem itens associados")
        ...
        if not item_found:
            logger.error(f"Item não está associado ao assento: {order_item_id} -> {seat_id}")
            raise HTTPException(status_code=404, detail="Item não está associado ao assento")
            
        # Remover da lista de assentos do item
        if order_item_id in self.order_item_seats:
            self.order_item_seats[order_item_id] = [
                item for item in self.order_item_seats[order_item_id]
                if item.seat_id != seat_id
            ]
            
        logger.info(f"Item removido do assento: {order_item_id} -> {seat_id}")
    
    async def get_items_by_order_and_seat(self, order_id: str, seat_id: str) -> List[Dict[str, Any]]:
        """
...
class SeatPaymentService:
    ...
    def __init__(self, seat_service: SeatService, session_service: PaymentSessionService, split_service: BillSplitService):
        self.seat_service = seat_service
    ...
    async def create_seat_payment(self, payment_data: SeatPaymentCreate) -> SeatPayment:
        ...
        logger.info(f"Pagamento associado ao assento: {payment_data.payment_id} -> {payment_data.seat_id}")
        return seat_payment
    ...
    async def create_seat_group(self, group_data: SeatGroupCreate) -> SeatGroup:
        ...
        group = SeatGroup(
            ...
            seat_ids=group_data.seat_ids
        )
        
        self.seat_groups[group_id] = group
        logger.info(f"Grupo de assentos criado: {group_id} com {len(group_data.seat_ids)} assentos")
        return group
    ...
    async def get_seat_group(self, group_id: str) -> SeatGroup:
        ...
        if group_id not in self.seat_groups:
            ...
            raise HTTPException(status_code=404, detail=f"Grupo de assentos não encontrado: {group_id}")
    ...
    async def get_groups_by_seat(self, seat_id: str) -> List[SeatGroup]:
        ...
        return [
            group for group in self.seat_groups.values()
            if seat_id in group.seat_ids
        ]
    
    async def create_seat_bill_split(self, session_id: str, seat_ids: List[str], include_shared_items: bool = True) -> Dict[str, Any]:
        """
