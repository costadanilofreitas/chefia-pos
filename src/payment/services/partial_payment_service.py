...
class PaymentSessionService:
    ...
    async def create_session(self, order_id: str, total_amount: float) -> PaymentSession:
        """
    ...
    async def get_session(self, session_id: str) -> PaymentSession:
        ...
        if session_id not in self.sessions:
            ...
            raise HTTPException(status_code=404, detail=f"Sessão de pagamento não encontrada: {session_id}")
    ...
    async def update_session(self, session_id: str, paid_amount: Optional[float] = None, 
                           status: Optional[PaymentSessionStatus] = None) -> PaymentSession:
        """
    ...
    async def close_session(self, session_id: str) -> PaymentSession:
        ...
        if session.status != PaymentSessionStatus.OPEN:
            logger.error(f"Não é possível fechar sessão com status {session.status}: {session_id}")
            raise HTTPException(
                status_code=400, 
        ...
        if session.remaining_amount > 0:
            logger.error(f"Não é possível fechar sessão com valor restante: {session_id}")
            raise HTTPException(
                status_code=400, 
    ...
    async def cancel_session(self, session_id: str) -> PaymentSession:
        ...
        if session.status != PaymentSessionStatus.OPEN:
            logger.error(f"Não é possível cancelar sessão com status {session.status}: {session_id}")
            raise HTTPException(
                status_code=400, 
        ...
        return await self.update_session(session_id, status=PaymentSessionStatus.CANCELLED)
    
    async def add_partial_payment(self, session_id: str, method: PaymentMethod, 
                                amount: float, customer_name: Optional[str] = None,
                                customer_email: Optional[str] = None, 
                                customer_phone: Optional[str] = None,
                                description: Optional[str] = None,
                                metadata: Dict[str, Any] = {}) -> PartialPayment:
        """
        ...
        if session.status != PaymentSessionStatus.OPEN:
            logger.error(f"Não é possível adicionar pagamento a sessão com status {session.status}: {session_id}")
            raise HTTPException(
                status_code=400, 
        ...
        if amount > session.remaining_amount:
            logger.error(f"Valor de pagamento excede valor restante: {amount} > {session.remaining_amount}")
            raise HTTPException(
                status_code=400, 
        ...
        logger.info(f"Pagamento parcial adicionado: {partial_payment.id} à sessão {session_id}")
...
class BillSplitService:
    ...
    async def create_split(self, session_id: str, split_method: BillSplitMethod, 
                         number_of_parts: int = 1) -> BillSplit:
        """
        ...
        if session.status != PaymentSessionStatus.OPEN:
            logger.error(f"Não é possível criar divisão para sessão com status {session.status}: {session_id}")
            raise HTTPException(
                status_code=400, 
        ...
        existing_splits = [s for s in self.splits.values() if s.session_id == session_id]
        if existing_splits:
    ...
    async def get_split(self, split_id: str) -> BillSplit:
        ...
        if split_id not in self.splits:
            ...
            raise HTTPException(status_code=404, detail=f"Divisão de conta não encontrada: {split_id}")
    ...
    async def add_split_part(self, split_id: str, amount: float, name: Optional[str] = None) -> BillSplitPart:
        """
        ...
        if total_parts_amount > session.total_amount:
            logger.error(f"Valor total das partes excede o valor da sessão: {total_parts_amount} > {session.total_amount}")
            raise HTTPException(
                status_code=400, 
    ...
    async def create_equal_split(self, session_id: str, number_of_parts: int, 
        ...
        return {
            "split": split,
            "parts": parts,
            "amount_per_part": amount_per_part
        }
    
    async def create_custom_split(self, session_id: str, 
                                parts: List[Dict[str, Union[str, float]]]) -> Dict[str, Any]:
        """
        ...
        if not parts:
            ...
            raise HTTPException(
                status_code=400, 
                detail="É necessário fornecer pelo menos uma parte"
            )
            
        # Verificar se o total das partes é igual ao valor da sessão
        total_parts = sum(float(p["amount"]) for p in parts)
        if abs(total_parts - session.total_amount) > 0.01:  # Tolerância para arredondamento
            logger.error(f"Valor total das partes ({total_parts}) não corresponde ao valor da sessão ({session.total_amount})")
            raise HTTPException(
                status_code=400, 
    ...
    async def pay_split_part(self, part_id: str, method: PaymentMethod,
        ...
        if not part:
            ...
            raise HTTPException(status_code=404, detail=f"Parte não encontrada: {part_id}")
        ...
        return {
            "part": part,
            "payment": payment,
            "session": await self.session_service.get_session(session.id)  # Obter sessão atualizada
        }
    
    async def add_seat_payment(self, payment_id: str, seat_id: str, amount: float) -> SeatPayment:
        """
