...
TRANSACTIONS_DATA_FILE = os.path.join(
    "/home/ubuntu/pos-modern/data", "transactions.json"
)
...
RECURRING_TRANSACTIONS_DATA_FILE = os.path.join(
    "/home/ubuntu/pos-modern/data", "recurring_transactions.json"
)
REPORTS_DATA_FILE = os.path.join(
    "/home/ubuntu/pos-modern/data", "financial_reports.json"
)
...
class AccountsService:
    ...
    async def create_account(
        self, account_data: AccountCreate, user_id: str, user_name: str
    ) -> Account:
        """
    ...
    async def update_account(
        self, account_id: str, account_data: AccountUpdate, user_id: str, user_name: str
    ) -> Optional[Account]:
        """
        Atualiza uma conta.
    ...
    async def list_accounts(
        ...
        for account_data in self.accounts:
            ...
            if (
                ...
                and account_data.get("is_active", True) != is_active
            ):
                continue
    ...
    async def update_account_balance(
        ...
        for i, account in enumerate(self.accounts):
            if account.get("id") == account_id:
                ...
                # Registra o log
                operation_desc = (
                    "adicionado a" if operation == "add" else "subtraído de"
                )
                await log_info(
    ...
    async def create_transaction(
        self, transaction_data: TransactionCreate, user_id: str, user_name: str
    ) -> Transaction:
        """
        ...
        if transaction.status == PaymentStatus.PAID:
            operation = (
                "add" if transaction.type == TransactionType.INCOME else "subtract"
            )
        ...
        for i, transaction in enumerate(self.transactions):
            if transaction.get("id") == transaction_id:
                ...
                if (
                    ...
                    operation = (
                        "add"
                        if current_transaction.type == TransactionType.INCOME
                        else "subtract"
                    )
    ...
    async def list_transactions(
        ...
        for transaction_data in self.transactions:
            ...
            transaction_date = (
                datetime.strptime(transaction_data.get("date"), "%Y-%m-%d").date()
                if isinstance(transaction_data.get("date"), str)
                else transaction_data.get("date")
            )
    ...
    async def create_receivable(
        self, receivable_data: ReceivableCreate, user_id: str, user_name: str
    ) -> Receivable:
        """
        ...
        for i, receivable in enumerate(self.receivables):
            if receivable.get("id") == receivable_id:
                ...
                if (
                    ...
                    default_accounts = await self.list_accounts(
                    ...
                    if default_accounts:
                        ...
                        payment_amount = update_dict.get(
                            "payment_amount", current_receivable.amount
                        )
                        ...
                        try:
                            await self.create_transaction(
                                transaction_data=TransactionCreate(
                                    ...
                                    source_type=(
                                        SourceType.ORDER
                                        if current_receivable.source_type
                                        == SourceType.ORDER
    ...
    async def list_receivables(
        ...
        for receivable_data in self.receivables:
            ...
            due_date = (
                datetime.strptime(receivable_data.get("due_date"), "%Y-%m-%d").date()
                if isinstance(receivable_data.get("due_date"), str)
                else receivable_data.get("due_date")
            )
    ...
    async def create_payable(
        self, payable_data: PayableCreate, user_id: str, user_name: str
    ) -> Payable:
        """
    ...
    async def update_payable(
        self, payable_id: str, update_data: PayableUpdate, user_id: str, user_name: str
    ) -> Optional[Payable]:
        """
            ...
            if payable.get("id") == payable_id:
                ...
                if (
                    ...
                    if default_accounts:
                        ...
                        payment_amount = update_dict.get(
                            "payment_amount", current_payable.amount
                        )
                        payment_date = update_dict.get("payment_date", date.today())
                        ...
                        try:
                            ...
                            await log_error(
                                ...
                                source=LogSource.ACCOUNTS,
                                module="payables",
                                details={"payable_id": payable_id, "error": str(e)},
                            )
    ...
    async def list_payables(
        ...
        for payable_data in self.payables:
            ...
            due_date = (
                datetime.strptime(payable_data.get("due_date"), "%Y-%m-%d").date()
                if isinstance(payable_data.get("due_date"), str)
                else payable_data.get("due_date")
            )
    ...
    async def create_recurring_transaction(
        self, transaction_data: RecurringTransactionCreate, user_id: str, user_name: str
    ) -> RecurringTransaction:
        """
    ...
    async def list_recurring_transactions(
        ...
        for transaction_data in self.recurring_transactions:
            ...
            if (
                is_active is not None
                and transaction_data.get("is_active", True) != is_active
            ):
                continue
        ...
        for recurring in active_transactions:
            ...
            if recurring.last_generated:
                last_generated = (
                    ...
                    if isinstance(recurring.last_generated, date)
                    else datetime.strptime(recurring.last_generated, "%Y-%m-%d").date()
                )

                # Verifica se já foi gerada para o período atual com base no tipo de recorrência
                if (
                    recurring.recurrence_type == "daily"
                    and last_generated >= current_date
                ):
                    continue
                    ...
                    if (
                        last_generated.year == current_date.year
                        and last_generated.month == current_date.month
                    ):
                        continue
                    ...
                    if (
                        ...
                        and current_date.day != recurring.day_of_month
                    ):
                        continue
                elif recurring.recurrence_type == "quarterly":
                    # Verifica se passaram 3 meses
                    months_diff = (
                        (current_date.year - last_generated.year) * 12
                        + current_date.month
            ...
            if (
                recurring.recurrence_type in ["weekly", "biweekly"]
                and recurring.day_of_week is not None
            ):
            ...
            try:
                ...
                await self.update_recurring_transaction(
                    ...
                    update_data=RecurringTransactionUpdate(last_generated=current_date),
                    user_id=recurring.created_by,
                    user_name="Sistema",
                )
            except Exception as e:
                await log_error(
                    ...
                    source=LogSource.ACCOUNTS,
                    module="recurring_transactions",
                    details={"recurring_id": recurring.id, "error": str(e)},
                )

        return generated_transaction_ids

    async def create_financial_report(
        self, report_data: FinancialReportCreate, user_id: str, user_name: str
    ) -> FinancialReport:
        """
    ...
    async def _generate_cash_flow_report(
        ...
        transactions = await self.list_transactions(
            start_date=start_date, end_date=end_date, status=PaymentStatus.PAID
        )
        ...
        for transaction in transactions:
            ...
            daily_flow[transaction_date]["balance"] = (
                daily_flow[transaction_date]["income"]
                - daily_flow[transaction_date]["expense"]
            )
    ...
    async def _generate_receivables_report(
        self, start_date: date, end_date: date
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "summary": {
                ...
                "total_receivables": total_pending
                + total_partially_paid
                + total_overdue,
            },
        }

    async def _generate_payables_report(
        self, start_date: date, end_date: date
    ) -> Dict[str, Any]:
        """
    ...
    async def _generate_income_statement_report(
        self, start_date: date, end_date: date
    ) -> Dict[str, Any]:
        """
        ...
        for category, amount in income_by_category.items():
            income_percentage[category] = (
                (amount / total_income * 100) if total_income > 0 else 0
            )

        expense_percentage = {}
        for category, amount in expense_by_category.items():
            expense_percentage[category] = (
        ...
        return {
            ...
            "summary": {
                ...
                "profit_margin": (
                    (net_result / total_income * 100) if total_income > 0 else 0
                ),
            },
        }
    ...
    async def list_reports(
        ...
        for report_data in self.reports:
            ...
            report_start_date = (
                datetime.strptime(report_data.get("start_date"), "%Y-%m-%d").date()
                if isinstance(report_data.get("start_date"), str)
                else report_data.get("start_date")
            )
            report_end_date = (
                datetime.strptime(report_data.get("end_date"), "%Y-%m-%d").date()
                if isinstance(report_data.get("end_date"), str)
                else report_data.get("end_date")
            )
    ...
    async def get_financial_summary(self) -> Dict[str, Any]:
        ...
        overdue_receivables = [
            r for r in pending_receivables if r.due_date < date.today()
        ]
        ...
        transactions = await self.list_transactions(
            start_date=thirty_days_ago, end_date=date.today(), status=PaymentStatus.PAID
        )

        total_income_30d = sum(
            t.amount for t in transactions if t.type == TransactionType.INCOME
        )
        total_expense_30d = sum(
    ...
    async def _publish_accounts_event(
        self,
        event_type: str,
