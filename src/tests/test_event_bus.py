...
async def test_event_bus_multiple_handlers(event_bus):
    ...
    # Publicar um evento
    test_event = Event(event_type=EventType.SYSTEM_STARTED, data={"version": "1.0.0"})
    await event_bus.publish(test_event)
...
async def test_event_bus_unsubscribe(event_bus):
    ...
    # Publicar um evento
    test_event = Event(
        event_type=EventType.USER_LOGGED_OUT, data={"user": {"username": "test_user"}}
    )
    await event_bus.publish(test_event)
...
async def test_event_history_limit(event_bus):
    ...
    for i in range(10):
        event = Event(event_type=EventType.SYSTEM_STARTED, data={"iteration": i})
