...
from src.auth.security import has_permission, get_current_user
...
def test_option_group_data():
    return {
        ...
        "max_selections": 1,
        "options": [
            {"name": "Pão Francês", "price_adjustment": 0.0, "is_default": True},
            {"name": "Pão Integral", "price_adjustment": 1.0, "is_default": False},
            {"name": "Pão Australiano", "price_adjustment": 2.0, "is_default": False},
        ],
    }
...
def test_update_option_group(client, test_option_group_data):
    ...
    update_data = {
        ...
        "options": [
            {"name": "Cheddar", "price_adjustment": 0.0, "is_default": True},
            {"name": "Prato", "price_adjustment": 0.0, "is_default": False},
        ],
    }
...
def test_create_composite_product(client, test_composite_product_data):
    response = client.post(
        "/api/v1/composite-products", json=test_composite_product_data
    )
    assert response.status_code == 200
    data = response.json()
...
def test_calculate_composite_price(client, test_composite_product_data):
    ...
    response = client.post(
        "/api/v1/composite-products/calculate-price", json=price_data
    )
    assert response.status_code == 200
    data = response.json()
...
def test_integrated_product_flow(client):
    ...
    composite_response = client.post(
        "/api/v1/composite-products", json=composite_product_data
    )
    composite_id = composite_response.json()["id"]

    # 6. Buscar seções do produto composto
    sections_response = client.get(
        f"/api/v1/composite-products/{composite_id}/sections"
    )
    sections = sections_response.json()
