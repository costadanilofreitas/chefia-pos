...
class EndToEndTestCase(unittest.TestCase):
    ...
    async def test_remote_order_integration(self):
        ...
        internal_order = await remote_order_service.get_order_by_external_id("ifood-order-123")
        ...
        # Verificar se o pedido foi convertido corretamente
        rappi_internal_order = await remote_order_service.get_order_by_external_id("rappi-order-123")
    ...
    async def test_waiter_table_layout(self):
        ...
        layout_data = {
            ...
            "tables": [
                {"id": "table-1", "name": "Mesa 1", "x": 100, "y": 100, "width": 80, "height": 80, "shape": "circle", "seats": 4},
                {"id": "table-2", "name": "Mesa 2", "x": 200, "y": 100, "width": 80, "height": 80, "shape": "circle", "seats": 4},
                {"id": "table-3", "name": "Mesa 3", "x": 300, "y": 100, "width": 120, "height": 80, "shape": "rectangle", "seats": 6}
            ]
        }
        ...
        await table_layout_service.update_table(layout.id, "table-1", updated_table_data)
        
        # Verificar se a mesa foi atualizada
        updated_layout = await table_layout_service.get_layout(layout.id)
        updated_table = next((t for t in updated_layout.tables if t.id == "table-1"), None)
        ...
        order_data = {
            ...
            "items": [
                {"product_id": "product-3", "quantity": 1, "unit_price": 30.0}
            ],
            "table_id": "table-1",
            "waiter_id": "waiter-2"
        }
    ...
    async def test_split_payment(self):
        ...
        order_data = {
            ...
            "items": [
                {"product_id": "product-4", "quantity": 1, "unit_price": 50.0}
            ],
            "table_id": "table-3",
            "waiter_id": "waiter-3"
        }
