...
class SimulatedSATDriver(SATDriver):
    ...
    async def get_status(self) -> SATStatusResponse:
        ...
        if random.random() < self.error_probability:
            random_status = random.choice(
                [SATStatus.BUSY, SATStatus.ERROR, SATStatus.BLOCKED]
            )
    ...
    async def emit_cfe(self, cfe: CFe) -> SATResponse:
        ...
        if random.random() < self.error_probability:
            ...
            return SATResponse(
                success=False, message=error_message, details={"simulated": True}
            )
        ...
        cfe_copy.qrcode = (
            f"https://satsp.fazenda.sp.gov.br/NFCE/consulta?p={cfe_copy.chave_acesso}"
        )
  ...
  <infCFe versaoDadosEnt="0.07">
    ...
    <total>
      <vCFe>{cfe_copy.valor_total:.2f}</vCFe>
        ...
        logging.info(
            f"Simulação: CF-e emitido com sucesso. Chave: {cfe_copy.chave_acesso}"
        )
    ...
    async def cancel_cfe(self, cfe: CFe, reason: str) -> SATResponse:
        ...
        return SATResponse(
            ...
            cfe=cfe_copy,
            details={"simulated": True, "reason": reason},
        )
