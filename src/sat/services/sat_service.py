...
class SATService:
        ...
        try:
            if os.path.exists(config_path):
                ...
                self.config = SATConfig(
                    enabled=False, driver_type=SATDriverType.SIMULATED
                )
                ...
                for filename in os.listdir(pos_config_dir):
                    if filename.endswith(".json"):
                        ...
                        with open(config_path, "r") as f:
                            ...
                            if "sat" in pos_config and isinstance(
                                pos_config["sat"], dict
                            ):
    ...
    async def get_status(self, terminal_id: str) -> SATStatusResponse:
        ...
        if not driver or isinstance(driver, SATConfig):
            return SATStatusResponse(
                status=SATStatus.UNKNOWN, message="Driver SAT não inicializado"
            )
        ...
        try:
            ...
            return SATStatusResponse(
                status=SATStatus.ERROR, message=f"Erro ao obter status: {str(e)}"
            )

    async def convert_order_to_cfe(
        self, order_data: Dict[str, Any], terminal_id: str
    ) -> CFe:
        ...
        if not terminal_config:
            raise ValueError(
                f"Configuração SAT não encontrada para o terminal {terminal_id}"
            )
        ...
        if payment:
            ...
            cfe_payment = CFePagamento(
                tipo=sat_payment_code, valor=payment.get("amount", 0)
            )
    ...
    async def emit_cfe(
        ...
        if not await self.is_enabled(terminal_id):
            return SATResponse(
                success=False, message="SAT não habilitado para este terminal"
            )

        driver = self.drivers.get(terminal_id)
        if not driver or isinstance(driver, SATConfig):
            return SATResponse(success=False, message="Driver SAT não inicializado")
        ...
        try:
            ...
            return SATResponse(success=False, message=f"Erro ao emitir CF-e: {str(e)}")

    async def cancel_cfe(
        self, cfe_id: str, reason: str, terminal_id: str
    ) -> SATResponse:
        """Cancela um CF-e."""
        if not await self.is_enabled(terminal_id):
            return SATResponse(
        ...
        try:
            ...
            if not cfe:
                return SATResponse(
                    success=False, message=f"CF-e {cfe_id} não encontrado"
                )
            ...
            return SATResponse(
                success=False, message=f"Erro ao cancelar CF-e: {str(e)}"
            )
