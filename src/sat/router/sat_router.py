...
async def get_sat_status(
    terminal_id: str, current_user: User = Depends(get_current_user)
):
    ...
    if not is_enabled:
        return SATStatusResponse(
            status=SATStatus.OFFLINE, message="SAT não habilitado para este terminal"
        )
...
async def emit_cfe(
    request: SATEmitRequest, current_user: User = Depends(get_current_user)
):
...
async def cancel_cfe(
    request: SATCancelRequest, current_user: User = Depends(get_current_user)
):
    """Cancela um CF-e."""
...
async def get_sat_config(current_user: User = Depends(get_current_user)):
...
async def process_pending_cfes(current_user: User = Depends(get_current_user)):
