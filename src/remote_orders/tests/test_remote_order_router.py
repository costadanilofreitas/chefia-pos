...
class TestRemoteOrderRouter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        async def mock_get_current_user():
            return MagicMock(
                id="user123",
                username="testuser",
                permissions=["remote_orders.read", "remote_orders.update", "remote_platforms.read", "remote_platforms.update"]
            )
        ...
        self.sample_remote_order = {
            ...
            "payment": {"method": "CREDIT", "status": "PAID", "total": 50.0, "prepaid": True},
            "subtotal": 45.0,
            "delivery_fee": 5.0,
            "total": 50.0,
            "raw_data": {}
    ...
    def test_receive_webhook(self, mock_service):
        ...
        mock_service.process_remote_order = AsyncMock(return_value=RemoteOrder(**self.sample_remote_order))
    ...
    def test_list_remote_orders(self, mock_service):
        ...
        # Configurar o mock do serviço como AsyncMock
        mock_service.list_remote_orders = AsyncMock(return_value=[RemoteOrder(**self.sample_remote_order)])
    ...
    def test_get_remote_order(self, mock_service):
        ...
        # Configurar o mock do serviço como AsyncMock
        mock_service.get_remote_order = AsyncMock(return_value=RemoteOrder(**self.sample_remote_order))
        
        # Fazer a requisição
        response = self.client.get(f"/api/v1/remote-orders/{self.sample_remote_order['id']}")
        ...
        # Verificar se o serviço foi chamado corretamente
        mock_service.get_remote_order.assert_called_once_with(self.sample_remote_order['id'])
    
    @patch('src.remote_orders.router.remote_order_router.remote_order_service')
    def test_accept_remote_order(self, mock_service):
        """Testa o endpoint de aceitação de pedido remoto."""
        # Configurar o mock do serviço como AsyncMock
        mock_service.accept_remote_order = AsyncMock(return_value=RemoteOrder(**self.sample_remote_order))
        
        # Fazer a requisição
        response = self.client.post(f"/api/v1/remote-orders/{self.sample_remote_order['id']}/accept")
        ...
        # Verificar se o serviço foi chamado corretamente
        mock_service.accept_remote_order.assert_called_once_with(self.sample_remote_order['id'])
    
    @patch('src.remote_orders.router.remote_order_router.remote_order_service')
    def test_reject_remote_order(self, mock_service):
        """Testa o endpoint de rejeição de pedido remoto."""
        # Configurar o mock do serviço como AsyncMock
        mock_service.reject_remote_order = AsyncMock(return_value=RemoteOrder(**self.sample_remote_order))
    ...
    def test_list_platform_configs(self, mock_service):
        ...
        # Configurar o mock do serviço como AsyncMock
        mock_service.list_platform_configs = AsyncMock(return_value=[RemotePlatformConfig(**self.sample_platform_config)])
