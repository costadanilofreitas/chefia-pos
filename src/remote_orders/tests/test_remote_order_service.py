...
class TestRemoteOrderService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('src.remote_orders.services.remote_order_service.get_event_bus', return_value=self.event_bus_mock):
            ...
            self.service._load_platform_configs = MagicMock(return_value=[
                {
                    "platform": "ifood",
                    "enabled": True,
                    "api_key": "test_key",
        ...
        self.ifood_adapter_mock.convert_to_remote_order.return_value = self.sample_remote_order
    ...
    def test_accept_remote_order(self):
        ...
        async def async_test():
            ...
            order_service_mock.create_order = AsyncMock(return_value=MagicMock(id="internal123"))
            
            # Patch para o serviço de pedidos
            with patch('src.remote_orders.services.remote_order_service.order_service', order_service_mock):
                # Configurar o mock para retornar o pedido de exemplo ao buscar
                self.service.get_remote_order = AsyncMock(return_value=self.sample_remote_order)
                self.service.update_remote_order = AsyncMock(return_value=self.sample_remote_order)
                
                # Executar a função
                result = await self.service.accept_remote_order(self.sample_remote_order.id)
