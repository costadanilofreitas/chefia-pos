...
class TestIFoodAdapter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ifood_order_data = {
            ...
            "items": [
                {
                    ...
                    "totalPrice": 31.80,
                    "notes": "Sem cebola",
                    "options": [{"id": "opt1", "name": "Bacon extra", "price": 3.00}],
                },
                {
                    "id": "item2",
                    "name": "Refrigerante",
            ...
            "payments": [
                {"method": "CREDIT", "status": "PAID", "prepaid": True, "value": 36.80}
            ],
            "subTotal": 36.80,
            "deliveryFee": 5.00,
            "discount": 0.00,
    ...
    def test_convert_to_remote_order(self):
        ...
        async def async_test():
            # Executar a função
            remote_order = await self.adapter.convert_to_remote_order(
                self.ifood_order_data, self.config
            )
    ...
    def test_update_order_status(self):
        ...
        async def async_test():
            ...
            remote_order = RemoteOrder(
                ...
                customer=RemoteOrderCustomer(name="João Silva", phone="11999999999"),
                payment=RemoteOrderPayment(
                    method="CREDIT", status="PAID", total=31.80, prepaid=True
                ),
                subtotal=31.80,
                total=31.80,
                raw_data={},
    ...
    def test_verify_webhook_signature(self):
        ...
        async def async_test():
            ...
            result = await self.adapter.verify_webhook_signature(
                "test_signature", json.dumps(self.ifood_order_data), self.config
            )
