...
class TestRemoteOrderIntegration(unittest.TestCase):
    ...
    def test_remote_order_to_internal_order_flow(
        self, mock_get_event_bus, mock_order_service, mock_run
    ):
        ...
        remote_order_service._adapters = {
            RemotePlatform.IFOOD: MagicMock(
                convert_to_remote_order=AsyncMock(
        ...
        remote_order_service._load_platform_configs = MagicMock(
            return_value=[
                {
                    "platform": "ifood",
                    "enabled": True,
    ...
    @patch("src.remote_orders.services.remote_order_service.order_service")
    @patch("src.remote_orders.services.remote_order_service.get_event_bus")
    def test_internal_order_status_update_propagation(
        self, mock_get_event_bus, mock_order_service, mock_run
    ):
        ...
        # Configurar funções de carregamento/salvamento de dados
        remote_order_service._load_remote_orders = MagicMock(
            return_value=[
                {
                    "id": "remote123",
                    "platform": "ifood",
        ...
        async def async_handle():
            ...
            await remote_order_service.handle_order_status_change(
                "internal123", OrderStatus.DELIVERED
            )
    ...
    @patch("src.remote_orders.services.remote_order_service.order_service")
    @patch("src.remote_orders.services.remote_order_service.get_event_bus")
    def test_error_handling_during_order_processing(
        self, mock_get_event_bus, mock_order_service, mock_run
    ):
        ...
        # Configurar funções de carregamento/salvamento de dados
        remote_order_service._load_remote_orders = MagicMock(
            return_value=[self.sample_remote_order.dict()]
        )
        remote_order_service._save_remote_orders = MagicMock()
        remote_order_service._load_platform_configs = MagicMock(
            return_value=[
        ...
        remote_order_service.get_remote_order = AsyncMock(
            return_value=self.sample_remote_order
        )
        remote_order_service.update_remote_order = AsyncMock()
        ...
        async def async_accept():
            try:
                ...
                await remote_order_service.accept_remote_order(
