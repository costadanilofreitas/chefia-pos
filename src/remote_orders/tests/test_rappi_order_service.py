...
class TestRappiOrderService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.adapter_patch = patch('src.remote_orders.services.rappi_order_service.RappiAdapter', return_value=self.mock_adapter)
        self.mock_rappi_adapter = self.adapter_patch.start()
        
        # Mock para métodos do serviço
        self.service.get_rappi_configuration = AsyncMock(return_value={
    ...
    async def test_process_rappi_webhook_valid(self):
        ...
        # Processar webhook
        result = await self.service.process_rappi_webhook(webhook_data, "valid_signature")
    ...
    async def test_process_rappi_webhook_invalid_signature(self):
        ...
        # Processar webhook
        result = await self.service.process_rappi_webhook(webhook_data, "invalid_signature")
    ...
    async def test_reject_remote_order(self):
        ...
        with patch.object(self.service, 'process_refund', AsyncMock(return_value=True)) as mock_process_refund:
            # Rejeitar pedido
            result = await self.service.reject_remote_order("rappi_order_123", "Restaurante fechado")
            ...
            mock_process_refund.assert_called_once_with(mock_order, "Restaurante fechado")
    ...
    async def test_notify_customer_status_change(self):
        ...
        for status, expected_message in status_messages.items():
            ...
            # Notificar cliente
            result = await self.service.notify_customer_status_change(mock_order, status)
    ...
    async def test_handle_confirmation_timeout(self):
        ...
        with patch.object(self.service, 'reject_remote_order', AsyncMock(return_value=True)) as mock_reject:
            # Criar evento de exemplo
            event = Event(data={
                "order_id": "rappi_order_123"
            })
