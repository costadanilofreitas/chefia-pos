...
class TestRappiAdapter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.session_patch = patch('aiohttp.ClientSession', return_value=self.mock_session)
    ...
    async def test_convert_status_to_rappi(self):
        ...
        self.assertEqual(await adapter.convert_status_to_rappi(RemoteOrderStatus.PENDING), "PENDING")
        self.assertEqual(await adapter.convert_status_to_rappi(RemoteOrderStatus.CONFIRMED), "CONFIRMED")
        self.assertEqual(await adapter.convert_status_to_rappi(RemoteOrderStatus.REJECTED), "REJECTED")
        self.assertEqual(await adapter.convert_status_to_rappi(RemoteOrderStatus.PREPARING), "PREPARING")
        self.assertEqual(await adapter.convert_status_to_rappi(RemoteOrderStatus.READY), "READY")
        self.assertEqual(await adapter.convert_status_to_rappi(RemoteOrderStatus.DELIVERING), "DELIVERING")
        self.assertEqual(await adapter.convert_status_to_rappi(RemoteOrderStatus.DELIVERED), "DELIVERED")
        self.assertEqual(await adapter.convert_status_to_rappi(RemoteOrderStatus.CANCELLED), "CANCELLED")
    ...
    async def test_process_webhook_order_created(self):
        ...
        with patch.object(adapter, 'convert_to_remote_order', return_value=mock_order) as mock_convert:
    ...
    async def test_validate_webhook_signature(self):
        ...
        is_valid = await adapter.validate_webhook_signature("invalid_signature", payload)
    ...
    async def test_request_refund(self):
        ...
        async with RappiAdapter(self.config) as adapter:
            ...
            call_args = self.mock_session.post.call_args
            self.assertIn("json", call_args[1])
            self.assertEqual(call_args[1]["json"]["reason"], "Pedido cancelado pelo cliente")
