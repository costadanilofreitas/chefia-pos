...
class RappiOrderService:
    ...
    async def process_rappi_webhook(
        ...
        restaurant_id = webhook_data.get("restaurantId") or webhook_data.get(
            "restaurant_id"
        )
        ...
        if not config:
            logger.error(
                f"Rappi configuration not found for restaurant {restaurant_id}"
            )
            return None
    ...
    async def confirm_remote_order(self, order_id: str) -> bool:
        ...
        if not config:
            logger.error(
                f"Rappi configuration not found for restaurant {order.restaurant_id}"
            )
            return False
    ...
    async def reject_remote_order(self, order_id: str, reason: str) -> bool:
        ...
        async with RappiAdapter(config) as adapter:
            ...
            if success:
                ...
                # Notificar cliente
                await self.notify_customer_status_change(
                    order, RemoteOrderStatus.REJECTED
                )
    ...
    async def process_refund(self, order: RemoteOrder, reason: str) -> bool:
        ...
        if not order.payment or not order.payment.online:
            logger.info(
                f"Order {order.id} does not have online payment, no refund needed"
            )
            return True
        ...
        async with RappiAdapter(config) as adapter:
            success = await adapter.request_refund(
                order.external_id, reason, order.payment.total
            )
    ...
    async def notify_customer_status_change(
        self, order: RemoteOrder, status: RemoteOrderStatus
    ) -> bool:
        """
        ...
        async with RappiAdapter(config) as adapter:
            ...
            if success:
                logger.info(
                    f"Customer notification sent for order {order.id}: {message}"
                )
                return True
            else:
                logger.error(
                    f"Failed to send customer notification for order {order.id}"
                )
                return False
    ...
    async def handle_confirmation_timeout(self, event: Event) -> None:
        ...
        if (
            not order
            or order.source != "rappi"
            or order.status != RemoteOrderStatus.PENDING
        ):
            return
        ...
        await self.reject_remote_order(order_id, "Tempo de confirmação expirado")

    async def update_product_availability(
        self, restaurant_id: str, product_id: str, available: bool
    ) -> bool:
        """
        ...
        async with RappiAdapter(config) as adapter:
            success = await adapter.update_product_availability(
                rappi_product_id, available
            )

            if success:
                logger.info(
                    f"Product {product_id} availability updated to {available} in Rappi"
                )
                return True
            else:
                logger.error(
                    f"Failed to update product {product_id} availability in Rappi"
                )
                return False

    async def get_rappi_product_id(
        self, restaurant_id: str, pos_product_id: str
    ) -> Optional[str]:
        """
