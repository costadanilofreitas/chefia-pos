...
from src.remote_orders.services.rappi_order_service import get_rappi_order_service, RappiOrderService
...
async def rappi_webhook(
    ...
    try:
        ...
        logger.info(f"Received Rappi webhook: {webhook_data.get('eventType', 'unknown')}")
        
        # Processar webhook
        result = await rappi_service.process_rappi_webhook(
            webhook_data,
            x_rappi_signature or ""
        )
        
        if result:
            return {"status": "success", "message": "Webhook processed successfully"}
        else:
            return {"status": "ignored", "message": "Webhook ignored or no action required"}
...
async def update_rappi_order_status(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid status: {data.get('status')}")
...
async def update_product_availability(
    ...
    if not success:
        raise HTTPException(status_code=400, detail="Failed to update product availability")
