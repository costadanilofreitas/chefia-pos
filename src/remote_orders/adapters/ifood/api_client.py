...
class IFoodAPIClient:
    ...
    def __init__(self, 
        ...
        if not self.merchant_id:
            logger.warning("ID do estabelecimento no iFood não configurado. A integração não funcionará corretamente.")
    ...
    async def get_events(self, limit: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/v1.0/events:polling?limit={limit}",
                headers=headers
            )
    ...
    async def acknowledge_events(self, event_ids: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": True,
                "acknowledged_count": len(event_ids)
            }
    ...
    async def get_order_details(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/v1.0/orders/{order_id}",
                headers=headers
            )
            ...
            logger.error(f"Erro ao obter detalhes do pedido {order_id} do iFood: {str(e)}")
            error_response = None
            try:
                error_response = e.response.json() if hasattr(e, 'response') else None
            except:
    ...
    async def update_order_status(self, order_id: str, status: str, reason: str = None) -> Dict[str, Any]:
        """
        ...
        # Validar status
        valid_statuses = [
            "CONFIRMED", "READY_TO_PICKUP", "DISPATCHED", "CONCLUDED", "CANCELLED"
        ]
        ...
        try:
            ...
            logger.info(f"Status do pedido {order_id} atualizado para {status} no iFood")
            return {
                "success": True,
                "order_id": order_id,
                "status": status
            ...
            logger.error(f"Erro ao atualizar status do pedido {order_id} no iFood: {str(e)}")
            error_response = None
            try:
                error_response = e.response.json() if hasattr(e, 'response') else None
            except:
    ...
    async def verify_webhook_signature(self, signature: str, payload: str) -> bool:
        ...
        try:
            ...
            if not is_valid:
                logger.warning(f"Assinatura de webhook inválida: {signature} != {expected_signature}")
    ...
    async def convert_to_remote_order(self, order_data: Dict[str, Any], config: RemotePlatformConfig) -> RemoteOrder:
        """
        ...
        try:
            ...
            remote_order = RemoteOrder(
                ...
                scheduled_for=datetime.fromisoformat(order_data.get("scheduledFor")) if order_data.get("scheduledFor") else None,
                raw_data=order_data
            )
            ...
            try:
                ...
                if event_type == "PLACED":
                    ...
                    if order_result.get("success"):
                        ...
                        logger.error(f"Erro ao obter detalhes do pedido {order_id}: {order_result.get('error')}")
    ...
    async def start_event_polling(self, callback=None, interval_seconds: int = 60) -> None:
        """
        ...
        """
        logger.info(f"Iniciando polling de eventos do iFood a cada {interval_seconds} segundos")
