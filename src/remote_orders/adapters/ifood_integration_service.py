...
class IFoodIntegrationService:
    ...
    def __init__(self, 
        ...
        self.auto_confirm_orders = self.config.settings.get("auto_confirm_orders", False)
        self.confirmation_timeout_minutes = self.config.settings.get("confirmation_timeout_minutes", 15)
    ...
    def _load_default_config(self) -> RemotePlatformConfig:
        ...
        return RemotePlatformConfig(
            ...
            settings={
                "auto_confirm_orders": os.environ.get("IFOOD_AUTO_CONFIRM", "false").lower() == "true",
                "confirmation_timeout_minutes": int(os.environ.get("IFOOD_CONFIRMATION_TIMEOUT", "15"))
            }
        )
    
    async def process_webhook(self, request: Request, x_ifood_signature: str = Header(None), background_tasks: BackgroundTasks = None) -> Dict[str, Any]:
        """
        ...
        """
        return await self.webhook_handler.process_webhook(request, x_ifood_signature, background_tasks)
    ...
    async def confirm_order(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if order.get("status") != RemoteOrderStatus.PENDING:
                logger.warning(f"Pedido {order_id} já foi processado, status atual: {order.get('status')}")
                return {"success": True, "message": "Pedido já processado", "status": order.get("status")}
            
            # Atualizar status no iFood
            ifood_order_id = order.get("external_order_id")
            update_result = await self.api_client.update_order_status(ifood_order_id, "CONFIRMED")
            
            if not update_result.get("success"):
                logger.error(f"Erro ao confirmar pedido {order_id} no iFood: {update_result.get('error')}")
    ...
    async def cancel_order(self, order_id: str, reason: str = "Cancelado pelo restaurante") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Atualizar status no iFood
            ifood_order_id = order.get("external_order_id")
            update_result = await self.api_client.update_order_status(ifood_order_id, "CANCELLED", reason)
            
            if not update_result.get("success"):
                logger.error(f"Erro ao cancelar pedido {order_id} no iFood: {update_result.get('error')}")
                return update_result
    ...
    async def update_order_status(self, order_id: str, status: RemoteOrderStatus) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Atualizar status no iFood
            ifood_order_id = order.get("external_order_id")
            update_result = await self.api_client.update_order_status(ifood_order_id, ifood_status)
            
            if not update_result.get("success"):
                logger.error(f"Erro ao atualizar status do pedido {order_id} no iFood: {update_result.get('error')}")
                return update_result
    ...
    async def _handle_order_confirmation(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            remote_order = await self.api_client.convert_to_remote_order(order_data, self.config)
            ...
            logger.info(f"Pedido {order_id} registrado para confirmação (auto: {auto_confirm})")
            return {
                "success": True,
                "order_id": order_id,
    ...
    async def _handle_notification(self, event_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if event_type == "new_order":
                ...
                if self.notification_service and order_id in self.orders:
                    customer_phone = self.orders[order_id].get("customer", {}).get("phone")
                    if customer_phone:
                        await self.notification_service.send_order_notification(
                            customer_phone,
                            order_id,
                ...
                if order_id in self.orders:
                    ...
                    # Processar reembolso se necessário
                    if self.orders[order_id].get("payment", {}).get("prepaid", False) and self.payment_service:
                        await self._process_refund(order_id, reason)
    ...
    async def _confirmation_timeout(self, order_id: str) -> None:
        ...
        try:
            ...
            if order_id in self.orders and self.orders[order_id]["status"] == RemoteOrderStatus.PENDING:
    ...
    async def _process_refund(self, order_id: str, reason: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.payment_service:
                ...
                if not payment_id:
                    logger.error(f"ID de pagamento não encontrado para pedido {order_id}")
                ...
                if refund_result.get("success"):
                    ...
                    order["payment"]["refund_id"] = refund_result.get("refund", {}).get("id")
                    order["updated_at"] = datetime.now().isoformat()
                    ...
                    logger.error(f"Erro ao processar reembolso para pedido {order_id}: {refund_result.get('error')}")
                    return refund_result
            else:
                logger.warning(f"Serviço de pagamento não disponível para reembolso do pedido {order_id}")
                return {"success": False, "error": "Serviço de pagamento não disponível"}
                
        except Exception as e:
            logger.error(f"Erro ao processar reembolso para pedido {order_id}: {str(e)}")
            return {"success": False, "error": str(e)}
    ...
    async def _handle_event_callback(self, event_type: str, data: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            if event_type == "new_order":
                ...
                if internal_order_id:
                    # Atualizar status interno
                    self.orders[internal_order_id]["status"] = RemoteOrderStatus.CANCELLED
                    self.orders[internal_order_id]["updated_at"] = datetime.now().isoformat()
                    self.orders[internal_order_id]["cancellation_reason"] = "Cancelado pelo iFood"
                    
                    # Processar reembolso se necessário
                    if self.orders[internal_order_id].get("payment", {}).get("prepaid", False) and self.payment_service:
                        await self._process_refund(internal_order_id, "Cancelado pelo iFood")
                    
                    # Enviar notificação
                    if self.notification_service:
                        customer_phone = self.orders[internal_order_id].get("customer", {}).get("phone")
                        if customer_phone:
                            await self.notification_service.send_order_notification(
                                customer_phone,
                                internal_order_id,
