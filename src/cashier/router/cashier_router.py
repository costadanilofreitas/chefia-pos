...
async def close_cashier(
    ...
    if cashier.status != CashierStatus.OPEN:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="O caixa já está fechado."
        )

    # Verificar se o operador é o mesmo que abriu o caixa ou se é um gerente
    if (
        cashier.current_operator_id != close_data.operator_id
        and current_user.role != "gerente"
    ):
        raise HTTPException(
...
async def register_cashier_operation(
    ...
    if (
        cashier.current_operator_id != operation.operator_id
        and current_user.role != "gerente"
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    if (
        operation.operation_type == OperationType.WITHDRAWAL
        and "cashier:withdrawal" not in current_user.permissions
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def get_terminal_cashier_status(
    terminal_id: str, current_user: User = Depends(get_current_active_user)
):
    """
    ...
    try:
        ...
        business_day = None
        try:
            business_day = await business_day_service.get_business_day(
                terminal_cashier.business_day_id
            )
        except Exception as e:
            logger.warning(
                f"Erro ao buscar business_day {terminal_cashier.business_day_id}: {e}"
            )
        ...
        return {
            ...
            "operator_name": getattr(
                terminal_cashier, "current_operator_name", "Operador"
            ),
...
async def get_current_cashiers(current_user: User = Depends(get_current_active_user)):
    """
    ...
    cashiers = await service.list_cashiers(
        business_day_id=business_day.id, status=CashierStatus.OPEN
    )
...
async def get_cashier(
    cashier_id: str, current_user: User = Depends(get_current_active_user)
):
    """
...
async def get_cashier_operations(
    ...
    operations = await service.get_cashier_operations(
        cashier_id, operation_type=operation_type, limit=limit, offset=offset
    )
...
async def update_cashier(
    ...
    current_user: User = Depends(
        has_permission(Permission.CASHIER_CLOSE)
    ),  # Assuming close permission allows update
):
    """
    ...
    updated_cashier = await service.update_cashier(
        cashier_id, update_data.dict(exclude_unset=True)
    )
