...
async def publish_cashier_opened(cashier: Cashier) -> None:
    ...
    logger.info(
        f"Evento de abertura de caixa publicado: {cashier.id} - Terminal: {cashier.terminal_id}"
    )
...
async def publish_cashier_closed(cashier: Cashier) -> None:
    ...
    await event_bus.publish(event)
    logger.info(
        f"Evento de fechamento de caixa publicado: {cashier.id} - Terminal: {cashier.terminal_id}"
    )


async def publish_cashier_operation(
    operation: CashierOperationResponse, cashier: Cashier
) -> None:
    ...
    await event_bus.publish(event)
    logger.info(
        f"Evento de operação de caixa publicado: {operation.operation_type} - Valor: {operation.amount} - Caixa: {cashier.id}"
    )


async def publish_cashier_updated(
    cashier: Cashier, updated_fields: Dict[str, Any]
) -> None:
    ...
    await event_bus.publish(event)
    logger.info(
        f"Evento de atualização de caixa publicado: {cashier.id} - campos: {list(updated_fields.keys())}"
    )
