...
class CashierService:
    ...
    async def create_cashier(self, cashier: Cashier) -> Cashier:
        ...
        if operator_cashier:
            raise ValueError(f"O operador já possui um caixa aberto (ID: {operator_cashier['id']}).")
        
        # Verificar se o terminal já tem um caixa aberto
        terminal_cashier = next((c for c in cashiers if c["terminal_id"] == cashier.terminal_id and c["status"] == CashierStatus.OPEN), None)
        if terminal_cashier:
            raise ValueError(f"O terminal já possui um caixa aberto (ID: {terminal_cashier['id']}).")
    ...
    async def get_cashier_by_operator(self, operator_id: str) -> Optional[Cashier]:
        ...
        cashier_dict = next((c for c in cashiers if c["current_operator_id"] == operator_id and c["status"] == CashierStatus.OPEN), None)
        if not cashier_dict:
            return None
    ...
    async def get_cashier_by_terminal(self, terminal_id: str) -> Optional[Cashier]:
        ...
        cashier_dict = next((c for c in cashiers if c["terminal_id"] == terminal_id and c["status"] == CashierStatus.OPEN), None)
        if not cashier_dict:
            return None
    ...
    async def update_cashier(self, cashier_id: str, update_data: Dict[str, Any]) -> Cashier:
        """Atualiza um caixa."""
        cashiers = self._load_cashiers()
        
        # Encontrar o caixa a ser atualizado
        cashier_index = next((i for i, c in enumerate(cashiers) if c["id"] == cashier_id), None)
    ...
    async def register_operation(
        ...
        if cashiers[cashier_index]["status"] != CashierStatus.OPEN:
            raise ValueError("O caixa está fechado. Não é possível registrar operações.")
        ...
        if operation.operation_type in [OperationType.SALE, OperationType.DEPOSIT]:
            ...
            new_expected_balance = expected_balance + (operation.amount if operation.payment_method == PaymentMethod.CASH else 0)
        elif operation.operation_type in [OperationType.REFUND, OperationType.WITHDRAWAL]:
            new_balance = current_balance - operation.amount
            new_expected_balance = expected_balance - (operation.amount if operation.payment_method == PaymentMethod.CASH else 0)
        else:
        ...
        if operation.operation_type in [OperationType.REFUND, OperationType.WITHDRAWAL] and new_balance < 0:
            raise ValueError(f"Saldo insuficiente para a operação. Saldo atual: R$ {current_balance:.2f}, Valor da operação: R$ {operation.amount:.2f}")
    ...
    async def get_cashier_operations(
        ...
        if operation_type:
            cashier_operations = [op for op in cashier_operations if op["operation_type"] == operation_type]
    ...
    async def generate_cashier_report(self, cashier_id: str) -> CashierReport:
        ...
        # Calcular totais
        total_sales = sum(op.amount for op in operations if op.operation_type == OperationType.SALE)
        total_refunds = sum(op.amount for op in operations if op.operation_type == OperationType.REFUND)
        total_withdrawals = sum(op.amount for op in operations if op.operation_type == OperationType.WITHDRAWAL)
        total_deposits = sum(op.amount for op in operations if op.operation_type == OperationType.DEPOSIT)
    ...
    def _load_cashiers(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Erro ao decodificar arquivo JSON de caixas. Iniciando com lista vazia.")
            return []
    ...
    def _load_operations(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Erro ao decodificar arquivo JSON de operações. Iniciando com lista vazia.")
            return []
