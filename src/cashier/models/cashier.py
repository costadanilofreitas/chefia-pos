...
class CashierBase(BaseModel):
    ...
    terminal_id: str = Field(..., description="ID do terminal/dispositivo onde o caixa está operando")
    business_day_id: str = Field(..., description="ID do dia de operação ao qual o caixa está associado")
...
class CashierCreate(CashierBase):
    ...
    notes: Optional[str] = Field(None, description="Observações sobre a abertura do caixa")
...
class CashierClose(BaseModel):
    ...
    physical_cash_amount: float = Field(..., description="Valor físico em dinheiro contado no fechamento")
    notes: Optional[str] = Field(None, description="Observações sobre o fechamento do caixa")
...
class CashierOperation(BaseModel):
    ...
    operator_id: str = Field(..., description="ID do usuário que está realizando a operação")
    payment_method: Optional[PaymentMethod] = Field(None, description="Método de pagamento (para vendas/reembolsos)")
    related_entity_id: Optional[str] = Field(None, description="ID da entidade relacionada (venda, pedido, etc.)")
...
class CashierWithdrawal(BaseModel):
    ...
    operator_id: str = Field(..., description="ID do usuário que está realizando a retirada")
    reason: str = Field(..., description="Motivo da retirada")
    authorized_by: Optional[str] = Field(None, description="ID do usuário que autorizou a retirada (se aplicável)")
    notes: Optional[str] = Field(None, description="Observações adicionais sobre a retirada")


class Cashier(CashierBase):
    """Modelo completo de um caixa."""
    id: str = Field(..., description="ID único do caixa")
    status: CashierStatus = Field(..., description="Status do caixa (aberto ou fechado)")
    current_operator_id: Optional[str] = Field(None, description="ID do operador atual do caixa")
    opening_balance: float = Field(0.0, description="Saldo inicial do caixa")
    current_balance: float = Field(0.0, description="Saldo atual do caixa")
    expected_balance: float = Field(0.0, description="Saldo esperado do caixa (calculado)")
    physical_cash_amount: Optional[float] = Field(None, description="Valor físico em dinheiro contado no fechamento")
    cash_difference: Optional[float] = Field(None, description="Diferença entre saldo esperado e físico")
    opened_at: Optional[str] = Field(None, description="Data e hora de abertura no formato ISO")
    closed_at: Optional[str] = Field(None, description="Data e hora de fechamento no formato ISO")
    created_at: str = Field(..., description="Data e hora de criação do registro")
    updated_at: str = Field(..., description="Data e hora da última atualização do registro")
    notes: Optional[str] = Field(None, description="Observações sobre o caixa")
