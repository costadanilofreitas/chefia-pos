...
class KioskOptimizationService:
    ...
    async def optimize_kiosk_experience(
        ...
        try:
            ...
            customer_behavior = await self._get_customer_behavior(restaurant_id, kiosk_id)
            ...
            kiosk_ids = [kiosk_id] if kiosk_id else await self._get_all_kiosk_ids(restaurant_id)
            ...
            for kid in kiosk_ids:
                ...
                expected_conversion_lift = await self._calculate_expected_conversion_lift(
                    restaurant_id=restaurant_id,
                    kiosk_id=kid,
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Error optimizing kiosk experience: {str(e)}"
            )
    
    async def _get_all_kiosk_ids(
        self,
        restaurant_id: str
    ) -> List[str]:
        """
    ...
    async def _get_sales_data(
        ...
        return {
            "top_selling_items": [
                {"id": "item-1", "name": "X-Burger", "category": "burgers", "price": 18.90, "sales_count": 1250},
                {"id": "item-2", "name": "Combo X-Burger", "category": "combos", "price": 25.90, "sales_count": 980},
                {"id": "item-3", "name": "Batata Frita G", "category": "sides", "price": 12.90, "sales_count": 870},
                {"id": "item-4", "name": "Milk Shake Chocolate", "category": "desserts", "price": 15.90, "sales_count": 750},
                {"id": "item-5", "name": "X-Salada", "category": "burgers", "price": 20.90, "sales_count": 720},
                {"id": "item-6", "name": "Combo X-Salada", "category": "combos", "price": 27.90, "sales_count": 680},
                {"id": "item-7", "name": "Refrigerante 500ml", "category": "drinks", "price": 8.90, "sales_count": 1500},
                {"id": "item-8", "name": "Sundae", "category": "desserts", "price": 10.90, "sales_count": 650}
            ],
    ...
    async def _identify_recommended_items(
        ...
        top_items = sales_data["top_selling_items"][:self.default_parameters["max_highlighted_items"]]
    ...
    async def _identify_recommended_promotions(
        ...
        if "payment" in abandonment_data["abandonment_by_step"] and abandonment_data["abandonment_by_step"]["payment"] > 0.1:
            recommended_promotions.append({
                "promotion_id": f"promo-{uuid.uuid4().hex[:8]}",
                "name": "Frete Grátis",
        ...
        return recommended_promotions[:self.default_parameters["max_highlighted_promotions"]]
    ...
    async def _calculate_expected_conversion_lift(
        ...
        conversion_lift = ((new_conversion - current_conversion) / current_conversion) * 100
    ...
    async def _generate_optimization_reason(
        ...
        top_abandonment_reason = abandonment_data["common_abandonment_reasons"][0]["reason"]
        ...
        abandonment_reason = abandonment_reasons_map.get(top_abandonment_reason, top_abandonment_reason)
