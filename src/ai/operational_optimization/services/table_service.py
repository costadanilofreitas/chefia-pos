...
class TableOptimizationService:
    ...
    def __init__(self):
        ...
        self.default_parameters = {
            ...
            "time_windows": ["11:00-13:00", "13:00-15:00", "18:00-20:00", "20:00-22:00"]
        }
    ...
    async def optimize_table_distribution(
        ...
        """
        logger.info(f"Optimizing table distribution for restaurant {restaurant_id} on {date.date()}")
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Error optimizing table distribution: {str(e)}"
            )
    ...
    async def _get_demand_forecast(
        ...
        while current_date <= end_date:
            ...
            for hour in range(10, 23):  # 10h às 22h
                ...
                point = ForecastPoint(
                    timestamp=datetime.combine(current_date.date(), datetime.min.time().replace(hour=hour)),
                    value=value,
    ...
    async def _get_current_table_layout(
        ...
        return [
            {"id": "table-1", "type": "small", "capacity": 2, "position": {"x": 1, "y": 1}, "area": "main"},
            {"id": "table-2", "type": "small", "capacity": 2, "position": {"x": 1, "y": 2}, "area": "main"},
            {"id": "table-3", "type": "small", "capacity": 2, "position": {"x": 1, "y": 3}, "area": "main"},
            {"id": "table-4", "type": "small", "capacity": 2, "position": {"x": 1, "y": 4}, "area": "main"},
            {"id": "table-5", "type": "medium", "capacity": 4, "position": {"x": 2, "y": 1}, "area": "main"},
            {"id": "table-6", "type": "medium", "capacity": 4, "position": {"x": 2, "y": 2}, "area": "main"},
            {"id": "table-7", "type": "medium", "capacity": 4, "position": {"x": 2, "y": 3}, "area": "main"},
            {"id": "table-8", "type": "medium", "capacity": 4, "position": {"x": 2, "y": 4}, "area": "main"},
            {"id": "table-9", "type": "large", "capacity": 6, "position": {"x": 3, "y": 1}, "area": "main"},
            {"id": "table-10", "type": "large", "capacity": 6, "position": {"x": 3, "y": 2}, "area": "main"},
            {"id": "table-11", "type": "xl", "capacity": 8, "position": {"x": 3, "y": 3}, "area": "main"},
            {"id": "table-12", "type": "xl", "capacity": 8, "position": {"x": 3, "y": 4}, "area": "main"}
        ]
    ...
    async def _calculate_expected_demand(
        ...
        relevant_points = [
            ...
            if p.timestamp.date() == date.date() and start_hour <= p.timestamp.hour < end_hour
        ]
    ...
    async def _calculate_recommended_tables(
        ...
        groups = {
            "1-2": int(total_customers * self.default_parameters["default_group_distribution"]["1-2"] / 1.5),
            "3-4": int(total_customers * self.default_parameters["default_group_distribution"]["3-4"] / 3.5),
            "5-6": int(total_customers * self.default_parameters["default_group_distribution"]["5-6"] / 5.5),
            "7+": int(total_customers * self.default_parameters["default_group_distribution"]["7+"] / 8)
        }
        ...
        if groups["5-6"] > current_tables["large"] and current_tables["small"] >= 2:
            combine_count = min(
                groups["5-6"] - current_tables["large"],
                current_tables["small"] // 2
            )
            if combine_count > 0:
                recommendations.append({
                    "action": "combine",
        ...
        if total_customers > sum(t["capacity"] for t in current_layout) * (240 / avg_table_time):
    ...
    async def _generate_recommendation_reason(
        ...
        policy_recs = [r for r in table_recommendations if r["action"] in ["reservations", "rotation"]]
        ...
        if add_recs:
            explanation += "Recomendamos adicionar " + ", ".join([f"{r['count']} mesas do tipo {r['table_type']}" for r in add_recs]) + ". "
        
        if remove_recs:
            explanation += "Recomendamos remover " + ", ".join([f"{r['count']} mesas do tipo {r['table_type']}" for r in remove_recs]) + ". "
        
        if combine_recs:
            explanation += "Recomendamos combinar mesas para acomodar grupos maiores. "
        
        if split_recs:
            explanation += "Recomendamos dividir mesas para acomodar mais grupos pequenos. "
