...
class OperationalOptimizationService:
    ...
    async def generate_all_recommendations(
        ...
        """
        logger.info(f"Generating all optimization recommendations for restaurant {restaurant_id}")
        ...
        try:
            ...
            if config.staffing_optimization_enabled:
                staff_recommendations = await self.staff_service.generate_staff_recommendations(
                    restaurant_id=restaurant_id,
                    start_date=start_date,
                    end_date=end_date
                )
            ...
            if config.delivery_optimization_enabled:
                ...
                while current_date <= end_date:
                    daily_optimizations = await self.delivery_service.optimize_delivery_operations(
                        restaurant_id=restaurant_id,
                        date=current_date
                    )
            ...
            if config.table_optimization_enabled:
                ...
                current_date = start_date
                while current_date <= end_date:
                    daily_recommendations = await self.table_service.optimize_table_distribution(
                        restaurant_id=restaurant_id,
                        date=current_date
                    )
            ...
            if config.kiosk_optimization_enabled:
                kiosk_optimizations = await self.kiosk_service.optimize_kiosk_experience(
                    restaurant_id=restaurant_id
                )
            ...
            if config.whatsapp_optimization_enabled:
                ...
                inactive_campaigns = await self.whatsapp_service.generate_campaign_recommendations(
                    restaurant_id=restaurant_id,
                    campaign_type="inactive_customers"
                )
                
                # Gerar campanhas de pós-compra
                post_purchase_campaigns = await self.whatsapp_service.generate_campaign_recommendations(
                    restaurant_id=restaurant_id,
            ...
            results["metrics"] = await self._calculate_optimization_metrics(results["recommendations"])
            
            return results
            
        except Exception as e:
            logger.error(f"Error generating optimization recommendations: {str(e)}", exc_info=True)
            raise HTTPException(
                status_code=500, 
    ...
    async def _calculate_optimization_metrics(
        ...
        if "staffing" in recommendations:
            ...
            for rec in staff_recs:
                if rec.current_staff_count and rec.recommended_staff_count < rec.current_staff_count:
                    # Estimar economia por funcionário reduzido (valor fictício)
                    staff_savings += (rec.current_staff_count - rec.recommended_staff_count) * 150.0
        ...
        if "tables" in recommendations:
            ...
            for rec in table_recs:
                ...
                revenue_increase += 0.1 * rec.expected_customer_volume * 30.0  # 10% mais clientes * ticket médio
        ...
        if "kiosk" in recommendations:
            ...
            for opt in kiosk_opts:
                # Estimar aumento de receita (valor fictício)
                revenue_increase += opt.expected_conversion_lift * 0.01 * 1000.0  # % de aumento * base diária
        ...
        if "whatsapp" in recommendations:
            ...
            if "inactive_customers" in recommendations["whatsapp"]:
                whatsapp_campaigns.extend(recommendations["whatsapp"]["inactive_customers"])
            ...
            for campaign in whatsapp_campaigns:
                ...
                if "inactive_customers" in campaign.name.lower():
                    ...
                    avg_order_value = campaign.target_segment.get("avg_previous_order_value", 50.0)
                    expected_response_rate = campaign.expected_response_rate
                    revenue_increase += customer_count * avg_order_value * expected_response_rate
                else:
                    # Campanhas de pós-compra
                    customer_count = campaign.target_segment.get("customer_count", 100)
                    avg_order_value = campaign.target_segment.get("avg_order_value", 50.0)
                    expected_response_rate = campaign.expected_response_rate
                    revenue_increase += customer_count * avg_order_value * expected_response_rate * 0.5  # 50% de retorno
        ...
        metrics["potential_revenue_increase"] = round(metrics["potential_revenue_increase"], 2)
        metrics["potential_efficiency_increase"] = round(metrics["potential_efficiency_increase"], 2)
