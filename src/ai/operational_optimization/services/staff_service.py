...
class StaffOptimizationService:
    ...
    async def generate_staff_recommendations(
        ...
        try:
                ...
                for role in roles:
                    ...
                    for period_name, (start_hour, end_hour) in time_periods.items():
                        ...
                        period_points = [
                            ...
                            if point.timestamp.date() == current_date.date()
                            and start_hour <= point.timestamp.hour < end_hour
                        ]
                        ...
                        avg_customer_volume = sum(
                            point.value for point in period_points
                        ) / len(period_points)

                        # Calcular número recomendado de funcionários
                        customers_per_staff = self.default_parameters[
                            "customers_per_staff"
                        ][role]
                        time_adjustment = self.default_parameters[
                            "time_period_adjustment"
                        ][period_name]
                        ...
                        recommended_staff = math.ceil(
                            avg_customer_volume / adjusted_capacity
                        )
            ...
            logger.error(
                f"Error generating staff recommendations: {str(e)}", exc_info=True
            )
            raise HTTPException(
                status_code=500,
    ...
    async def optimize_staff_schedule(
        ...
        """
        logger.info(
            f"Optimizing staff schedule for restaurant {restaurant_id} on {date}"
        )
        ...
        try:
            ...
            recommendations = await self.generate_staff_recommendations(
                restaurant_id=restaurant_id, start_date=date, end_date=date
            )
            ...
            for rec in recommendations:
                ...
                optimized_schedule["schedule"][period][role] = {
                    ...
                    "current_count": current_schedule.get("schedule", {})
                    .get(period, {})
                    .get(role, {})
                    .get("count", 0),
            ...
            optimized_schedule["metrics"] = {
                ...
                "optimization_percentage": (
                    round((1 - total_recommended / total_current) * 100, 2)
                    if total_current > 0
                    else 0
                ),
            }
