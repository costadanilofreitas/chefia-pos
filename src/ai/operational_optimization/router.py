...
async def recommend_whatsapp_campaigns(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Error recommending WhatsApp campaigns: {str(e)}"
        )
...
async def schedule_whatsapp_campaign(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Error scheduling WhatsApp campaign: {str(e)}"
        )
...
async def generate_all_recommendations(
    ...
    try:
        ...
        logger.error(f"Error generating all optimization recommendations: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, 
