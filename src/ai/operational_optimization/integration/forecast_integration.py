...
from src.ai.demand_forecast.models import ForecastResult, TimeGranularity, ForecastDimension, ModelType
...
class ForecastIntegrationService:
    ...
    async def get_demand_forecast(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Error getting demand forecast: {str(e)}"
            )
            ...
            while current_date <= end_date:
                ...
                for hour in range(8, 23):  # 8h às 22h
                    ...
                    weather_record = {
                        ...
                        "precipitation": 0 if condition in ["clear", "cloudy"] else random.uniform(0.1, 15),
                        "wind_speed": random.uniform(0, 30),
                        "weather_condition": condition,
                        "weather_code": {"clear": 800, "cloudy": 803, "rainy": 500, "stormy": 200}[condition]
                    }
    ...
    async def get_events_data(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Error getting events data: {str(e)}"
            )
    ...
    async def enrich_forecast_with_external_data(
        ...
        """
        logger.info(f"Enriching forecast with external data for restaurant {restaurant_id}")
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Error enriching forecast: {str(e)}"
            )
