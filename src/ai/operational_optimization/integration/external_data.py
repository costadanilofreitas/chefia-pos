...
class ExternalDataService:
        ...
        try:
            ...
            for day in range(days):
                ...
                for hour in range(24):
                    ...
                    weather_record = {
                        "dt": int(datetime.combine(date.date(), datetime.min.time().replace(hour=hour)).timestamp()),
                        "main": {
                            "temp": temperature,
                        ...
                        "weather": [{
                            "id": {"clear": 800, "cloudy": 803, "rainy": 500, "stormy": 200}[condition],
                            "main": {"clear": "Clear", "cloudy": "Clouds", "rainy": "Rain", "stormy": "Thunderstorm"}[condition],
                            "description": {"clear": "céu limpo", "cloudy": "nuvens dispersas", "rainy": "chuva leve", "stormy": "tempestade"}[condition],
                            "icon": {"clear": "01d", "cloudy": "03d", "rainy": "10d", "stormy": "11d"}[condition]
                        }],
                        "clouds": {
                            "all": {"clear": random.uniform(0, 10), "cloudy": random.uniform(30, 70), "rainy": random.uniform(60, 90), "stormy": random.uniform(80, 100)}[condition]
                        },
                        "wind": {
                            "speed": random.uniform(0, 30),
                            "deg": random.uniform(0, 360)
                        },
                        "visibility": {"clear": 10000, "cloudy": 8000, "rainy": 5000, "stormy": 2000}[condition],
                        "pop": {"clear": 0, "cloudy": 0.1, "rainy": 0.7, "stormy": 0.9}[condition],
                        "rain": {"3h": 0 if condition in ["clear", "cloudy"] else random.uniform(0.1, 15)},
                        "sys": {
                            "pod": "d" if 6 <= hour < 18 else "n"
                        },
                        "dt_txt": datetime.combine(date.date(), datetime.min.time().replace(hour=hour)).strftime("%Y-%m-%d %H:%M:%S")
                    }
    ...
    async def get_nearby_events(
        ...
        try:
            ...
            for i in range(num_events):
                ...
                event_start = datetime.combine(event_date.date(), datetime.min.time().replace(hour=event_hour))
                ...
                import math
                distance = math.sqrt(lat_offset**2 + lon_offset**2) * 111  # Aproximação simples: 1 grau = 111 km
                ...
                event_record = {
                    ...
                    "start": {
                        ...
                        "utc": (event_start - timedelta(hours=3)).strftime("%Y-%m-%dT%H:%M:%SZ")
                    },
                    "end": {
                        "timezone": "America/Sao_Paulo",
                        "local": event_end.strftime("%Y-%m-%dT%H:%M:%S"),
                        "utc": (event_end - timedelta(hours=3)).strftime("%Y-%m-%dT%H:%M:%SZ")
                    },
                    "organization_id": f"org-{random.randint(1000, 9999)}",
                    "created": (datetime.now() - timedelta(days=random.randint(30, 90))).strftime("%Y-%m-%dT%H:%M:%SZ"),
                    "changed": (datetime.now() - timedelta(days=random.randint(0, 30))).strftime("%Y-%m-%dT%H:%M:%SZ"),
                    "capacity": attendance,
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Error getting nearby events: {str(e)}"
            )
    ...
    async def get_holidays(
        ...
        """
        logger.info(f"Getting holidays for country {country}, year {year}, region {region}")
        ...
        try:
            ...
            national_holidays = [
                {"date": f"{year}-01-01", "name": "Confraternização Universal", "is_national": True},
                {"date": f"{year}-04-21", "name": "Tiradentes", "is_national": True},
                {"date": f"{year}-05-01", "name": "Dia do Trabalho", "is_national": True},
                {"date": f"{year}-09-07", "name": "Independência do Brasil", "is_national": True},
                {"date": f"{year}-10-12", "name": "Nossa Senhora Aparecida", "is_national": True},
                {"date": f"{year}-11-02", "name": "Finados", "is_national": True},
                {"date": f"{year}-11-15", "name": "Proclamação da República", "is_national": True},
            ...
            if region == "SP":
                regional_holidays = [
                    {"date": f"{year}-01-25", "name": "Aniversário de São Paulo", "region": "SP"},
                    {"date": f"{year}-07-09", "name": "Revolução Constitucionalista", "region": "SP"}
                ]
            ...
            mobile_holidays = [
                {"date": f"{year}-02-20", "name": "Carnaval", "is_national": True, "impact_level": 5},
                {"date": f"{year}-04-07", "name": "Sexta-feira Santa", "is_national": True, "impact_level": 5},
                {"date": f"{year}-04-09", "name": "Páscoa", "is_national": True, "impact_level": 5},
                {"date": f"{year}-06-08", "name": "Corpus Christi", "is_national": True, "impact_level": 4}
            ]
