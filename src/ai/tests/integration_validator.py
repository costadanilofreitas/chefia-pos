...
from src.ai.operational_optimization.services.optimization_service import OperationalOptimizationService
from src.ai.operational_optimization.integration.forecast_integration import ForecastIntegrationService
from src.ai.operational_optimization.integration.external_data import ExternalDataService
...
class IntegrationValidator:
        ...
        try:
            ...
            try:
                ...
                results["errors"].append({
                    "test": "Obter previsão de demanda",
                    "error": str(e)
                })
            ...
            try:
                ...
                test_result["details"] = {
                    ...
                    "first_recommendation": staff_recommendations[0].dict() if staff_recommendations else None
                }
            except Exception as e:
                test_result["status"] = "failed"
                test_result["error"] = str(e)
            ...
            try:
                all_recommendations = await self.optimization_service.generate_all_recommendations(
                    restaurant_id=restaurant_id,
                    start_date=start_date,
                    end_date=end_date
                )
                ...
                test_result["status"] = "success"
                test_result["details"] = {
                    "recommendations_types": list(all_recommendations["recommendations"].keys()),
            ...
            logger.error(f"Error validating forecast integration: {str(e)}", exc_info=True)
            results["success"] = False
            results["errors"].append({
    ...
    async def validate_external_data_integration(self, restaurant_id: str) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dict[str, Any]: Resultados da validação
        """
        logger.info(f"Validating external data integration for restaurant {restaurant_id}")
        ...
        try:
            ...
            try:
                ...
                enriched_forecast = await self.forecast_integration.enrich_forecast_with_external_data(
                    forecast=forecast,
                    restaurant_id=restaurant_id,
                    location=location
                )
                ...
                test_result["details"] = {
                    ...
                    "metadata": enriched_forecast.metadata if hasattr(enriched_forecast, "metadata") else None
                }
            except Exception as e:
                test_result["status"] = "failed"
                test_result["error"] = str(e)
                results["success"] = False
                results["errors"].append({
                    "test": "Enriquecer previsão com dados externos",
                    "error": str(e)
                })
            ...
            logger.error(f"Error validating external data integration: {str(e)}", exc_info=True)
            results["success"] = False
            results["errors"].append({
                "test": "Validação geral",
                "error": str(e)
    ...
    async def run_all_validations(self, restaurant_id: str) -> Dict[str, Any]:
        ...
        external_data_results = await self.validate_external_data_integration(restaurant_id)
        ...
        results["overall_success"] = (
            forecast_results["success"] and 
            external_data_results["success"] and 
