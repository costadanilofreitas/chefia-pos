...
class MarketingCampaignService:
    ...
    def __init__(self):
        ...
        self.campaigns_table = self.dynamodb.Table(
            os.environ.get("CAMPAIGNS_TABLE", "pos_marketing_campaigns")
        )
        self.templates_table = self.dynamodb.Table(
            os.environ.get("TEMPLATES_TABLE", "pos_marketing_templates")
        )
        self.customers_table = self.dynamodb.Table(
            os.environ.get("CUSTOMERS_TABLE", "pos_customers")
        )

        # Configurar filas SQS
        self.campaign_queue_url = os.environ.get(
            "CAMPAIGN_QUEUE_URL",
            "https://sqs.us-east-1.amazonaws.com/123456789012/pos_marketing_campaigns.fifo",
        )
        self.message_queue_url = os.environ.get(
            "MESSAGE_QUEUE_URL",
            "https://sqs.us-east-1.amazonaws.com/123456789012/pos_marketing_messages.fifo",
        )
    ...
    async def create_campaign(self, campaign_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            campaign_item = {
                ...
                "scheduled_date": campaign_data.get(
    ...
    async def generate_personalized_message(
        ...
        try:
            # Obter template
            template_response = self.templates_table.get_item(
            ...
            prompt = self._prepare_ai_prompt(
                template_content, customer_data, campaign_data
            )
            ...
            logger.info(
                f"Generated personalized message for customer {customer_data.get('customer_id')}"
            )
            return generated_message

        except Exception as e:
            logger.error(
                f"Error generating personalized message: {str(e)}", exc_info=True
            )
            raise

    async def send_message(
        self, customer_id: str, message: str, channel: str, metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if channel not in ["whatsapp", "telegram"]:
                raise ValueError(
                    f"Invalid channel: {channel}. Must be 'whatsapp' or 'telegram'"
                )
            ...
            if channel == "whatsapp" and not customer.get("phone_number"):
                raise ValueError(
                    f"Customer {customer_id} does not have a phone number configured"
                )

            if channel == "telegram" and not customer.get("telegram_id"):
                raise ValueError(
                    f"Customer {customer_id} does not have a Telegram ID configured"
                )
            ...
            logger.info(
                f"Message queued for sending: {message_id} to {customer_id} via {channel}"
            )
            return {
                "message_id": message_id,
                "status": "queued",
    ...
    async def process_campaign(self, campaign_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            customers = await self._get_target_customers(
                target_audience, campaign["restaurant_id"]
            )

            logger.info(
                f"Processing campaign {campaign_id} for {len(customers)} customers"
            )
            ...
            for customer in customers:
                try:
                    ...
                    message = await self.generate_personalized_message(
                        campaign["message_template"], customer, campaign
                    )
                    ...
                    elif not customer.get("phone_number") and customer.get(
                        "telegram_id"
                    ):
                        channel = "telegram"
                    ...
                    logger.error(
                        f"Error processing customer {customer.get('customer_id')}: {str(e)}"
                    )
                    results["processed"] += 1
                    results["failed"] += 1
            ...
            final_status = (
                "completed" if results["failed"] == 0 else "completed_with_errors"
            )
            ...
            logger.info(
                f"Campaign {campaign_id} processed: {results['success']} successful, {results['failed']} failed"
            )
            return results
    ...
    async def list_templates(
        self, restaurant_id: str, template_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _validate_campaign_data(self, campaign_data: Dict[str, Any]) -> None:
        ...
        if campaign_data["type"] not in valid_types:
            raise ValueError(
                f"Invalid campaign type: {campaign_data['type']}. Must be one of {valid_types}"
            )
    ...
    async def _get_target_customers(
        self, target_criteria: Dict[str, Any], restaurant_id: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if "inactive_days" in target_criteria:
                cutoff_date = (
                    datetime.now() - timedelta(days=target_criteria["inactive_days"])
                ).isoformat()
            ...
            if "min_orders" in target_criteria:
                ...
                scan_params["ExpressionAttributeValues"][":min_orders"] = (
                    target_criteria["min_orders"]
                )

            if "order_value" in target_criteria:
                scan_params[
                    "FilterExpression"
                ] += " AND average_order_value >= :min_value"
                scan_params["ExpressionAttributeValues"][":min_value"] = (
                    target_criteria["order_value"]
                )
            ...
            logger.info(
                f"Found {len(customers)} customers matching target criteria for restaurant {restaurant_id}"
            )
            return customers
