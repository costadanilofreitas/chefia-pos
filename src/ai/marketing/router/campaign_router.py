...
async def create_campaign(
    ...
    try:
        logger.info(
            f"Creating campaign for restaurant {campaign_data.get('restaurant_id')}"
        )
...
async def get_campaign(
    campaign_id: str, service: MarketingCampaignService = Depends(get_campaign_service)
):
    """
...
async def create_template(
    ...
    try:
        logger.info(
            f"Creating template for restaurant {template_data.get('restaurant_id')}"
        )
...
async def send_message(
    ...
    try:
        ...
        if not customer_id or not message:
            raise HTTPException(
                status_code=400, detail="customer_id and message are required"
            )
...
async def generate_message(
    ...
    try:
        ...
        message = await service.generate_personalized_message(
            template_id, customer_data, campaign_data
        )
