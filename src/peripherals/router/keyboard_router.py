...
async def list_keyboards(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao listar teclados: {str(e)}")
...
async def get_keyboard_config(
    ...
    try:
        ...
        if not config:
            raise HTTPException(status_code=404, detail=f"Teclado {device_id} não encontrado")
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Erro ao obter configuração do teclado: {str(e)}"
        )
...
async def update_keyboard_config(
    ...
    try:
        ...
        return {"message": f"Configuração do teclado {device_id} atualizada com sucesso"}
    except HTTPException:
        raise
    except Exception as e:
...
async def activate_keyboard(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, 
                detail=f"Não foi possível ativar o teclado {device_id}"
            )
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Erro ao ativar teclado: {str(e)}"
        )
...
async def deactivate_keyboard(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Erro ao desativar teclado: {str(e)}"
        )
...
async def list_available_commands():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Erro ao listar comandos disponíveis: {str(e)}"
        )
...
async def discover_keyboards(
    ...
    try:
        ...
        return {
            "message": f"{len(devices)} teclados descobertos",
            "devices": devices
        }
    except Exception as e:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Erro ao descobrir teclados: {str(e)}"
        )
