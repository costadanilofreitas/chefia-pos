...
async def add_peripheral(
    ...
    try:
        ...
        if "id" not in config:
            raise HTTPException(
                status_code=400, detail="ID do periférico é obrigatório"
            )

        if "type" not in config:
            raise HTTPException(
                status_code=400, detail="Tipo do periférico é obrigatório"
            )

        if "driver" not in config:
            raise HTTPException(
                status_code=400, detail="Driver do periférico é obrigatório"
            )
        ...
        if config["type"] not in available_drivers:
            raise HTTPException(
                status_code=400,
                detail=f"Tipo de periférico não suportado: {config['type']}",
            )

        if config["driver"] not in available_drivers[config["type"]]:
            raise HTTPException(
                status_code=400,
                detail=f"Driver não suportado para {config['type']}: {config['driver']}",
            )
...
async def initialize_peripheral(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Falha ao inicializar periférico"
            )
...
async def print_document(
    ...
    try:
        ...
        if peripheral.config.type not in ["thermal_printer", "conventional_printer"]:
            raise HTTPException(
                status_code=400,
                detail=f"Periférico {peripheral_id} não é uma impressora",
            )
...
async def open_cash_drawer(
    ...
    try:
        ...
        if peripheral.config.type != "cash_drawer":
            raise HTTPException(
                status_code=400,
                detail=f"Periférico {peripheral_id} não é uma gaveta de dinheiro",
            )
...
async def process_payment(
    ...
    try:
        ...
        if peripheral.config.type != "payment_terminal":
            raise HTTPException(
                status_code=400,
                detail=f"Periférico {peripheral_id} não é um terminal de pagamento",
            )

        # Verificar dados necessários
        if "amount" not in payment_data:
            raise HTTPException(
                status_code=400, detail="Valor do pagamento é obrigatório"
            )
...
async def read_barcode(
    ...
    try:
        ...
        if peripheral.config.type != "barcode_reader":
            raise HTTPException(
                status_code=400,
                detail=f"Periférico {peripheral_id} não é um leitor de código de barras",
            )
...
async def read_pix(
    ...
    try:
        ...
        if peripheral.config.type != "pix_reader":
            raise HTTPException(
                status_code=400,
                detail=f"Periférico {peripheral_id} não é um leitor de PIX",
            )
