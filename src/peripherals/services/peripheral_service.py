...
class PeripheralFactory:
    ...
    async def create_peripheral(config: Dict[str, Any]) -> Peripheral:
        ...
        # Adicionar suporte para outros tipos no futuro
            ...
            raise ConfigurationException(f"Tipo de periférico não suportado: {peripheral_type}")
...
class PeripheralManager:
    ...
    async def add_peripheral(self, peripheral_id: str, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not success:
                raise PeripheralException(f"Falha ao inicializar periférico: {peripheral_id}")
            
            # Armazenar o periférico
            self.peripherals[peripheral_id] = peripheral
        except Exception as e:
            raise PeripheralException(f"Erro ao adicionar periférico {peripheral_id}: {str(e)}")
    ...
    async def list_peripherals(self, peripheral_type: Optional[PeripheralType] = None) -> List[Dict[str, Any]]:
    ...
    async def update_peripheral_config(self, peripheral_id: str, config: Dict[str, Any]) -> bool:
