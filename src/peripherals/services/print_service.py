...
class PrintService:
    ...
    async def print_barcode(self, printer_id: str, data: str, barcode_type: str = "CODE128") -> bool:
    ...
    async def open_cash_drawer(self, printer_id: str) -> bool:
        ...
        if not printer.config.cash_drawer_enabled:
            raise PrinterException(f"Impressora {printer_id} não suporta gaveta de dinheiro")
        
        # Abrir a gaveta
        return await printer.open_cash_drawer()
    
    def _render_template(self, template: ReceiptTemplate, data: Dict[str, Any]) -> Dict[str, Any]:
