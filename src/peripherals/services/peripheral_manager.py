...
class PeripheralFactory:
    ...
    def create_peripheral(
        ...
        try:
            ...
            if config.driver not in driver_map:
                logging.error(
                    f"Driver não suportado para {config.type}: {config.driver}"
                )
                return None
            ...
            logging.error(
                f"Erro ao importar driver {config.driver} para {config.type}: {str(e)}"
            )
            return None
        except Exception as e:
            logging.error(
                f"Erro ao criar periférico {config.type}/{config.driver}: {str(e)}"
            )
            return None
    ...
    def get_available_drivers(cls, peripheral_type: Optional[str] = None) -> Dict[str, List[str]]:
        ...
        return {
            ptype: list(drivers.keys()) for ptype, drivers in cls._driver_map.items()
        }
...
class PeripheralManager:
            ...
            for peripheral_id, peripheral_config in config_data["peripherals"].items():
                ...
                if peripheral_config.get("auto_load", True):
                    ...
                    config = PeripheralConfig(
                        ...
                        name=peripheral_config.get(
                            "name", f"{peripheral_config['type']}_{peripheral_id}"
                        ),
                    ...
                    if peripheral:
                        ...
                        logging.info(
                            f"Periférico carregado: {peripheral_id} ({config.type}/{config.driver})"
                        )
                    else:
    ...
    async def initialize_all(self) -> Dict[str, bool]:
        ...
        for peripheral_id, peripheral in self.peripherals.items():
            try:
                ...
                logging.error(
                    f"Erro ao inicializar periférico {peripheral_id}: {str(e)}"
                )
    ...
    async def _handle_peripheral_event(
        self, peripheral_id: str, event_type: str, data: Dict[str, Any]
    ) -> None:
        """Manipula eventos de periféricos e notifica callbacks registrados."""
        # Criar evento
        event = {"peripheral_id": peripheral_id, "type": event_type, "data": data}
