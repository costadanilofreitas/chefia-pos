...
class KeyboardManager:
    ...
    def _load_configs(self):
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                logger.info(
                    f"Configurações de teclado carregadas: {len(self.configs)} dispositivos"
                )
            else:
                logger.warning(
    ...
    def _save_configs(self):
        ...
        try:
            ...
            logger.info(
                f"Configurações de teclado salvas: {len(self.configs)} dispositivos"
            )
        except Exception as e:
    ...
    def start(self):
        ...
        for device_id, device in self.devices.items():
            thread = threading.Thread(
                target=self._monitor_device, args=(device_id, device), daemon=True
            )
            thread.start()
            self.threads.append(thread)

        logger.info(
            f"Monitoramento de teclados iniciado. {len(self.devices)} dispositivos encontrados."
        )
    ...
    def _monitor_device(self, device_id: str, device: Dict[str, Any]):
        ...
        """
        logger.info(
            f"Iniciando monitoramento do dispositivo: {device['name']} ({device_id})"
        )
    ...
    def _reconnect_device(self, device_id: str):
        ...
        try:
            ...
            if device_id in self.devices:
                ...
                logger.warning(
                    f"Não foi possível reconectar o dispositivo {device_id}."
                )
        except Exception as e:
    ...
    def update_device_config(self, device_id: str, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Erro ao atualizar configuração do dispositivo {device_id}: {e}"
            )
            return False
