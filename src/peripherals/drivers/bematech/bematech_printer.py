...
class BematechThermalPrinter(ThermalPrinter):
    ...
    def __init__(self, config: ThermalPrinterConfig):
        ...
        self.encoding = config.options.get("encoding", "cp850")  # Codificação padrão Bematech
    ...
    async def _initialize_serial(self) -> None:
        ...
        try:
            ...
            if port == "auto" or not port:
                ...
                if port == "auto" or not port:
                    raise PeripheralException("Porta serial para Bematech não encontrada automaticamente")
