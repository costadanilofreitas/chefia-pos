...
# Constantes para comandos ESC/POS da Bematech
ESC = b"\x1b"
GS = b"\x1d"
LF = b"\x0a"
CR = b"\x0d"
...
class BematechThermalPrinter(BasePeripheralDriver):
    """Driver para impressoras térmicas Bematech."""

    def __init__(self, config: ThermalPrinterConfig):
        # Convert ThermalPrinterConfig to PeripheralConfig for BasePeripheralDriver
        peripheral_config = PeripheralConfig(
            id=config.id,
            type="printer",
            driver="bematech_thermal",
            name=config.name,
        ...
        self.encoding = config.options.get(
            "encoding", "cp850"
        )  # Codificação padrão Bematech
        self.model = config.options.get("model", "MP-4200 TH").upper()
        self.initialized = False
        self.thermal_config = config  # Guardar a configuração específica de thermal
        ...
        self.commands = self.model_commands.get(
            self.model, self.model_commands["DEFAULT"]
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logging.error(f"Erro ao inicializar impressora Bematech: {str(e)}")
    ...
    async def _initialize_usb(self) -> None:
        ...
        try:
            ...
            if self.thermal_config.options.get(
                ...
                # Procurar dispositivos Bematech
                # VID/PID para Bematech (podem variar por modelo)
                dev = usb.core.find(idVendor=0x0B1B)  # VID Bematech

                if dev is None:
                    raise PeripheralException("Impressora Bematech não encontrada")
    ...
    async def _initialize_serial(self) -> None:
        ...
        try:
            ...
            port = self.thermal_config.options.get("port", "auto")
            if port == "auto" or not port:
                ...
                for p in ports:
                    if "bematech" in p.description.lower():
                        port = p.device
                        break

                if port == "auto" or not port:
                    raise PeripheralException(
                        "Porta serial para Bematech não encontrada automaticamente"
                    )
    ...
    async def _initialize_network(self) -> None:
        ...
        try:
            ...
            port = self.thermal_config.options.get(
                "port", 9100
            )  # Porta padrão para impressoras

            if not address:
                raise PeripheralException("Endereço de rede não especificado")

            # Criar socket
            self.connection = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # type: ignore
            self.connection.settimeout(self.thermal_config.options.get("timeout", 5))
    ...
    async def _send_command(self, command: bytes) -> bool:
        ...
        try:
            ...
            logging.error(f"Erro ao enviar comando para impressora Bematech: {str(e)}")
            await self.update_status(PeripheralStatus.ERROR, str(e))
            return False
    ...
    async def shutdown(self) -> bool:
        ...
        try:
            ...
            logging.error(f"Erro ao finalizar impressora Bematech: {str(e)}")
            await self.update_status(PeripheralStatus.ERROR, str(e))
            return False
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        try:
            # Enviar comando de status (específico para Bematech)
            status_command = ESC + b"v"
            ...
            if self.thermal_config.options.get("connection_type", "serial") == "usb":
                ...
                if (
                    ...
                    # Interpretar resposta (específico para Bematech)
            ...
            logging.error(f"Erro ao obter status da impressora Bematech: {str(e)}")
            await self.update_status(PeripheralStatus.ERROR, str(e))
        ...
        try:
            ...
            if "header" in receipt:
                ...
                if "company_name" in header:
                    ...
                    commands.append(
                        header["company_name"].encode(self.encoding, errors="replace")
                        + CR
                        + LF
                    )
                ...
                if "address" in header:
                    commands.append(
                        header["address"].encode(self.encoding, errors="replace")
                        + CR
                        + LF
                    )
                ...
                if "document" in header:
                    commands.append(
                        header["document"].encode(self.encoding, errors="replace")
                        + CR
                        + LF
                    )
                ...
                if "datetime" in header:
                    commands.append(
                        header["datetime"].encode(self.encoding, errors="replace")
                        + CR
                        + LF
                    )
            ...
            if "order_info" in receipt:
                ...
                if "order_number" in order_info:
                    commands.append(
                        f"Pedido: {order_info['order_number']}".encode(
                ...
                if "customer" in order_info and order_info["customer"]:
                    commands.append(
                        f"Cliente: {order_info['customer']}".encode(
                            self.encoding, errors="replace"
                        )
                        + CR
                        + LF
                ...
                if "attendant" in order_info and order_info["attendant"]:
                    commands.append(
                        f"Atendente: {order_info['attendant']}".encode(
                            self.encoding, errors="replace"
                        )
                        + CR
                        + LF
            ...
            if "items" in receipt and receipt["items"]:
                # Cabeçalho dos itens
                commands.append(
                    b"ITEM  DESCRICAO                      QTD   VALOR    TOTAL"
                    + CR
                    + LF
                )
                commands.append(b"-" * 48 + CR + LF)
                ...
                for i, item in enumerate(receipt["items"], 1):
                    ...
                    line = (
                        f"{item_number} {description} {quantity} {unit_price} {total}"
                    )
                    commands.append(
                        line.encode(self.encoding, errors="replace") + CR + LF
                    )
            ...
            if "totals" in receipt:
                ...
                if "subtotal" in totals:
                    commands.append(
                        f"Subtotal: R$ {totals['subtotal']:.2f}".encode(
                            self.encoding, errors="replace"
                        )
                        + CR
                        + LF
                ...
                if "discount" in totals and totals["discount"] > 0:
                    commands.append(
                        f"Desconto: R$ {totals['discount']:.2f}".encode(
                            self.encoding, errors="replace"
                        )
                        + CR
                        + LF
                ...
                if "service_fee" in totals and totals["service_fee"] > 0:
                    commands.append(
                        f"Taxa de serviço: R$ {totals['service_fee']:.2f}".encode(
                            self.encoding, errors="replace"
                        )
                        + CR
                        + LF
                ...
                if "total" in totals:
                    commands.append(ESC + b"!" + b"\x30")  # Texto grande
                    commands.append(
                        f"TOTAL: R$ {totals['total']:.2f}".encode(
                            self.encoding, errors="replace"
                        )
                        + CR
                        + LF
            ...
            if "payment" in receipt:
                ...
                if "method" in payment:
                    commands.append(
                        f"Forma de pagamento: {payment['method']}".encode(
                            self.encoding, errors="replace"
                        )
                        + CR
                        + LF
                ...
                if "amount" in payment:
                    commands.append(
                        f"Valor pago: R$ {payment['amount']:.2f}".encode(
                            self.encoding, errors="replace"
                        )
                        + CR
                        + LF
                ...
                if "change" in payment and payment["change"] > 0:
                    commands.append(
                        f"Troco: R$ {payment['change']:.2f}".encode(
                            self.encoding, errors="replace"
                        )
                        + CR
                        + LF
            ...
            if "footer" in receipt:
                ...
                if "message" in footer:
                    commands.append(
                        footer["message"].encode(self.encoding, errors="replace")
                        + CR
                        + LF
                    )
    ...
    async def cut_paper(self, partial: bool = True) -> bool:
        ...
        try:
            # Comando de corte (pode variar por modelo)
            cut_command = self.commands["cut_paper"]

            # Adicionar parâmetro para corte parcial/total
            if partial:
                cut_command += b"\x01"  # Corte parcial
            else:
                cut_command += b"\x00"  # Corte total
    ...
    async def open_cash_drawer(self) -> bool:
        ...
        if not self.thermal_config.options.get("cash_drawer_enabled", False):
    ...
    async def print_image(self, image_path: str) -> bool:
        ...
        try:
            ...
            # Use Image.Resampling.LANCZOS no Pillow 10.0+ ou Image.LANCZOS em versões antigas
            try:
                resample = Image.Resampling.LANCZOS  # type: ignore
            except AttributeError:
                resample = Image.LANCZOS  # type: ignore
            img = img.resize((int(paper_width_px), new_height), resample)  # type: ignore

            # Converter para preto e branco
            img = img.convert("1")  # type: ignore
            ...
            # Comando para Bematech (pode variar por modelo)
            commands = []
            ...
            await self.update_status(
                PeripheralStatus.ERROR, "Biblioteca PIL não disponível"
            )
            return False
        except Exception as e:
    ...
    async def print_barcode(self, data: str, barcode_type: str = "CODE128") -> bool:
        ...
        try:
            # Mapeamento de tipos de código de barras para códigos Bematech
            barcode_types = {
                "UPC-A": 0,
                "UPC-E": 1,
                "EAN13": 2,
            ...
            if barcode_type not in barcode_types:
                logging.warning(
                    f"Tipo de código de barras não suportado: {barcode_type}"
                )
            ...
            height = min(255, self.thermal_config.options.get("barcode_height", 80))
            await self._send_command(GS + b"h" + bytes([height]))

            # Largura do código de barras
            width = min(6, self.thermal_config.options.get("barcode_width", 2))
    ...
    async def print_qrcode(self, data: str, size: int = 6) -> bool:
        ...
        try:
            ...
            await self._send_command(
                GS
                + b"(k"
                + bytes([data_length & 0xFF, (data_length >> 8) & 0xFF])
                + b"1P0"
                + qr_data
            )
