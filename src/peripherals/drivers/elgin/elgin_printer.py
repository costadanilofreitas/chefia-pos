...
class ElginThermalPrinter(BasePeripheralDriver):
    ...
    def __init__(self, config: ThermalPrinterConfig):
        ...
        self.model = config.options.get("model", "i9").upper()
        ...
        try:
            ...
            if connection_type == "usb":
                ...
                raise PeripheralException(
                    f"Tipo de conexão não suportado: {connection_type}"
                )
    ...
    async def _initialize_serial(self) -> None:
        ...
        try:
            ...
            if port == "auto" or not port:
                ...
                if port == "auto" or not port:
                    raise PeripheralException(
                        "Porta serial para Elgin não encontrada automaticamente"
                    )
