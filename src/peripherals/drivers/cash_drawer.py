...
class StandaloneCashDrawer(CashDrawer):
    ...
    def __init__(self, config: CashDrawerConfig):
        ...
        self.device = None
        self.open_command = bytes.fromhex(config.options.get("open_command", "1B70001919"))
        self.status_command = bytes.fromhex(config.options.get("status_command", "1B7601"))
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(self.device_path):
                ...
                await self.update_status(PeripheralStatus.ERROR, f"Dispositivo não encontrado: {self.device_path}")
                return False
...
class NetworkCashDrawer(CashDrawer):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            await self.update_status(PeripheralStatus.ERROR, "Biblioteca Socket não disponível")
            return False
        except Exception as e:
            logging.error(f"Erro ao inicializar gaveta de dinheiro: {str(e)}")
            await self.update_status(PeripheralStatus.ERROR, str(e))
