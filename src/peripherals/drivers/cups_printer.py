...
class CupsPrinter(ConventionalPrinter):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                await self.update_status(PeripheralStatus.ERROR, f"Impressora {self.printer_name} não encontrada")
                return False
            ...
            if "idle" in result.stdout or "accepting" in result.stdout:
                ...
                logging.warning(f"Impressora {self.printer_name} não está pronta: {result.stdout}")
                await self.update_status(PeripheralStatus.WARNING, "Impressora não está pronta")
                return False
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode != 0:
                await self.update_status(PeripheralStatus.ERROR, "Erro ao verificar status")
                return {
    ...
    async def _print_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                await self.update_status(PeripheralStatus.ERROR, f"Erro ao imprimir: {result.stderr}")
                return False
            ...
            await self.update_status(PeripheralStatus.BUSY, f"Imprimindo trabalho {job_id}")
    ...
    async def print_pdf(self, pdf_path: str, options: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                await self.update_status(PeripheralStatus.ERROR, f"Erro ao imprimir PDF: {result.stderr}")
                return False
            ...
            await self.update_status(PeripheralStatus.BUSY, f"Imprimindo PDF, trabalho {job_id}")
    ...
    async def print_report(self, report_data: Dict[str, Any], template: str = "default") -> bool:
    ...
    async def _generate_report_pdf(self, report_data: Dict[str, Any], template: str) -> Optional[str]:
        ...
        try:
            ...
            await self.update_status(PeripheralStatus.ERROR, "Biblioteca ReportLab não disponível")
            return None
        except Exception as e:
