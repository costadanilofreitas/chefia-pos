    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.device.isOpened():
                ...
                await self.update_status(
                    PeripheralStatus.ERROR,
                    f"Não foi possível abrir a câmera: {self.device_path}",
                )
                return False
            ...
            logging.error(
                f"Biblioteca não disponível: {str(e)}. Instale 'opencv-python' e 'pyzbar'."
            )
            await self.update_status(
                PeripheralStatus.ERROR, f"Biblioteca não disponível: {str(e)}"
            )
            return False
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.device or not self.device.isOpened():
                await self.update_status(
                    PeripheralStatus.ERROR, "Câmera não está aberta"
                )
                return {
