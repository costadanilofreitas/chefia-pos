...
class TestPeripheralDrivers(unittest.TestCase):
    ...
    def test_thermal_printer_initialization(self):
        ...
        async def test_init():
            ...
            self.assertEqual(status["status"], PeripheralStatus.ONLINE, "Status da impressora não é ONLINE")
            ...
            status = await self.printer.get_status()
            self.assertEqual(status["status"], PeripheralStatus.OFFLINE, "Status da impressora não é OFFLINE após finalização")
    ...
    def test_barcode_reader(self):
        ...
        async def test_barcode():
            ...
            self.assertEqual(status["status"], PeripheralStatus.ONLINE, "Status do leitor não é ONLINE")
            
            # Simular leitura (com timeout curto para o teste)
            with patch.object(self.barcode_reader, '_simulate_scan', return_value="7891234567890"):
                barcode = await self.barcode_reader.read(timeout=1)
                self.assertEqual(barcode, "7891234567890", "Código de barras lido incorretamente")
    ...
    def test_pix_reader(self):
        ...
        async def test_pix():
            ...
            # Simular leitura (com timeout curto para o teste)
            pix_payload = "00020126580014BR.GOV.BCB.PIX0136123e4567-e89b-12d3-a456-426655440000"
            with patch.object(self.pix_reader, '_simulate_scan', return_value=pix_payload):
    ...
    def test_cash_drawer(self):
        ...
        async def test_drawer():
            ...
            self.assertEqual(status["status"], PeripheralStatus.ONLINE, "Status da gaveta não é ONLINE")
            ...
            status = await self.cash_drawer.get_status()
            self.assertEqual(status["status"], PeripheralStatus.WARNING, "Status da gaveta não é WARNING após abertura")
            ...
            status = await self.cash_drawer.get_status()
            self.assertEqual(status["status"], PeripheralStatus.ONLINE, "Status da gaveta não é ONLINE após fechamento")
    ...
    def test_payment_terminal(self):
        ...
        async def test_payment():
            ...
            self.assertEqual(status["status"], PeripheralStatus.ONLINE, "Status do terminal não é ONLINE")
            ...
            self.assertTrue(result["success"], f"Falha ao processar pagamento: {result.get('message', '')}")
            
            # Verificar detalhes do pagamento
            self.assertIsNotNone(result["transaction_id"], "ID da transação não retornado")
            self.assertIsNotNone(result["details"].get("authorization_code"), "Código de autorização não retornado")
            
            # Cancelar pagamento
            cancel_result = await self.payment_terminal.cancel_transaction(result["transaction_id"])
            self.assertTrue(cancel_result["success"], f"Falha ao cancelar pagamento: {cancel_result.get('message', '')}")
    ...
    def test_peripheral_factory(self):
        ...
        self.assertIsInstance(printer, SimulatedThermalPrinter, "Tipo incorreto de impressora térmica")
        ...
        self.assertIsNotNone(barcode_reader, "Falha ao criar leitor de código de barras")
        self.assertIsInstance(barcode_reader, SimulatedBarcodeReader, "Tipo incorreto de leitor de código de barras")
        ...
        self.assertIsInstance(pix_reader, SimulatedPixReader, "Tipo incorreto de leitor de PIX")
        ...
        self.assertIsInstance(cash_drawer, SimulatedCashDrawer, "Tipo incorreto de gaveta de dinheiro")
        
        # Testar criação de terminal de pagamento
        payment_terminal = PeripheralFactory.create_peripheral(self.payment_terminal_config)
        self.assertIsNotNone(payment_terminal, "Falha ao criar terminal de pagamento")
        self.assertIsInstance(payment_terminal, SimulatedPaymentTerminal, "Tipo incorreto de terminal de pagamento")
        ...
        self.assertIsNone(invalid_peripheral, "Periférico com tipo inválido não deveria ser criado")
        ...
        invalid_driver_peripheral = PeripheralFactory.create_peripheral(invalid_driver_config)
        self.assertIsNone(invalid_driver_peripheral, "Periférico com driver inválido não deveria ser criado")

    def test_peripheral_manager(self):
        """Testa o gerenciador de periféricos."""
        # Criar gerenciador com configuração temporária
        config_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "config", "test_peripherals.json")
        ...
        with patch('src.peripherals.services.peripheral_manager.PeripheralManager.config_path', config_path):
            manager = PeripheralManager()
            
            # Verificar se os periféricos foram carregados
            peripherals = manager.get_all_peripherals()
            self.assertEqual(len(peripherals), 2, "Número incorreto de periféricos carregados")
            self.assertIn("test_printer", peripherals, "Impressora não foi carregada")
            self.assertIn("test_barcode", peripherals, "Leitor de código de barras não foi carregado")
            ...
            self.assertNotIn("test_barcode", peripherals, "Leitor de código de barras não foi removido")
            ...
            self.assertIn("test_printer", printers, "Impressora não encontrada por tipo")
            ...
            async def test_init_shutdown():
                ...
                self.assertEqual(len(results), 2, "Número incorreto de resultados de inicialização")
                self.assertTrue(results["test_printer"], "Falha ao inicializar impressora")
                self.assertTrue(results["test_pix"], "Falha ao inicializar leitor de PIX")
                
                # Finalizar todos
                results = await manager.shutdown_all()
                self.assertEqual(len(results), 2, "Número incorreto de resultados de finalização")
                self.assertTrue(results["test_printer"], "Falha ao finalizar impressora")
