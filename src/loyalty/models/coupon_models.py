...
class CouponBase(BaseModel):
    ...
    discount_type: CouponType = Field(
    ...
    scope: CouponScope = Field(
        ..., description="Scope of the coupon (order or product)"
    )
    product_id: Optional[uuid.UUID] = Field(
        None, description="Product ID if scope is PRODUCT"
    )
    min_order_value: Optional[float] = Field(
        None, description="Minimum order value required"
    )
    max_discount: Optional[float] = Field(
        None, description="Maximum discount amount (for percentage discounts)"
    )
    valid_from: date = Field(..., description="Start date of validity")
    valid_until: Optional[date] = Field(
        None, description="End date of validity (None = no expiration)"
    )
    max_uses: Optional[int] = Field(
        None, description="Maximum number of uses (None = unlimited)"
    )
...
class Coupon(CouponBase):
    ...
    id: uuid.UUID = Field(
        default_factory=uuid.uuid4, description="Unique identifier for the coupon"
    )
