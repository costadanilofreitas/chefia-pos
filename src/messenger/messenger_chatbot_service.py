...
class MessengerChatbotService(BaseChatbotService):
    ...
    async def process_incoming_message(
        ...
        try:
            ...
            await self.add_message_to_conversation(
                from_id, {"role": "user", "content": text, "type": message_type}
            )
    ...
    async def _handle_initial_state(
        ...
        if any(
            ...
            await self.update_conversation_state(
                user_id, ConversationState.MENU_BROWSING
            )
            ...
            order_word in text
            for order_word in ["pedido", "pedir", "comprar", "quero", "gostaria"]
        ):
            # Atualizar estado para navegação do cardápio
            await self.update_conversation_state(
                user_id, ConversationState.MENU_BROWSING
            ...
            status_word in text
            for status_word in ["status", "acompanhar", "onde", "pedido"]
        ):
            # Atualizar estado para acompanhamento de pedido
            await self.update_conversation_state(
                user_id, ConversationState.ORDER_TRACKING
            )
            ...
            await self.add_message_to_conversation(
                user_id, {"role": "assistant", "content": "Como posso ajudar hoje?"}
            )
    ...
    async def _handle_menu_browsing(
        self, user_id: str, message_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if message_type in ["postback", "quick_reply"]:
            ...
            if payload.startswith("category_"):
                ...
                await self.update_conversation_context(
                    user_id, {"selected_category": category}
                )
                ...
                return {"type": "generic_template", "elements": elements}
            ...
            # Verificar se é visualização do carrinho
                ...
                await self.update_conversation_state(
                    user_id, ConversationState.CART_REVIEW
                )
                ...
                if cart:
                    ...
                    await self.add_message_to_conversation(
                        user_id, {"role": "assistant", "content": cart_text}
                    )
                    ...
                    await self.add_message_to_conversation(
                        user_id,
                        {"role": "assistant", "content": "Seu carrinho está vazio."},
                    )
            ...
            # Verificar se é finalização de pedido
                ...
                await self.update_conversation_state(
                    user_id, ConversationState.CHECKOUT
                )
        ...
        # Mensagem de texto durante navegação do cardápio
            ...
            if any(
                cart_word in text
                for cart_word in ["carrinho", "pedido", "itens", "compras"]
            ):
                # Simular clique no botão de ver carrinho
                return await self._handle_menu_browsing(
                    user_id, {"type": "postback", "payload": "view_cart"}
                )

            # Verificar se é pedido para finalizar compra
            elif any(
                checkout_word in text
                for checkout_word in ["finalizar", "checkout", "pagar", "concluir"]
            ):
                # Simular clique no botão de finalizar pedido
                return await self._handle_menu_browsing(
                    user_id, {"type": "postback", "payload": "checkout"}
                )

            # Verificar se é pedido para voltar ao cardápio
            elif any(
                menu_word in text
                for menu_word in ["cardápio", "cardapio", "menu", "voltar"]
            ):
                # Simular clique no botão de continuar comprando
                return await self._handle_menu_browsing(
                    user_id, {"type": "postback", "payload": "continue_shopping"}
                )
    ...
    async def _handle_item_selection(
        ...
        # Mensagem de texto durante seleção de item
            ...
            await self.add_message_to_conversation(
                user_id,
                {"role": "assistant", "content": "Escolha uma categoria do cardápio:"},
            )
    ...
    async def _handle_cart_review(
        ...
        if message_type in ["postback", "quick_reply"]:
            ...
            # Verificar se é limpar carrinho
                ...
                await self.add_message_to_conversation(
                    user_id, {"role": "assistant", "content": "Seu carrinho foi limpo."}
                )
        ...
        # Mensagem de texto durante revisão do carrinho
            ...
            if any(
                ...
                # Simular clique no botão de finalizar pedido
                return await self._handle_cart_review(
                    user_id, {"type": "postback", "payload": "checkout"}
                )
            ...
            # Verificar se é pedido para voltar ao cardápio
                ...
                menu_word in text
                for menu_word in ["cardápio", "cardapio", "menu", "voltar", "continuar"]
            ):
                # Simular clique no botão de continuar comprando
                return await self._handle_cart_review(
                    user_id, {"type": "postback", "payload": "continue_shopping"}
            ...
            # Verificar se é pedido para limpar carrinho
                ...
                clear_word in text
                for clear_word in ["limpar", "esvaziar", "remover", "cancelar"]
            ):
                # Simular clique no botão de limpar carrinho
                return await self._handle_cart_review(
                    user_id, {"type": "postback", "payload": "clear_cart"}
                )
    ...
    async def _handle_checkout(
        ...
        if checkout_step == "name":
            ...
            await self.update_conversation_context(
                user_id, {"customer_name": name, "checkout_step": "phone"}
            )
            ...
            await self.update_conversation_context(
                user_id, {"customer_phone": phone, "checkout_step": "address"}
            )
            ...
            await self.update_conversation_context(
                user_id,
                {"customer_address": address, "checkout_step": "payment_method"},
            )

            # Solicitar método de pagamento
            await self.add_message_to_conversation(
                user_id,
                {"role": "assistant", "content": "Perfeito! Como você deseja pagar?"},
            )
            ...
            if message_data.get("type") in ["postback", "quick_reply"]:
                ...
                if payload.startswith("payment_"):
                    ...
                    order_summary += (
                        f"Pagamento: {self._get_payment_method_name(payment_method)}"
                    )

                    # Responder com resumo do pedido
                    await self.add_message_to_conversation(
                        user_id, {"role": "assistant", "content": order_summary}
                    )
            ...
            # Mensagem de texto durante seleção de método de pagamento
                ...
                if "pix" in text:
                    ...
                    credit_word in text
                    for credit_word in ["crédito", "credito", "cartão", "cartao"]
                ):
                    payment_method = "credit"
                elif any(
                    cash_word in text
                    for cash_word in ["dinheiro", "espécie", "especie"]
                ):
                    payment_method = "cash"
                else:
                ...
                return await self._handle_checkout(
                    user_id,
                    {"type": "postback", "payload": f"payment_{payment_method}"},
                )
            ...
            if message_data.get("type") in ["postback", "quick_reply"]:
                ...
                if payload == "confirm_order":
                    ...
                    await self.update_conversation_state(
                        user_id, ConversationState.PAYMENT
                    )
                    ...
                    if payment_method == "pix":
                        ...
                        payment_link = (
                            f"https://restaurante.com.br/pagamento/{order_id}"
                        )

                        # Salvar link de pagamento
                        await self.update_conversation_context(
                            user_id, {"payment_link": payment_link}
                        )
                    ...
                    await self.update_conversation_state(
                        user_id, ConversationState.INITIAL
                    )
            ...
            # Mensagem de texto durante confirmação
                ...
                if any(
                    confirm_word in text
                    for confirm_word in ["confirmar", "sim", "ok", "confirmo"]
                ):
                    # Simular clique no botão de confirmar pedido
                    return await self._handle_checkout(
                        user_id, {"type": "postback", "payload": "confirm_order"}
                    )

                # Verificar se é cancelamento
                elif any(
                    cancel_word in text
                    for cancel_word in ["cancelar", "não", "nao", "cancela"]
                ):
                    # Simular clique no botão de cancelar pedido
                    return await self._handle_checkout(
                        user_id, {"type": "postback", "payload": "cancel_order"}
                    )
    ...
    async def _handle_order_tracking(
        ...
        # Mensagem de texto durante acompanhamento de pedido
            ...
            if any(
                ...
                if order_id:
                    ...
                    return await self._handle_order_tracking(
                        user_id,
                        {"type": "postback", "payload": f"track_order_{order_id}"},
                    )
                else:
            ...
            # Verificar se é número de pedido
                ...
                # Salvar número do pedido
                await self.update_conversation_context(user_id, {"order_id": order_id})

                # Simular clique no botão de acompanhar pedido
                return await self._handle_order_tracking(
                    user_id, {"type": "postback", "payload": f"track_order_{order_id}"}
                )

            # Verificar se é pedido de novo pedido
            elif any(
                new_order_word in text
                for new_order_word in ["novo", "outro", "mais", "adicional"]
            ):
                # Simular clique no botão de novo pedido
                return await self._handle_order_tracking(
                    user_id, {"type": "postback", "payload": "new_order"}
                )

            # Verificar se é pedido de suporte
            elif any(
                support_word in text
                for support_word in ["suporte", "ajuda", "atendente", "humano"]
            ):
                # Simular clique no botão de suporte
                return await self._handle_order_tracking(
                    user_id, {"type": "postback", "payload": "support"}
                )
        ...
        if feedback_step == "rating":
            ...
            if message_data.get("type") in ["postback", "quick_reply"]:
                ...
                if payload.startswith("rating_"):
                    ...
                    # Salvar avaliação
                    await self.update_conversation_context(
                        user_id, {"feedback_rating": rating, "feedback_step": "comment"}
                    )
            ...
            await self.update_conversation_context(
                user_id, {"feedback_comment": comment, "feedback_step": "completed"}
            )
    ...
    async def _handle_support(
        ...
        if message_data.get("type") in ["postback", "quick_reply"]:
            ...
            if payload == "back_to_menu":
                ...
                await self.add_message_to_conversation(
                    user_id, {"role": "assistant", "content": "Como posso ajudar?"}
                )
