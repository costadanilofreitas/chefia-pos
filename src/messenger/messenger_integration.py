...
class MessengerIntegration(BasePlatformIntegration):
    ...
    def __init__(self, 
        ...
        self.page_access_token = page_access_token or os.environ.get('MESSENGER_PAGE_ACCESS_TOKEN')
        ...
        if not self.page_access_token:
            logger.warning("Token de acesso da página não configurado. A integração não funcionará corretamente.")
    ...
    async def send_generic_template(self, to: str, elements: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if response.status_code != 200:
            logger.error(f"Erro na API do Messenger: {response.status_code}, {response.text}")
            raise Exception(f"Erro na API do Messenger: {response.status_code}, {response.text}")
