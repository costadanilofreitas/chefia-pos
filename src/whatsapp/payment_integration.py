...
class WhatsAppPaymentIntegration:
    ...
    def __init__(self, api_key: str = None, environment: str = "sandbox"):
        ...
        if not self.api_key:
            logger.warning(
                "Chave de API do Asaas não configurada. A integração de pagamentos não funcionará corretamente."
            )
        ...
        logger.info(
            f"Integração com Asaas inicializada no ambiente: {self.environment}"
        )
    ...
    async def create_customer(self, customer_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/customers", headers=self.headers, json=customer_data
            )
    ...
    async def find_customer_by_phone(self, phone: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/customers?mobilePhone={phone}", headers=self.headers
            )
            ...
            if data.get("data") and len(data["data"]) > 0:
                ...
                return {"success": True, "customer": customer, "found": True}
            else:
    ...
    async def create_pix_payment(
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/payments", headers=self.headers, json=payment_data
            )
            ...
            logger.info(
                f"Pagamento PIX criado com sucesso no Asaas: {payment.get('id')}"
            )
            return {
                "success": True,
                "payment": payment,
    ...
    async def create_credit_card_payment(
        ...
        try:
            ...
            logger.info(
                f"Pagamento com cartão criado com sucesso no Asaas: {payment.get('id')}"
            )
    ...
    async def get_payment_status(self, payment_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Status do pagamento obtido com sucesso: {payment.get('status')}"
            )
            return {
                "success": True,
                "payment": payment,
    ...
    async def refund_payment(
        ...
        payment_id: str,
        value: float = None,
        description: str = "Reembolso automático",
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Pagamento reembolsado com sucesso no Asaas: {refund.get('id')}"
            )
    ...
    async def generate_payment_link(self, payment_id: str) -> Optional[str]:
        ...
        if not payment_response.get("success"):
            logger.error(
                f"Erro ao gerar link de pagamento: {payment_response.get('error')}"
            )
            return None
