...
class WhatsAppChatbotService:
    ...
    async def process_incoming_message(
        ...
        try:
            ...
            response = await self._process_message_by_state(
                message, customer, conversation
            )
    ...
    async def _get_or_create_customer(self, phone_number: str) -> WhatsAppCustomer:
        ...
        customer = WhatsAppCustomer(
            id=customer_id, phone=phone_number, last_interaction=datetime.now()
        )
    ...
    async def _process_message_by_state(
        ...
        if (
            not customer.is_registered
            and conversation.state != ConversationState.REGISTRATION
        ):
        ...
        if conversation.state == ConversationState.GREETING:
            ...
            return await self._process_category_selection(
                message, customer, conversation
            )
    ...
    async def _process_greeting(
        ...
        if (
            message_text == "1"
            or "cardápio" in message_text
            or "cardapio" in message_text
        ):
            # Ver cardápio
            ...
            message_text == "4"
            or "atendente" in message_text
            or "suporte" in message_text
        ):
            # Falar com atendente
            conversation.state = ConversationState.SUPPORT
            return {"type": "text", "text": self.message_templates["support_prompt"]}
        else:
            # Resposta padrão
            return {
                "type": "text",
    ...
    async def _create_categories_list(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "interactive": {
                ...
                "body": {"text": self.message_templates["menu_intro"]},
                "action": {"button": "Ver categorias", "sections": sections},
            },
        }
    ...
    async def _create_category_items_list(
        ...
        return {
            ...
            "interactive": {
                ...
                "body": {
                    "text": self.message_templates["category_items"].format(
                        category_name=category["name"]
                    )
                },
                "action": {"button": "Ver itens", "sections": sections},
            },
        }
    ...
    async def _create_item_details(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        buttons = [
            {
                "type": "reply",
                "reply": {"id": "add_to_cart", "title": "Adicionar ao carrinho"},
            }
        ]
        ...
        if has_customizations:
            buttons.insert(
                ...
                {
                    "type": "reply",
                    "reply": {"id": "customize", "title": "Personalizar"},
                },
            )

        # Adicionar botão para voltar
        buttons.append(
            {"type": "reply", "reply": {"id": "back_to_category", "title": "Voltar"}}
        )
    ...
    async def _show_next_customization(
        self, item: Dict[str, Any], conversation: WhatsAppConversation
    ) -> Dict[str, Any]:
        """
        ...
        for option in customization.get("options", []):
            ...
            rows.append({"id": option["id"], "title": option["name"] + price_text})

        return {
            "type": "interactive",
            "interactive": {
                "type": "list",
                "header": {"type": "text", "text": customization["name"]},
                "body": {
                ...
                "footer": {
                    "text": (
                        "Obrigatório"
                        if customization.get("required", False)
                        else "Opcional"
                    )
                },
                "action": {
                    "button": "Ver opções",
                    "sections": [{"title": customization["name"], "rows": rows}],
                },
            },
        }
        ...
        if message.message_type == MessageType.INTERACTIVE:
            ...
            if current_step < len(customizations):
                ...
                if selected_option:
                    ...
                    selected_customizations = conversation.context.get(
                        "customizations", []
                    )
                    ...
                    return await self._show_next_customization(
                        selected_item, conversation
                    )
    ...
    async def _add_to_cart(
        ...
        if not cart:
            ...
            cart = WhatsAppCart(
                id=cart_id, customer_id=customer_id, items=[], subtotal=0, total=0
            )
    ...
    async def _create_cart_review(self, customer_id: str) -> Dict[str, Any]:
        ...
        buttons = [
            ...
            {
                "type": "reply",
                "reply": {"id": "continue_shopping", "title": "Continuar comprando"},
            },
            {
                "type": "reply",
                "reply": {"id": "clear_cart", "title": "Esvaziar carrinho"},
            },
        ]
        ...
        return {
            ...
            "interactive": {
                "type": "button",
                "header": {"type": "text", "text": "Seu Carrinho"},
                "body": {
    ...
    async def _process_checkout(
        ...
        if (
            message_text == "1"
            or "delivery" in message_text
            or "entrega" in message_text
        ):
            # Delivery
            ...
            message_text == "2" or "retirada" in message_text or "local" in message_text
        ):
            # Retirada no local
            conversation.context["delivery_method"] = "pickup"
            conversation.state = ConversationState.PAYMENT
            return {"type": "text", "text": self.message_templates["payment_prompt"]}
            ...
            # Opção inválida
            return {"type": "text", "text": self.message_templates["checkout_prompt"]}
    ...
    async def _process_payment(
        ...
        if message_text == "1" or "pix" in message_text:
            ...
            message_text == "2"
            or "crédito" in message_text
            or "credito" in message_text
        ):
            # Pagamento com cartão de crédito
            payment_method = "credit_card"
            payment_details = {"payment_link": "https://example.com/pay/123456"}

        elif (
            message_text == "3" or "débito" in message_text or "debito" in message_text
        ):
        ...
        if payment_method:
            ...
            order = WhatsAppOrder(
                ...
                delivery_address=(
                    customer.address
                    if conversation.context.get("delivery_method") == "delivery"
                    else None
                ),
            )
    ...
    async def _process_registration(
        ...
        if registration_step == "name":
            ...
            return {
                "type": "text",
                "text": self.message_templates["registration_email"].format(
                    name=customer.name
                ),
            }
            ...
            return {
                ...
                + "\n\n"
                + self.message_templates["greeting"].format(
                    restaurant_name="Restaurante Demo"
                ),
            }
        ...
        conversation.context["registration_step"] = "name"
        return {"type": "text", "text": self.message_templates["registration_prompt"]}
    ...
    async def _process_order_status(
        ...
        for order_id, order in self.orders.items():
            if order.customer_id == customer.id and order_id.startswith(message_text):
                ...
                payment_status_text = payment_status_mapping.get(
    ...
    async def _process_support(
        ...
        if "ticket_subject" in conversation.context:
            ...
            return {
                "type": "text",
                "text": self.message_templates["ticket_created"].format(
        ...
        if (
            message_text == "1"
            or "problema" in message_text
            or "pedido" in message_text
        ):
            # Problema com pedido
            ...
            message_text == "2"
            or "dúvida" in message_text
            or "duvida" in message_text
            or "cardápio" in message_text
            or "cardapio" in message_text
        ):
            ...
            message_text == "3"
            or "horário" in message_text
            or "horario" in message_text
            or "funcionamento" in message_text
        ):
            ...
            return {"type": "text", "text": "Por favor, descreva como podemos ajudar:"}
    ...
    async def _process_feedback(
        ...
        if feedback_step == "rating":
            ...
            return {"type": "text", "text": self.message_templates["feedback_comment"]}
    ...
    async def handle_webhook(self, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Resposta para {message_data.get('From', '')}: {json.dumps(response)}"
            )
