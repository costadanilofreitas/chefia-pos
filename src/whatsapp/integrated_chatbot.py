...
class WhatsAppIntegratedChatbot:
    ...
    async def send_message(self, to: str, message_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if message_type == "text":
                return self.twilio_integration.send_message(to, message_data.get("text", ""))
                ...
                return {"success": False, "error": f"Tipo de mensagem não suportado: {message_type}"}
                
        except Exception as e:
            logger.error(f"Erro ao enviar mensagem: {str(e)}")
            return {"success": False, "error": str(e)}
    
    async def send_order_notification(self, phone: str, order_id: str, status: OrderStatus) -> Dict[str, Any]:
        """
        ...
        try:
            # Gerar notificação
            notification = await self.order_confirmation.generate_order_notification(order_id, status)
            
            if not notification:
                logger.error(f"Não foi possível gerar notificação para pedido {order_id}")
                return {"success": False, "error": "Não foi possível gerar notificação"}
            
            # Enviar mensagem
            return self.twilio_integration.send_message(phone, notification.get("message", ""))
            
        except Exception as e:
            logger.error(f"Erro ao enviar notificação de pedido: {str(e)}")
            return {"success": False, "error": str(e)}
    
    async def send_marketing_campaign(self, 
                                    customer_data: Dict[str, Any],
                                    campaign_type: str,
                                    restaurant_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """
    ...
    async def _handle_payment_action(self, response: Dict[str, Any], message_data: Dict[str, Any]) -> None:
        """
        ...
        if action == "payment_create_pix":
            ...
            customer_result = await self.payment_integration.find_customer_by_phone(message_data.get("from_number"))
            ...
            if customer_result.get("found"):
                ...
                create_result = await self.payment_integration.create_customer(customer_data)
            ...
            if not payment_result.get("success"):
                logger.error(f"Erro ao criar pagamento PIX: {payment_result.get('error')}")
                return
    ...
    async def _handle_order_action(self, response: Dict[str, Any], message_data: Dict[str, Any]) -> None:
        """
    ...
    async def _process_sqs_messages(self) -> None:
        ...
        while self.running:
            try:
                ...
                messages = await self.sqs_integration.receive_messages_from_pos(max_messages=10, wait_time_seconds=20)
                ...
                for message in messages:
                    try:
                        ...
                        if message_type == "order_status_update":
                            ...
                            if customer_phone:
                                await self.send_order_notification(customer_phone, order_id, status)
                            ...
                            await self.send_marketing_campaign(customer_data, campaign_type, restaurant_data)
                        
                        # Excluir mensagem processada
                        await self.sqs_integration.delete_message(message.get("receipt_handle"))
...
async def send_message(data: Dict[str, Any]):
    ...
    try:
        ...
        if "to" not in data or "message" not in data:
            raise HTTPException(status_code=400, detail="Campos 'to' e 'message' são obrigatórios")
        
        # Enviar mensagem
        result = await chatbot.send_message(data["to"], data["message"])
        
        if not result.get("success", False):
            raise HTTPException(status_code=500, detail=result.get("error", "Erro ao enviar mensagem"))
...
async def send_campaign(data: Dict[str, Any]):
    ...
    try:
        ...
        if "customer_data" not in data or "campaign_type" not in data:
            raise HTTPException(status_code=400, detail="Campos 'customer_data' e 'campaign_type' são obrigatórios")
        
        # Enviar campanha
        result = await chatbot.send_marketing_campaign(
            data["customer_data"],
            data["campaign_type"],
            data.get("restaurant_data")
        )
        
        if not result.get("success", False):
            raise HTTPException(status_code=500, detail=result.get("error", "Erro ao enviar campanha"))
