...
class WhatsAppSQSIntegration:
    ...
    def __init__(
        ...
        self.incoming_queue_url = incoming_queue_url or os.environ.get(
            "WHATSAPP_INCOMING_QUEUE_URL"
        )
        self.outgoing_queue_url = outgoing_queue_url or os.environ.get(
            "WHATSAPP_OUTGOING_QUEUE_URL"
        )

        # Validar configurações
        if not self.incoming_queue_url or not self.outgoing_queue_url:
            logger.warning(
                "URLs das filas SQS não configuradas. A integração SQS não funcionará corretamente."
            )
    ...
    async def send_message_to_pos(
        self, message_data: Dict[str, Any], message_group_id: str = None
    ) -> Dict[str, Any]:
        """
        ...
        if not message_group_id:
            ...
            message_group_id = message_data.get(
                "from_number", f"default-group-{uuid.uuid4()}"
            )
        ...
        try:
            ...
            logger.info(
                f"Mensagem enviada com sucesso para SQS: {response.get('MessageId')}"
            )
            return {
                "success": True,
            ...
            return {
                ...
                "error": str(e),
                "error_code": (
                    e.response["Error"]["Code"] if hasattr(e, "response") else "Unknown"
                ),
            }

    async def receive_messages_from_pos(
        ...
        try:
            ...
            for message in messages:
                try:
                    ...
                    logger.error(
                        f"Erro ao decodificar mensagem SQS: {message.get('Body')}"
                    )
    ...
    async def delete_message(self, receipt_handle: str) -> bool:
        ...
        try:
            ...
            self.sqs_client.delete_message(
                QueueUrl=self.incoming_queue_url, ReceiptHandle=receipt_handle
            )

            logger.info(
                f"Mensagem excluída com sucesso de SQS: {receipt_handle[:10]}..."
            )
            return True
    ...
    async def create_queues_if_not_exist(
        self,
        incoming_queue_name: str = "whatsapp-incoming.fifo",
        outgoing_queue_name: str = "whatsapp-outgoing.fifo",
    ) -> Dict[str, str]:
        """
