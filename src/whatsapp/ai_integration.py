...
class WhatsAppAIIntegration:
    ...
    async def generate_campaign_message(
        ...
        return await self.generate_response(
            prompt, system_prompt=system_prompt, max_tokens=256, temperature=0.7
        )
    ...
    async def analyze_customer_feedback(self, feedback_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self.generate_response(
                prompt, system_prompt=system_prompt, max_tokens=512, temperature=0.2
            )
    ...
    async def generate_menu_recommendations(
        ...
        try:
            # Gerar recomendações
            response = await self.generate_response(
                prompt, system_prompt=system_prompt, max_tokens=1024, temperature=0.3
            )
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                for rec in recommendations:
                    ...
                    item_details = next(
                        (item for item in menu_items if item.get("id") == item_id), None
                    )

                    if item_details:
                        detailed_recommendations.append(
                            {**item_details, "recommendation_reason": reason}
                        )

                logger.info(
                    f"Recomendações geradas com sucesso: {detailed_recommendations}"
                )
