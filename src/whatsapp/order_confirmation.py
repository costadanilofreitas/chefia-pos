...
class WhatsAppOrderConfirmation:
    ...
    def __init__(self, 
        ...
        logger.info(f"Gerenciador de confirmação de pedidos inicializado com modo padrão: {default_confirmation_mode}")
    
    def set_restaurant_confirmation_mode(self, restaurant_id: str, mode: OrderConfirmationMode) -> None:
        """
        ...
        logger.info(f"Modo de confirmação para restaurante {restaurant_id} definido como: {mode}")
    
    def get_restaurant_confirmation_mode(self, restaurant_id: str) -> OrderConfirmationMode:
        """
        ...
        """
        if restaurant_id in self.restaurant_settings and "confirmation_mode" in self.restaurant_settings[restaurant_id]:
    ...
    async def register_order(self, 
        ...
        order = {
            ...
            "confirmation_deadline": (datetime.now() + timedelta(minutes=self.confirmation_timeout_minutes)).isoformat(),
            "data": order_data or {}
        }
        ...
        logger.info(f"Pedido {order_id} registrado para confirmação no modo: {confirmation_mode}")
        return order
    ...
    async def cancel_order(self, order_id: str, reason: str = None) -> Dict[str, Any]:
        ...
        if order["payment_id"] and order["payment_status"] == PaymentStatus.PAID and self.payment_integration:
            ...
            refund_result = await self.payment_integration.refund_payment(
                order["payment_id"],
                description=f"Reembolso automático - Pedido {order_id} cancelado" + (f": {reason}" if reason else "")
            )
            
            if refund_result.get("success"):
                order["payment_status"] = PaymentStatus.REFUNDED
                order["refund_id"] = refund_result.get("refund", {}).get("id")
                logger.info(f"Reembolso iniciado para pedido {order_id}: {order['refund_id']}")
            else:
                logger.error(f"Erro ao iniciar reembolso para pedido {order_id}: {refund_result.get('error')}")
    ...
    async def update_payment_status(self, order_id: str, payment_status: PaymentStatus) -> Dict[str, Any]:
        """
        ...
        """
        if order_id not in self.orders:
            logger.error(f"Pedido {order_id} não encontrado para atualização de pagamento")
            return None
        ...
        if payment_status == PaymentStatus.PAID and order["confirmation_mode"] == OrderConfirmationMode.AUTOMATIC:
            await self.confirm_order(order_id)
        
        logger.info(f"Status de pagamento do pedido {order_id} atualizado para: {payment_status}")
        return order
    ...
    async def generate_order_notification(self, order_id: str, status: OrderStatus = None) -> Dict[str, Any]:
        """
        ...
        notification = {
            ...
            "message": status_messages.get(current_status, "Atualização no status do seu pedido."),
            "created_at": datetime.now().isoformat()
        }
