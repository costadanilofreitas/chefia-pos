...
class InstagramDirectIntegration(BasePlatformIntegration):
    ...
    def __init__(
        self, access_token: str = None, app_secret: str = None, verify_token: str = None
    ):
        """
        ...
        try:
            ...
            for element in elements:
                ...
                if element.get("title") or element.get("subtitle"):
                    ...
                    text_payload = {"recipient": {"id": to}, "message": {"text": text}}
            ...
            return {
                "success": True,
                "message_ids": [
                    r.get("message_id") for r in responses if "message_id" in r
                ],
                "recipient_id": to,
            }
    ...
    def _make_api_request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if response.status_code != 200:
            logger.error(
                f"Erro na API do Instagram: {response.status_code}, {response.text}"
            )
            raise Exception(
                f"Erro na API do Instagram: {response.status_code}, {response.text}"
            )
