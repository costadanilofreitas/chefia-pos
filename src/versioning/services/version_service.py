...
VERSION_API_URL = "https://api.example.com/pos-modern/versions"  # Replace with actual API URL
...
class VersionService:
    ...
    async def check_for_updates(self, force: bool = False) -> UpdateCheckResult:
        ...
        if not should_check and self.system_version.last_update_check:
            last_check = datetime.fromisoformat(str(self.system_version.last_update_check))
            hours_since_check = (datetime.utcnow() - last_check).total_seconds() / 3600
            should_check = hours_since_check >= self.system_version.update_check_interval_hours
        ...
        try:
            ...
            if latest_version and semver.compare(latest_version.version, self.system_version.current_version) > 0:
        ...
        try:
            ...
            return VersionInfo(
                ...
                installation_steps=[
                    ...
                    {"type": "extract", "source": f"pos-modern-{latest_version}.zip", "destination": SYSTEM_DIR},
                    {"type": "run", "command": "npm install"},
                    {"type": "run", "command": "pip install -r requirements.txt"}
                ],
                rollback_steps=[
                    {"type": "restore", "path": SYSTEM_DIR}
                ]
            )
        except Exception as e:
    ...
    async def get_version_info(self, version: str) -> Optional[VersionInfo]:
        ...
        try:
            ...
            elif self.system_version.available_version and version == self.system_version.available_version:
    ...
    async def update_system(self, request: UpdateRequest) -> UpdateResult:
        ...
        if not request.force:
            ...
            if version_info.min_required_version and semver.compare(
        ...
        try:
            ...
            if version_info.file_url:
                ...
                # Verify the hash
                if version_info.file_hash and not await self._verify_hash(download_path, version_info.file_hash):
                    return UpdateResult(
                        success=False,
        ...
        try:
            ...
            if os.path.exists(backup_config_file):
                with open(backup_config_file, 'r') as f:
                    ...
                    restored_version = backup_config.get("current_version", self.system_version.current_version)
            else:
    ...
    async def set_update_check_interval(self, hours: int) -> SystemVersion:
        ...
        Returns:
            SystemVersion: Updated system version information
        """
        self.system_version.update_check_interval_hours = max(1, hours)  # Minimum 1 hour
        self._save_config()
        return self.system_version
