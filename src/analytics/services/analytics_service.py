...
class AnalyticsService:
    ...
    def get_analytics_summary(self, restaurant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            top_dashboards = self.dashboard_service.get_top_dashboards(
        ...
        try:
            ...
            self.event_bus.emit(
                "dashboard.shared",
                {
                    ...
                    "dashboard_id": dashboard_id,
                    "restaurant_id": dashboard.restaurant_id,
                    "shared_with": share_data.get("user_id")
                    or share_data.get("role_id")
                    or share_data.get("email"),
...
class DataSourceService:
    ...
    def get_data_for_chart(
        self, chart_config: ChartConfiguration, filters: List[DashboardFilter] = None
    ) -> Dict[str, Any]:
        ...
        if data_source == DataSourceType.SALES:
            ...
            data = {
                ...
                "category": np.random.choice(
                    ["Food", "Beverage", "Dessert"], size=len(dates)
                ),
            }
            ...
            data = {
                ...
                "category": np.random.choice(
                    ["Meat", "Produce", "Dairy", "Dry Goods"], size=len(products)
                ),
            }
            df = pd.DataFrame(data)
            ...
            data = {
                ...
                "source": np.random.choice(
                    ["Direct", "iFood", "Website", "Referral"], size=len(dates)
                ),
            }
            df = pd.DataFrame(data)
            ...
            data = {
                ...
                "platform": np.random.choice(
                    ["iFood", "Direct", "Rappi", "Uber Eats"], size=len(dates)
                ),
            }
            df = pd.DataFrame(data)
    ...
    def _process_data_for_chart(
        self, df: pd.DataFrame, chart_config: ChartConfiguration
    ) -> Dict[str, Any]:
            ...
            if serie.field in df.columns:
                ...
                if chart_config.axes and len(chart_config.axes) > 0:
                    ...
                    if x_axis and x_axis.field in df.columns:
                        ...
                        # Adiciona à resposta
                        result["series"].append({"name": serie.name, "data": values})
                        ...
                        result["series"].append(
                            {"name": serie.name, "data": df[serie.field].tolist()}
                        )
                else:
...
class AlertService:
    ...
    def list_alerts(
        ...
        try:
            ...
            if dashboard_id:
                filtered_alerts = [
                    a for a in filtered_alerts if a.dashboard_id == dashboard_id
                ]

            if is_active is not None:
                filtered_alerts = [
                    a for a in filtered_alerts if a.is_active == is_active
                ]
    ...
    def _send_alert_notifications(
        ...
        try:
            ...
            message = (
                ...
                or f"Alerta: {alert.name} - Valor atual: {value}, Limite: {alert.threshold}"
            )
            ...
            for channel in alert.notification_channels:
                if channel == "email" and alert.recipients:
                    ...
                    self.logger.info(
                        f"Sending SMS alert to {alert.recipients}: {message}"
                    )
                    ...
                    self.logger.info(
                        f"Sending webhook to {alert.webhook_url}: {message}"
                    )
        except Exception as e:
...
class ExportService:
    ...
    def list_exports(
        ...
        try:
            ...
            if dashboard_id:
                filtered_exports = [
                    e for e in filtered_exports if e.dashboard_id == dashboard_id
                ]

            if user_id:
                filtered_exports = [
                    e for e in filtered_exports if e.created_by == user_id
                ]
...
class ReportService:
    ...
    def list_scheduled_reports(
        ...
        try:
            ...
            if dashboard_id:
                filtered_reports = [
                    r for r in filtered_reports if r.dashboard_id == dashboard_id
                ]

            if user_id:
                filtered_reports = [
                    r for r in filtered_reports if r.created_by == user_id
                ]

            if is_active is not None:
                filtered_reports = [
                    r for r in filtered_reports if r.is_active == is_active
                ]
