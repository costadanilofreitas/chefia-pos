...
class DashboardFilter(BaseModel):
    ...
    is_global: bool = Field(False, description="Se o filtro é global para todo o dashboard")
    ...
    default_value: Optional[FilterValue] = Field(None, description="Valor padrão do filtro")
...
class ChartSeries(BaseModel):
    ...
    type: Optional[ChartType] = Field(None, description="Tipo de gráfico para esta série específica")
    axis: Optional[str] = Field("left", description="Eixo para a série (left/right)")
    aggregation: Optional[str] = Field("sum", description="Função de agregação (sum, avg, min, max, count)")
    format: Optional[str] = Field(None, description="Formato de exibição (currency, percentage, number, date)")
    format_options: Optional[Dict[str, Any]] = Field(None, description="Opções de formatação")
...
class ChartAxis(BaseModel):
    ...
    position: str = Field("bottom", description="Posição do eixo (bottom, left, top, right)")
    format: Optional[str] = Field(None, description="Formato de exibição")
    format_options: Optional[Dict[str, Any]] = Field(None, description="Opções de formatação")
    time_granularity: Optional[TimeGranularity] = Field(None, description="Granularidade temporal para eixos de tempo")
...
class ChartConfiguration(BaseModel):
    ...
    filters: Optional[List[DashboardFilter]] = Field(None, description="Filtros específicos do gráfico")
    options: Optional[Dict[str, Any]] = Field(None, description="Opções adicionais do gráfico")
    ...
    refresh_interval: Optional[int] = Field(None, description="Intervalo de atualização em segundos")
    drill_down_enabled: bool = Field(False, description="Se o drill-down está habilitado")
    drill_down_config: Optional[Dict[str, Any]] = Field(None, description="Configuração de drill-down")
...
class DashboardLayout(BaseModel):
    ...
    breakpoints: Optional[Dict[str, int]] = Field(None, description="Breakpoints para responsividade")
...
class DashboardItem(BaseModel):
    ...
    chart_id: Optional[str] = Field(None, description="ID da configuração do gráfico, se aplicável")
    ...
    style: Optional[Dict[str, Any]] = Field(None, description="Estilo personalizado do item")
...
class Dashboard(BaseModel):
    ...
    filters: Optional[List[DashboardFilter]] = Field(None, description="Filtros globais do dashboard")
    created_at: datetime = Field(..., description="Data de criação")
    updated_at: datetime = Field(..., description="Data de atualização")
    last_viewed_at: Optional[datetime] = Field(None, description="Data da última visualização")
    view_count: int = Field(0, description="Contador de visualizações")
    thumbnail_url: Optional[str] = Field(None, description="URL da miniatura do dashboard")
    permissions: Optional[Dict[str, Any]] = Field(None, description="Permissões do dashboard")
...
class DashboardShare(BaseModel):
    ...
    user_id: Optional[str] = Field(None, description="ID do usuário com quem foi compartilhado")
    role_id: Optional[str] = Field(None, description="ID do papel com quem foi compartilhado")
    ...
    created_by: str = Field(..., description="ID do usuário que criou o compartilhamento")
...
class DataSourceConfig(BaseModel):
    ...
    connection_string: Optional[str] = Field(None, description="String de conexão, se aplicável")
    query: Optional[str] = Field(None, description="Query para obtenção dos dados")
    refresh_interval: Optional[int] = Field(None, description="Intervalo de atualização em segundos")
    parameters: Optional[Dict[str, Any]] = Field(None, description="Parâmetros da fonte de dados")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Metadados da fonte de dados")
    created_at: datetime = Field(..., description="Data de criação")
    updated_at: datetime = Field(..., description="Data de atualização")
...
class DashboardPreset(BaseModel):
    ...
    business_type: str = Field(..., description="Tipo de negócio (restaurant, bar, cafe, etc.)")
    dashboard_template_id: str = Field(..., description="ID do template de dashboard")
    is_default: bool = Field(False, description="Se é o preset padrão para o tipo de negócio")
    created_at: datetime = Field(..., description="Data de criação")
    updated_at: datetime = Field(..., description="Data de atualização")
...
class UserDashboardPreference(BaseModel):
    ...
    default_dashboard_id: Optional[str] = Field(None, description="ID do dashboard padrão")
    favorite_dashboards: Optional[List[str]] = Field(None, description="IDs dos dashboards favoritos")
    theme: Optional[str] = Field("light", description="Tema preferido (light/dark)")
    refresh_interval: Optional[int] = Field(None, description="Intervalo de atualização em segundos")
    notifications_enabled: bool = Field(True, description="Se as notificações estão habilitadas")
    email_reports_enabled: bool = Field(False, description="Se os relatórios por email estão habilitados")
    created_at: datetime = Field(..., description="Data de criação")
    updated_at: datetime = Field(..., description="Data de atualização")
...
class DashboardAlert(BaseModel):
    ...
    frequency: str = Field("realtime", description="Frequência de verificação (realtime, hourly, daily, weekly)")
    notification_channels: List[str] = Field(..., description="Canais de notificação (email, sms, push, webhook)")
    recipients: Optional[List[str]] = Field(None, description="Destinatários das notificações")
    webhook_url: Optional[str] = Field(None, description="URL do webhook para notificação")
    message_template: Optional[str] = Field(None, description="Template da mensagem de alerta")
    ...
    last_triggered_at: Optional[datetime] = Field(None, description="Data do último disparo")
...
class DashboardExport(BaseModel):
    ...
    dashboard_id: str = Field(..., description="ID do dashboard")
    format: str = Field(..., description="Formato da exportação (pdf, excel, csv, image)")
    filters: Optional[Dict[str, Any]] = Field(None, description="Filtros aplicados na exportação")
    created_at: datetime = Field(..., description="Data de criação")
    ...
    status: str = Field("pending", description="Status da exportação (pending, processing, completed, failed)")
...
class ScheduledReport(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(None, description="Filtros aplicados no relatório")
    ...
    last_run_status: Optional[str] = Field(None, description="Status da última execução")
