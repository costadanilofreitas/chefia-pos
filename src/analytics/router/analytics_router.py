...
from src.analytics.models.dashboard_models import (
    Dashboard, ChartConfiguration, 
    DashboardFilter, DataSourceType, DashboardShare, DashboardAlert, DashboardExport,
    ScheduledReport
)
from src.analytics.services.analytics_service import (
    AnalyticsService
)
...
async def get_dashboard(
    ...
    analytics_service.dashboard_service.record_dashboard_view(dashboard_id, current_user.id)
...
async def update_dashboard(
    ...
    return analytics_service.dashboard_service.update_dashboard(dashboard_id, update_data)
...
async def list_dashboards(
    ...
    is_public: Optional[bool] = Query(None, description="Filtrar por dashboards públicos"),
    is_favorite: Optional[bool] = Query(None, description="Filtrar por dashboards favoritos"),
...
async def duplicate_dashboard(
    dashboard_id: str = Path(..., description="ID do dashboard"),
    new_name: Optional[str] = Query(None, description="Novo nome para o dashboard duplicado"),
    current_user: User = Depends(get_current_user),
    analytics_service: AnalyticsService = Depends(get_analytics_service)
):
    """Duplica um dashboard existente"""
    return analytics_service.dashboard_service.duplicate_dashboard(dashboard_id, new_name)
...
async def list_data_sources(
    ...
    return analytics_service.data_source_service.list_available_data_sources(restaurant_id)
...
async def get_data_source_metadata(
    ...
    return analytics_service.data_source_service.get_data_source_metadata(data_source_type, restaurant_id)
...
async def get_chart_data(
    ...
    return analytics_service.data_source_service.get_data_for_chart(chart_config, filters)
...
async def export_dashboard(
    ...
    format: str = Query(..., description="Formato de exportação (pdf, excel, csv, image)"),
    filters: Optional[Dict[str, Any]] = None,
    current_user: User = Depends(get_current_user),
    analytics_service: AnalyticsService = Depends(get_analytics_service)
):
    """Exporta um dashboard para um formato específico"""
    return analytics_service.export_service.export_dashboard(dashboard_id, format, filters, current_user.id)
...
async def update_scheduled_report(
    ...
    # Adiciona o usuário atual como atualizador
    update_data["updated_by"] = current_user.id
    return analytics_service.report_service.update_scheduled_report(report_id, update_data)
...
async def list_scheduled_reports(
    dashboard_id: Optional[str] = Query(None, description="Filtrar por dashboard"),
    is_active: Optional[bool] = Query(None, description="Filtrar por relatórios ativos"),
    page: int = Query(1, description="Número da página"),
    page_size: int = Query(20, description="Tamanho da página"),
    current_user: User = Depends(get_current_user),
    analytics_service: AnalyticsService = Depends(get_analytics_service)
