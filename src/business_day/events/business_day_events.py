...
async def publish_business_day_opened(business_day: BusinessDay) -> None:
    ...
    logger.info(
        f"Evento de abertura de dia publicado: {business_day.id} - {business_day.date}"
    )
...
async def publish_business_day_closed(business_day: BusinessDay) -> None:
    ...
    await event_bus.publish(event)
    logger.info(
        f"Evento de fechamento de dia publicado: {business_day.id} - {business_day.date}"
    )


async def publish_business_day_updated(
    business_day: BusinessDay, updated_fields: Dict[str, Any]
) -> None:
    ...
    await event_bus.publish(event)
    logger.info(
        f"Evento de atualização de dia publicado: {business_day.id} - campos: {list(updated_fields.keys())}"
    )
