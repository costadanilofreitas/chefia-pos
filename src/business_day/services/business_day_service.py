...
class BusinessDayService:
    ...
    async def create_business_day(self, business_day: BusinessDay) -> BusinessDay:
        ...
        open_day = next((day for day in business_days if day["status"] == DayStatus.OPEN), None)
        if open_day:
    ...
    async def get_business_day(self, business_day_id: str) -> Optional[BusinessDay]:
        ...
        business_day_dict = next((day for day in business_days if day["id"] == business_day_id), None)
    ...
    async def get_open_business_day(self) -> Optional[BusinessDay]:
        ...
        open_day_dict = next((day for day in business_days if day["status"] == DayStatus.OPEN), None)
    ...
    async def update_business_day(self, business_day_id: str, update_data: Dict[str, Any]) -> BusinessDay:
        """Atualiza um dia de operação."""
        business_days = self._load_business_days()
        
        # Encontrar o dia a ser atualizado
        day_index = next((i for i, day in enumerate(business_days) if day["id"] == business_day_id), None)
        if day_index is None:
            raise ValueError(f"Dia de operação com ID {business_day_id} não encontrado.")
    ...
    async def close_business_day(
        ...
        if open_cashiers:
            raise ValueError(f"Existem {len(open_cashiers)} caixas abertos. Feche todos os caixas antes de fechar o dia.")
    ...
    async def generate_daily_sales_report(self, business_day_id: str) -> DailySalesReport:
        """
        ...
        return DailySalesReport(
            ...
            top_selling_products=[
                {"product_id": "p1", "name": "X-Burger", "quantity": 25, "total": business_day.total_sales * 0.3},
                {"product_id": "p2", "name": "Batata Frita", "quantity": 20, "total": business_day.total_sales * 0.15},
                {"product_id": "p3", "name": "Refrigerante", "quantity": 30, "total": business_day.total_sales * 0.1}
            ],
    ...
    def _load_business_days(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Erro ao decodificar arquivo JSON de dias de operação. Iniciando com lista vazia.")
            return []
