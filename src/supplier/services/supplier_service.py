...
PURCHASE_ORDERS_DATA_FILE = os.path.join(
    "/home/ubuntu/pos-modern/data", "purchase_orders.json"
)
...
class SupplierService:
    ...
    async def create_supplier(
        self, supplier_data: SupplierCreate, user_id: str, user_name: str
    ) -> Supplier:
        """
    ...
    async def delete_supplier(
        self, supplier_id: str, user_id: str, user_name: str
    ) -> bool:
        """
        Delete a supplier.
        ...
        for i, supplier in enumerate(self.suppliers):
            if supplier.get("id") == supplier_id:
                ...
                has_active_orders = any(
                    po.get("supplier_id") == supplier_id
                    and po.get("status")
                    not in [
    ...
    def _supplier_matches_query(
        self, supplier_data: Dict[str, Any], query: SupplierQuery
    ) -> bool:
        """
        ...
        if (
            query.name
            and query.name.lower() not in supplier_data.get("name", "").lower()
        ):
            return False
        ...
        # Check active status
        if query.is_active is not None and query.is_active != supplier_data.get(
            "is_active", True
        ):
            return False
        ...
        if (
            query.city
            and query.city.lower()
            != supplier_data.get("address", {}).get("city", "").lower()
        ):
            return False
        ...
        if (
            query.state
            and query.state.lower()
            != supplier_data.get("address", {}).get("state", "").lower()
        ):
            return False
    ...
    async def create_purchase_order(
        self, order_data: PurchaseOrderCreate, user_id: str, user_name: str
    ) -> PurchaseOrder:
        """
                ...
                if status not in [
                    ...
                    await log_error(
                        ...
                        details={
                            ...
                            "purchase_order_number": order.get("order_number"),
                        },
                    )
                    return None
    ...
    def _is_valid_status_change(
        self, current_status: PurchaseOrderStatus, new_status: PurchaseOrderStatus
    ) -> bool:
        """
    ...
    async def query_purchase_orders(
        ...
        for order_data in self.purchase_orders:
            if self._purchase_order_matches_query(
                order_data, supplier_id, status, start_date, end_date
            ):
    ...
    def _purchase_order_matches_query(
        self,
    ...
    async def get_purchase_order_statistics(self) -> Dict[str, Any]:
        ...
        for status in PurchaseOrderStatus:
            status_counts[status.value] = sum(
                1 for po in self.purchase_orders if po.get("status") == status.value
            )
