...
async def get_transaction_events(
    transaction_id: str, repository: TraceRepository = Depends(get_trace_repository)
):
    """
    ...
    if not events:
        raise HTTPException(
            status_code=404, detail="Eventos não encontrados para esta transação"
        )
...
async def get_transaction_stats(
    ...
    stats = await repository.get_transaction_stats(
        start_time=start_datetime, end_time=end_datetime, group_by=group_by
    )
...
async def get_transaction_types():
    ...
    """
    return {"types": [t.value for t in TransactionType]}
...
async def get_transaction_origins():
    ...
    """
    return {"origins": [o.value for o in TransactionOrigin]}
...
async def get_transaction_statuses():
    ...
    """
    return {"statuses": [s.value for s in TransactionStatus]}
