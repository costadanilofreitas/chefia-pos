...
class TraceRepository:
    ...
    async def _ensure_indexes(self):
        ...
        try:
            ...
            await self.events_collection.create_index([("transaction_id", 1), ("timestamp", 1)])
    ...
    async def _update_transaction_summary(self, event: Dict[str, Any]) -> None:
        ...
        if transaction:
            ...
            if event_type in [EventType.COMPLETED, EventType.FAILED, EventType.CANCELED]:
                update["$set"]["end_time"] = timestamp
                update["$set"]["duration_ms"] = (timestamp - transaction["start_time"]).total_seconds() * 1000
    ...
    async def get_events_by_transaction_id(self, transaction_id: str) -> List[Dict[str, Any]]:
        """
        ...
        try:
            cursor = self.events_collection.find(
                {"transaction_id": transaction_id}
            ).sort("timestamp", 1)  # Ordenar por timestamp (crescente)
    ...
    async def get_transaction_summary(self, transaction_id: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def search_transactions(
        ...
        """
        try:
            cursor = self.transactions_collection.find(filters) \
