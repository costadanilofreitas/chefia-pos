...
class EventMonitor:
    ...
    def subscribe(self, callback: Callable[[Event], None]) -> None:
        ...
        if callback not in self.subscribers:
            ...
            self.logger.debug(
                f"Novo subscriber adicionado ao monitor. Total: {len(self.subscribers)}"
            )
    ...
    def unsubscribe(self, callback: Callable[[Event], None]) -> bool:
        ...
        if callback in self.subscribers:
            ...
            self.logger.debug(
                f"Subscriber removido do monitor. Restantes: {len(self.subscribers)}"
            )
            return True
        return False
    ...
    def get_event_history(
        ...
        if event_type:
            filtered_events = [
                e for e in filtered_events if e.metadata.get("event_type") == event_type
            ]
            ...
            for key, value in filters.items():
                if key.startswith("data."):
                    ...
                    filtered_events = [
                        ...
                        and e.metadata[metadata_key] == value
                    ]
