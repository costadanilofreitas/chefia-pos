    ...
    async def dispatch(self, request: Request, call_next):
        ...
        for check_path, param_name in INSTANCE_CHECK_PATHS.items():
            if path.startswith(check_path):
                module_name = check_path.split("/")[3] # Extract module name (kds, pos, waiter)
            ...
            if instance_id_str:
                try:
                    ...
                    if not os.path.exists(config_file_path):
                        ...
                        return JSONResponse(
                            status_code=403,
                            content={"detail": f"Instance ID {instance_id} for module {module_name} is not licensed."}
                        )
                except ValueError:
                    # Handle cases where the ID is not a valid integer
                    return JSONResponse(
                        status_code=400,
                        content={"detail": f"Invalid instance ID format for {instance_id_param}."}
                    )
