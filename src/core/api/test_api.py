...
class EventData(BaseModel):
    ...
    event_type: str = Field(
        ..., description="Tipo do evento (deve ser um dos tipos definidos em EventType)"
    )
    data: Dict[str, Any] = Field(default_factory=dict, description="Dados do evento")
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Metadados do evento"
    )
...
class EventSequence(BaseModel):
    ...
    interval_ms: int = Field(
        default=1000, description="Intervalo entre eventos em milissegundos"
    )
...
class EventTypeInfo(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, description="Descrição do tipo de evento"
    )
...
async def run_event_sequence(
    sequence_id: str, events: List[EventData], interval_ms: int
):
    """
    ...
    try:
        ...
        for i, event_data in enumerate(events):
            ...
            if (
                sequence_id not in active_sequences
                or active_sequences[sequence_id]["status"] == "cancelled"
            ):
            ...
            try:
                ...
                logger.error(
                    f"Erro ao publicar evento na sequência {sequence_id}: {str(e)}"
                )
                active_sequences[sequence_id]["results"].append(
                    {"error": str(e), "event_type": event_data.event_type}
                )
...
async def list_events(
    ...
    start_time: Optional[str] = Query(
        None, description="Timestamp inicial (formato ISO)"
    ),
    ...
    try:
        ...
        events = monitor.get_event_history(
            limit=limit, event_type=event_type, start_time=start_time, end_time=end_time
        )
...
async def get_event(event_id: str):
    ...
    try:
        ...
        if not event:
            raise HTTPException(
                status_code=404, detail=f"Evento com ID {event_id} não encontrado"
            )
...
async def list_event_types():
    ...
    try:
        ...
        for event_type in EventType:
            ...
            result.append(EventTypeInfo(name=event_type.value, description=description))
        return result
    except Exception as e:
...
async def create_sequence(sequence: EventSequence, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        background_tasks.add_task(
            run_event_sequence, sequence_id, sequence.events, sequence.interval_ms
        )
...
async def get_sequence_status(sequence_id: str):
    ...
    if sequence_id not in active_sequences:
        raise HTTPException(
            status_code=404, detail=f"Sequência com ID {sequence_id} não encontrada"
        )
...
async def websocket_events(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            if (
                event_type_filter
                and event.metadata.get("event_type") != event_type_filter
            ):
                continue
