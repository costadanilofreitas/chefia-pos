...
async def create_category(
    category: CategoryCreate, current_user: dict = Depends(get_current_user_mock)
):
    """Cria uma nova categoria."""
    category_id = str(uuid.uuid4())
    new_category = ProductCategory(id=category_id, **category.dict())
...
async def create_product(
    product: ProductCreate, current_user: dict = Depends(get_current_user_mock)
):
...
async def list_products(
    ...
    if search:
        ...
        products = [
            ...
            or (p.sku and search_lower in p.sku.lower())
            or (p.barcode and search_lower in p.barcode.lower())
        ]
...
async def create_ingredient(
    ingredient: IngredientCreate, current_user: dict = Depends(get_current_user_mock)
):
    """Cria um novo ingrediente."""
    ingredient_id = str(uuid.uuid4())
    new_ingredient = Ingredient(id=ingredient_id, **ingredient.dict())
