...
class ProductService:
    ...
    async def list_products(
        ...
        if category_id:
            filtered_products = [
                p for p in filtered_products if category_id in p.get("categories", [])
            ]
        if status:
        ...
        if is_featured is not None:
            filtered_products = [
                ...
                if p.get("is_featured", False) == is_featured
            ]
        if has_ingredients is not None:
            if has_ingredients:
                filtered_products = [
                    p for p in filtered_products if p.get("ingredients", [])
                ]
            else:
                filtered_products = [
                    p for p in filtered_products if not p.get("ingredients", [])
                ]
        ...
        if search:
            ...
            filtered_products = [
                ...
                or (p.get("description") and search in p["description"].lower())
                or search in str(p.get("sku", "")).lower()
                or search in str(p.get("barcode", "")).lower()
            ]

        filtered_products.sort(
            key=lambda x: (x.get("display_order", 0), x.get("name", ""))
        )
        paginated_products = filtered_products[offset : offset + limit]

        images = self._load_images()
        main_images = {
            img["product_id"]: img["file_path"]
            for img in images
    ...
    async def delete_product(self, product_id: str) -> bool:
        ...
        is_used_in_composite = any(
            section["product_id"] == product_id for section in composite_sections
        )
        ...
        if is_used_in_combo or is_used_in_composite:
            ...
            for img in product_images:
                try:
                    os.remove(os.path.join(IMAGES_DIR, img["file_name"]))
    ...
    async def create_combo(
        self, product_data: ProductCreate, items: List[ComboItem]
    ) -> Product:
        """Cria um novo combo."""
    ...
    async def update_combo(
        ...
        if not product or product.type != ProductType.COMBO:
            return None
    ...
    async def update_composite_product(
        self, product_id: str, data: CompositeProductUpdate
    ) -> Optional[Product]:
    ...
    async def calculate_composite_product_price(
        self, product_id: str, section_product_ids: Dict[str, str]
    ) -> float:
        """Calcula o preço de um produto composto."""
        product = await self.get_product(product_id)
        if not product or product.type != ProductType.COMPOSITE:
            raise ValueError(f"Produto {product_id} não é um produto composto")
        ...
        for section in sections:
            ...
            if not selected_product_id:
                raise ValueError(f"Produto não selecionado para a seção {section.name}")

            selected_product = await self.get_product(selected_product_id)
            if not selected_product:
                raise ValueError(
                    f"Produto {selected_product_id} da seção {section.name} não encontrado"
                )
    ...
    async def list_categories(
        self, type: Optional[str] = None, parent_id: Optional[str] = None
        ...
        if type:
            filtered_categories = [
                c for c in filtered_categories if c.get("type") == type
            ]
        if parent_id:
            filtered_categories = [
                c for c in filtered_categories if c.get("parent_id") == parent_id
            ]
        else:
            filtered_categories = [
                c for c in filtered_categories if not c.get("parent_id")
            ]  # List root categories if parent_id is None
    ...
    async def update_category(
        ...
        categories = self._load_categories()
        category_index = next(
            (i for i, c in enumerate(categories) if c["id"] == category_id), None
        )
    ...
    async def delete_category(self, category_id: str) -> bool:
        ...
        if is_used:
            raise HTTPException(
                status_code=400,
                detail="Categoria está em uso por produtos e não pode ser excluída.",
            )
        ...
        if has_children:
            raise HTTPException(
                status_code=400,
                detail="Categoria possui subcategorias e não pode ser excluída.",
            )
    ...
    async def add_product_image(
        self, product_id: str, file_name: str, file_path: str, is_main: bool = False
    ) -> ProductImage:
    ...
    async def get_product_images(self, product_id: str) -> List[ProductImage]:
        ...
        images = self._load_images()
        return [
            ProductImage(**img) for img in images if img["product_id"] == product_id
        ]
    ...
    async def delete_product_image(self, image_id: str) -> bool:
        ...
        try:
            os.remove(os.path.join(IMAGES_DIR, image_data["file_name"]))
        except Exception as e:
            logger.error(f"Erro ao remover arquivo de imagem: {e}")
    ...
    async def export_menu(self, menu_id: str, format: str = "json") -> Optional[str]:
        ...
        export_data = MenuExport(
            ...
            combo_items=[
                item for item in combo_items if item["combo_id"] in menu.products
            ],
            composite_sections=[
                sec for sec in composite_sections if sec["product_id"] in menu.products
            ],
            ingredients=[
                ing for ing in ingredients if ing["product_id"] in menu.products
            ],
            option_groups=[
                og for og in option_groups if og["product_id"] in menu.products
            ],
        )
    ...
    async def get_ingredient(self, ingredient_id: str) -> Optional[Ingredient]:
        ...
        ingredients = self._load_ingredients()
        ingredient_dict = next(
            (ing for ing in ingredients if ing["id"] == ingredient_id), None
        )
    ...
    async def update_ingredient(
        ...
        ingredient_index = next(
            (i for i, ing in enumerate(ingredients) if ing["id"] == ingredient_id), None
        )
    ...
    async def update_option_group(
        ...
        option_groups = self._load_option_groups()
        group_index = next(
            (i for i, og in enumerate(option_groups) if og["id"] == group_id), None
        )
