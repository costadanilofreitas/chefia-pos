...
class ComboRulesService:
    ...
    async def validate_combo_creation(self, product_data: Dict[str, Any], items: List[Dict[str, Any]]) -> None:
        """
        ...
        if not items:
            raise HTTPException(
                status_code=400,
                detail="Um combo deve ter pelo menos um item"
            )
        ...
        for product_id in product_ids:
            ...
            if not product:
                raise HTTPException(
                    status_code=400,
                    detail=f"Produto não encontrado: {product_id}"
                )
        ...
        for group_id in exchange_group_ids:
            ...
            if not group:
                raise HTTPException(
                    status_code=400,
                    detail=f"Grupo de troca não encontrado: {group_id}"
                )
    
    async def validate_combo_update(self, combo_id: str, items: Optional[List[Dict[str, Any]]] = None) -> None:
        """
        ...
        if combo.type != ProductType.COMBO:
            raise HTTPException(
                status_code=400,
                detail="O produto não é um combo"
            )
    ...
    async def process_combo_customization(
        ...
        if not combo_items:
            raise HTTPException(
                status_code=400,
                detail="O combo não possui itens"
            )
        ...
        for customization in customizations:
            ...
            replacement_product = await product_service.get_product(replacement_product_id)
            ...
            price_difference = (replacement_product.price - original_product.price) * quantity
