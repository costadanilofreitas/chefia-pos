...
class OrderService:
    ...
    async def create_order(
        self, order_data: OrderCreate, cashier_id: Optional[str] = None
    ) -> Order:
        """Cria um novo pedido."""
        orders = self._load_orders()
        ...
        await event_bus.publish(
            Event(event_type=EventType.ORDER_CREATED, data=order.dict())
        )
    ...
    async def get_order(self, order_id: str) -> Optional[Order]:
        ...
        order_items_db = self._load_order_items()
        items = [
            OrderItem(**item) for item in order_items_db if item["order_id"] == order_id
        ]
    ...
    async def list_orders(
        ...
        if customer_id:
            filtered_orders = [
                o for o in filtered_orders if o.get("customer_id") == customer_id
            ]
        if cashier_id:
            filtered_orders = [
                o for o in filtered_orders if o.get("cashier_id") == cashier_id
            ]
        if status:
            filtered_orders = [o for o in filtered_orders if o.get("status") == status]
        if payment_status:
            filtered_orders = [
                o for o in filtered_orders if o.get("payment_status") == payment_status
            ]
        if order_type:
            filtered_orders = [
                o for o in filtered_orders if o.get("order_type") == order_type
            ]
        if start_date:
            filtered_orders = [
                o for o in filtered_orders if o.get("created_at", "") >= start_date
            ]
        if end_date:
            filtered_orders = [
                o for o in filtered_orders if o.get("created_at", "") <= end_date
            ]
        ...
        for order_dict in paginated_orders:
            items = [
                ...
                if item["order_id"] == order_dict["id"]
            ]
            order = Order(**order_dict)
            order.items = items
            result.append(order)
    ...
    async def update_order(
        self, order_id: str, update_data: OrderUpdate
    ) -> Optional[Order]:
        """Atualiza um pedido."""
        orders = self._load_orders()
        order_index = next(
            (i for i, o in enumerate(orders) if o["id"] == order_id), None
        )
        ...
        if any(
            key in update_dict
            for key in ["discount", "coupon_discount", "points_discount", "tax"]
        ):
        ...
        await event_bus.publish(
            Event(
                ...
                data={"order": order.dict(), "updates": update_dict},
            )
        )
        return order

    async def add_order_item(
        self, order_id: str, item_data: OrderItemCreate
    ) -> Optional[OrderItem]:
        ...
        if order.status not in [OrderStatus.PENDING, OrderStatus.PREPARING]:
            raise HTTPException(
                status_code=400,
                detail="Não é possível adicionar itens a um pedido que já foi entregue ou cancelado",
            )
        ...
        await event_bus.publish(
            Event(
                event_type=EventType.ORDER_UPDATED,
                data={"order_id": order_id, "item_added": order_item.dict()},
            )
        )
        return order_item

    async def update_order_item(
        self, item_id: str, update_data: OrderItemUpdate
    ) -> Optional[OrderItem]:
        """Atualiza um item de pedido."""
        order_items_db = self._load_order_items()
        item_index = next(
            (i for i, item in enumerate(order_items_db) if item["id"] == item_id), None
        )
        ...
        if not order or order.status not in [
            ...
            raise HTTPException(
                status_code=400,
                detail="Não é possível modificar itens de um pedido que já foi entregue ou cancelado",
            )
        ...
        await event_bus.publish(
            Event(
                event_type=EventType.ORDER_UPDATED,
                data={"order_id": order_id, "item_updated": item},
            )
        )
    ...
    async def remove_order_item(self, item_id: str) -> bool:
        ...
        if not order or order.status not in [
            ...
            raise HTTPException(
                status_code=400,
                detail="Não é possível remover itens de um pedido que já foi entregue ou cancelado",
            )
        ...
        await event_bus.publish(
            Event(
                event_type=EventType.ORDER_UPDATED,
                data={"order_id": order_id, "item_removed": item_id},
            )
        )
        return True
    ...
    async def apply_coupon(
        self, order_id: str, coupon_request: ApplyCouponRequest
    ) -> DiscountResponse:
        ...
        if order.status not in [OrderStatus.PENDING, OrderStatus.PREPARING]:
            raise HTTPException(
                status_code=400,
                detail="Não é possível aplicar cupom a um pedido que já foi entregue ou cancelado",
            )

        if order.payment_status == PaymentStatus.PAID:
            raise HTTPException(
                status_code=400,
                detail="Não é possível aplicar cupom a um pedido já pago",
            )
        ...
        total_discount = (
            ...
            + updated_order.coupon_discount
            + updated_order.points_discount
        )
    ...
    async def apply_points(
        self, order_id: str, points_request: ApplyPointsRequest
    ) -> DiscountResponse:
        ...
        if not order.customer_id:
            raise HTTPException(
                status_code=400, detail="Este pedido não está associado a um cliente"
            )

        if order.status not in [OrderStatus.PENDING, OrderStatus.PREPARING]:
            raise HTTPException(
                status_code=400,
                detail="Não é possível aplicar pontos a um pedido que já foi entregue ou cancelado",
            )

        if order.payment_status == PaymentStatus.PAID:
            raise HTTPException(
                status_code=400,
                detail="Não é possível aplicar pontos a um pedido já pago",
            )
    ...
    async def finalize_order(
        self, order_id: str, payment_method: PaymentMethod
    ) -> Order:
        ...
        if order.status not in [
            ...
            raise HTTPException(
                status_code=400,
                detail="Não é possível finalizar um pedido que já foi entregue ou cancelado",
            )
        ...
        if order.customer_id:
            ...
            items_summary = ", ".join(
                [f"{item.quantity}x {item.product_name}" for item in order.items[:3]]
            )
            ...
            await customer_service.add_purchase_history(
                uuid.UUID(order.customer_id), purchase_entry
            )
        ...
        event_bus = get_event_bus()
        await event_bus.publish(
            Event(event_type=EventType.ORDER_FINALIZED, data=updated_order.dict())
        )
