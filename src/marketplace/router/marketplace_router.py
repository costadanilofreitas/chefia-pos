...
@router.post(
    "/integrations", response_model=Integration, status_code=status.HTTP_201_CREATED
)
...
async def list_integrations(
    ...
    integrations, total = integration_service.list_integrations(
...
async def list_integrations_by_type(
    ...
    if type not in [t.value for t in IntegrationType]:
        raise HTTPException(
            status_code=400, detail=f"Tipo de integração inválido: {type}"
        )

    # Busca as integrações
    integrations, total = integration_service.list_integrations(
        {"type": type}, page, page_size
    )
...
@router.post(
    "/restaurants/{restaurant_id}/integrations/{integration_id}",
    response_model=IntegrationConfiguration,
)
async def configure_integration(
    ...
    """
    try:
        config = integration_service.configure_integration(
            integration_id, restaurant_id, configuration
        )
        return config
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
...
async def get_integration_configuration(
    ...
    """
    config = integration_service.get_integration_configuration(
        integration_id, restaurant_id
    )
...
@router.get(
    "/restaurants/{restaurant_id}/integrations", response_model=List[Dict[str, Any]]
)
...
async def create_delivery_order(
    ...
    try:
        ...
        order = delivery_adapter.receive_order(
            order_data["integration_id"], order_data["restaurant_id"], order_data
        )
...
@router.get(
    "/payments/transactions/{transaction_id}", response_model=PaymentTransaction
)
...
@router.post(
    "/payments/transactions/{transaction_id}/refund", response_model=PaymentTransaction
)
...
@router.post(
    "/crm/customers", response_model=CRMCustomer, status_code=status.HTTP_201_CREATED
)
async def create_crm_customer(
    ...
    if "integration_id" not in customer_data:
        raise HTTPException(
            status_code=400, detail="Campo obrigatório ausente: integration_id"
        )

    if "restaurant_id" not in customer_data:
        raise HTTPException(
            status_code=400, detail="Campo obrigatório ausente: restaurant_id"
        )
...
async def get_api_usage_metrics(
    ...
    """
    metrics = marketplace_service.get_api_usage_metrics(
