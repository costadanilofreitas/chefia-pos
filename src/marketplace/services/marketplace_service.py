...
class MarketplaceService:
    ...
    def __init__(self, db_service, config_service):
        ...
        self.delivery_adapter = DeliveryAdapter(db_service, config_service, self.webhook_service)
        self.payment_adapter = PaymentAdapter(db_service, config_service, self.webhook_service)
    ...
    def register_integration_usage(self, partner_id: str, api_key_id: str, endpoint: str, 
                                  method: str, status_code: int, response_time: int,
                                  request_size: int, response_size: int, ip_address: str,
                                  user_agent: str) -> APIUsage:
        """
    ...
    def get_api_usage_metrics(self, partner_id: Optional[str] = None, 
        ...
        total_response_time = sum(record.get("response_time", 0) for record in usage_records)
        average_response_time = total_response_time / total_requests
        
        # Calcula a taxa de sucesso
        successful_requests = sum(1 for record in usage_records if record.get("status_code", 0) < 400)
...
class PartnerService:
    ...
    def _validate_partner_data(self, partner_data: Dict[str, Any]) -> None:
        ...
        existing = self.db_service.find_one("marketplace_partners", {
    ...
    def get_partner(self, partner_id: str) -> Optional[Partner]:
        ...
        """
        partner_data = self.db_service.find_one("marketplace_partners", {"id": partner_id})
    ...
    def update_partner(self, partner_id: str, update_data: Dict[str, Any]) -> Optional[Partner]:
        """
        ...
        if "name" in update_data and update_data["name"] != partner_data["name"]:
            ...
            if existing:
                raise ValueError(f"Já existe um parceiro com o nome {update_data['name']}")
        ...
        self.db_service.update_one("marketplace_partners", {"id": partner_id}, {"$set": update_data})
    ...
    def list_partners(self, filters: Dict[str, Any], page: int = 1, page_size: int = 20) -> Tuple[List[Partner], int]:
        """
...
class IntegrationService:
    ...
    def _validate_integration_data(self, integration_data: Dict[str, Any]) -> None:
        ...
        Raises:
            ValueError: Se os dados forem inválidos
        """
        required_fields = ["partner_id", "type", "name", "description", "configuration_schema", "version"]
        ...
        partner = self.db_service.find_one("marketplace_partners", {"id": integration_data["partner_id"]})
        
        if not partner:
            raise ValueError(f"Parceiro não encontrado: {integration_data['partner_id']}")
    ...
    def get_integration(self, integration_id: str) -> Optional[Integration]:
        ...
        """
        integration_data = self.db_service.find_one("marketplace_integrations", {"id": integration_id})
    ...
    def update_integration(self, integration_id: str, update_data: Dict[str, Any]) -> Optional[Integration]:
        """
        ...
        if "type" in update_data and update_data["type"] not in [t.value for t in IntegrationType]:
            raise ValueError(f"Tipo de integração inválido: {update_data['type']}")
        
        # Verifica se o schema de configuração está sendo alterado
        if "configuration_schema" in update_data and not isinstance(update_data["configuration_schema"], dict):
            raise ValueError("Schema de configuração deve ser um objeto JSON")
        ...
        # Atualiza no banco de dados
        self.db_service.update_one("marketplace_integrations", {"id": integration_id}, {"$set": update_data})
        
        # Retorna a integração atualizada
        return Integration(**integration_data)
    
    def list_integrations(self, filters: Dict[str, Any], page: int = 1, page_size: int = 20) -> Tuple[List[Integration], int]:
        """
        ...
        integration_objects = [Integration(**integration) for integration in integrations]
        
        return integration_objects, total
    
    def configure_integration(self, integration_id: str, restaurant_id: str, 
                             configuration: Dict[str, Any]) -> IntegrationConfiguration:
        """
        ...
        existing_config = self.db_service.find_one("marketplace_integration_configurations", {
        ...
        if existing_config:
            ...
            self.db_service.insert_one("marketplace_integration_configurations", config_data)
            
            return IntegrationConfiguration(**config_data)
    
    def _validate_configuration(self, schema: Dict[str, Any], configuration: Dict[str, Any]) -> None:
        """
    ...
    def get_integration_configuration(self, integration_id: str, restaurant_id: str) -> Optional[IntegrationConfiguration]:
        """
        ...
        """
        config_data = self.db_service.find_one("marketplace_integration_configurations", {
            "integration_id": integration_id,
            "restaurant_id": restaurant_id
        })
    ...
    def list_restaurant_integrations(self, restaurant_id: str) -> List[Dict[str, Any]]:
        ...
        configs = self.db_service.find("marketplace_integration_configurations", {
            "restaurant_id": restaurant_id
        })
...
class WebhookService:
    ...
    def _validate_webhook_data(self, webhook_data: Dict[str, Any]) -> None:
        ...
        # Verifica se a integração existe
        integration = self.db_service.find_one("marketplace_integrations", {"id": webhook_data["integration_id"]})
        
        if not integration:
            raise ValueError(f"Integração não encontrada: {webhook_data['integration_id']}")
        
        # Verifica se os tipos de evento são válidos
        if not isinstance(webhook_data["event_types"], list) or not webhook_data["event_types"]:
    ...
    def get_webhook(self, webhook_id: str) -> Optional[Webhook]:
        ...
        """
        webhook_data = self.db_service.find_one("marketplace_webhooks", {"id": webhook_id})
    ...
    def update_webhook(self, webhook_id: str, update_data: Dict[str, Any]) -> Optional[Webhook]:
        """
        Atualiza um webhook
        ...
        if "event_types" in update_data:
            if not isinstance(update_data["event_types"], list) or not update_data["event_types"]:
                raise ValueError("Tipos de evento devem ser uma lista não vazia")
        ...
        # Atualiza no banco de dados
        self.db_service.update_one("marketplace_webhooks", {"id": webhook_id}, {"$set": update_data})
    ...
    def list_webhooks(self, filters: Dict[str, Any], page: int = 1, page_size: int = 20) -> Tuple[List[Webhook], int]:
        """
    ...
    def trigger_webhook(self, integration_id: str, restaurant_id: str, event_type: str, payload: Dict[str, Any]) -> List[Tuple[str, bool, str]]:
        """
        ...
        webhooks = self.db_service.find("marketplace_webhooks", {
            "integration_id": integration_id,
            "restaurant_id": restaurant_id,
        ...
        for webhook_data in webhooks:
            ...
            # Registra a entrega
            self._register_webhook_delivery(webhook.id, event_type, payload, success, message)
            
            results.append((webhook.id, success, message))
        
        return results
    
    def _send_webhook(self, webhook: Webhook, event_type: str, payload: Dict[str, Any]) -> Tuple[bool, str]:
        """
        Envia um webhook
        ...
        try:
            ...
            if response.status_code >= 200 and response.status_code < 300:
                return True, f"Webhook enviado com sucesso. Código de status: {response.status_code}"
            else:
                return False, f"Falha ao enviar webhook. Código de status: {response.status_code}. Resposta: {response.text}"
    ...
    def _register_webhook_delivery(self, webhook_id: str, event_type: str, payload: Dict[str, Any], 
                                  success: bool, message: str, response_code: Optional[int] = None,
    ...
    def retry_failed_deliveries(self) -> List[Tuple[str, bool, str]]:
        ...
        deliveries = self.db_service.find("marketplace_webhook_deliveries", {
            "status": "failed",
        ...
        for delivery in deliveries:
            ...
            if success:
                ...
                next_attempt = datetime.now() + timedelta(minutes=5 * (2 ** (attempt_count - 1)))
...
class APIKeyService:
    ...
    def create_api_key(self, partner_id: str, name: str, scopes: List[str], 
...
class DeliveryAdapter:
    ...
    def receive_order(self, integration_id: str, restaurant_id: str, order_data: Dict[str, Any]) -> DeliveryOrder:
        """
    ...
    def _validate_order_data(self, order_data: Dict[str, Any]) -> None:
        ...
        Raises:
            ValueError: Se os dados forem inválidos
        """
        required_fields = ["external_id", "customer", "items", "delivery_address", "payment", "total_value", "delivery_fee"]
        ...
        if "external_id" in order_data:
            existing = self.db_service.find_one("marketplace_delivery_orders", {
    ...
    def get_order(self, order_id: str) -> Optional[DeliveryOrder]:
        ...
        """
        order_data = self.db_service.find_one("marketplace_delivery_orders", {"id": order_id})
    ...
    def update_order_status(self, order_id: str, status: DeliveryOrderStatus) -> Optional[DeliveryOrder]:
        """
        ...
        # Atualiza no banco de dados
        self.db_service.update_one("marketplace_delivery_orders", {"id": order_id}, {"$set": update_data})
    ...
    def list_orders(self, filters: Dict[str, Any], page: int = 1, page_size: int = 20) -> Tuple[List[DeliveryOrder], int]:
        """
    ...
    def sync_menu(self, integration_id: str, restaurant_id: str, menu_data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            config = self.db_service.find_one("marketplace_integration_configurations", {
                "integration_id": integration_id,
                "restaurant_id": restaurant_id
            })
            
            if not config:
                logger.error(f"Configuração não encontrada para integração {integration_id} e restaurante {restaurant_id}")
                return False
            
            # Simula a sincronização
            logger.info(f"Sincronizando cardápio para integração {integration_id} e restaurante {restaurant_id}")
...
class PaymentAdapter:
    ...
    def create_transaction(self, integration_id: str, restaurant_id: str, 
                          transaction_data: Dict[str, Any]) -> PaymentTransaction:
        """
    ...
    def _validate_transaction_data(self, transaction_data: Dict[str, Any]) -> None:
        ...
        Raises:
            ValueError: Se os dados forem inválidos
        """
        required_fields = ["order_id", "amount", "method", "status", "payment_data", "customer"]
        ...
        if transaction_data["method"] not in [m.value for m in PaymentMethod]:
            raise ValueError(f"Método de pagamento inválido: {transaction_data['method']}")
        
        # Verifica se o status é válido
        if transaction_data["status"] not in [s.value for s in PaymentStatus]:
            raise ValueError(f"Status de pagamento inválido: {transaction_data['status']}")
    ...
    def get_transaction(self, transaction_id: str) -> Optional[PaymentTransaction]:
        ...
        """
        transaction_data = self.db_service.find_one("marketplace_payment_transactions", {"id": transaction_id})
    ...
    def update_transaction_status(self, transaction_id: str, status: PaymentStatus) -> Optional[PaymentTransaction]:
        """
        ...
        # Atualiza no banco de dados
        self.db_service.update_one("marketplace_payment_transactions", {"id": transaction_id}, {"$set": update_data})
    ...
    def refund_transaction(self, transaction_id: str, amount: Optional[float] = None) -> Optional[PaymentTransaction]:
        """
        ...
        if transaction.status not in [PaymentStatus.AUTHORIZED, PaymentStatus.CAPTURED]:
            raise ValueError(f"Transação com status {transaction.status} não pode ser estornada")
    ...
    def list_transactions(self, filters: Dict[str, Any], page: int = 1, page_size: int = 20) -> Tuple[List[PaymentTransaction], int]:
        """
        ...
        transaction_objects = [PaymentTransaction(**transaction) for transaction in transactions]
        
        return transaction_objects, total
    
    def get_payment_methods(self, integration_id: str, restaurant_id: str) -> List[Dict[str, Any]]:
        """
...
class CRMAdapter:
    ...
    def register_customer(self, integration_id: str, restaurant_id: str, 
                         customer_data: Dict[str, Any]) -> CRMCustomer:
        """
        ...
        if "email" in customer_data and customer_data["email"]:
            existing_customer = self.db_service.find_one("marketplace_crm_customers", {
                "integration_id": integration_id,
                "restaurant_id": restaurant_id,
                "email": customer_data["email"]
            })
        
        if not existing_customer and "phone" in customer_data and customer_data["phone"]:
            existing_customer = self.db_service.find_one("marketplace_crm_customers", {
                "integration_id": integration_id,
                "restaurant_id": restaurant_id,
    ...
    def _validate_customer_data(self, customer_data: Dict[str, Any]) -> None:
        ...
        if not any(field in customer_data for field in ["email", "phone"]):
            raise ValueError("Pelo menos um dos campos 'email' ou 'phone' deve ser informado")
    ...
    def get_customer(self, customer_id: str) -> Optional[CRMCustomer]:
        ...
        """
        customer_data = self.db_service.find_one("marketplace_crm_customers", {"id": customer_id})
    ...
    def update_customer(self, customer_id: str, update_data: Dict[str, Any]) -> Optional[CRMCustomer]:
        """
        Atualiza um cliente
        ...
        # Atualiza no banco de dados
        self.db_service.update_one("marketplace_crm_customers", {"id": customer_id}, {"$set": update_data})
    ...
    def list_customers(self, filters: Dict[str, Any], page: int = 1, page_size: int = 20) -> Tuple[List[CRMCustomer], int]:
        """
    ...
    def get_customer_orders(self, customer_id: str, page: int = 1, page_size: int = 20) -> Tuple[List[Dict[str, Any]], int]:
        """
    ...
    def update_loyalty_points(self, customer_id: str, points_delta: int) -> Optional[CRMCustomer]:
        """
