...
class Partner(BaseModel):
    ...
    contact_phone: Optional[str] = Field(
        None, description="Telefone de contato do parceiro"
    )
    status: PartnerStatus = Field(
        default=PartnerStatus.PENDING, description="Status do parceiro"
    )
    approval_date: Optional[datetime] = Field(
        None, description="Data de aprovação do parceiro"
    )
...
class Integration(BaseModel):
    ...
    configuration_schema: Dict[str, Any] = Field(
        ..., description="Schema JSON para configuração"
    )
    webhook_events: List[str] = Field(
        default_factory=list, description="Eventos suportados para webhooks"
    )
    status: IntegrationStatus = Field(
        default=IntegrationStatus.ACTIVE, description="Status da integração"
    )
...
class IntegrationConfiguration(BaseModel):
    ...
    status: ConfigurationStatus = Field(
        default=ConfigurationStatus.ACTIVE, description="Status da configuração"
    )
    created_at: datetime = Field(
        default_factory=datetime.now, description="Data de criação"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "configuration": {"api_key": "abc123", "merchant_id": "merchant-123"},
                "status": "active",
                "created_at": "2025-01-01T00:00:00Z",
                "updated_at": "2025-01-01T00:00:00Z",
            }
...
class Webhook(BaseModel):
    ...
    status: WebhookStatus = Field(
        default=WebhookStatus.ACTIVE, description="Status do webhook"
    )
    created_at: datetime = Field(
        default_factory=datetime.now, description="Data de criação"
    )
...
class WebhookDelivery(BaseModel):
    ...
    next_attempt: Optional[datetime] = Field(
        None, description="Data da próxima tentativa"
    )
    created_at: datetime = Field(
        default_factory=datetime.now, description="Data de criação"
    )
...
class DeliveryOrder(BaseModel):
    ...
    scheduled_time: Optional[datetime] = Field(
        None, description="Horário agendado para entrega"
    )
    created_at: datetime = Field(
        default_factory=datetime.now, description="Data de criação"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "payment": {"method": "CREDIT_CARD", "status": "PAID", "total": 41.80},
                "total_value": 31.80,
                "delivery_fee": 10.00,
                "status": "confirmed",
                "created_at": "2025-01-01T00:00:00Z",
...
class PaymentTransaction(BaseModel):
    ...
    external_id: Optional[str] = Field(
        None, description="ID da transação na plataforma externa"
    )
    ...
    payment_data: Dict[str, Any] = Field(
        ..., description="Dados específicos do pagamento"
    )
    customer: Dict[str, Any] = Field(..., description="Dados do cliente")
    created_at: datetime = Field(
        default_factory=datetime.now, description="Data de criação"
...
class CRMCustomer(BaseModel):
    ...
    restaurant_id: str = Field(..., description="ID do restaurante")
    external_id: Optional[str] = Field(
        None, description="ID do cliente na plataforma externa"
    )
    ...
    addresses: List[Dict[str, Any]] = Field(
        default_factory=list, description="Endereços do cliente"
    )
    preferences: Dict[str, Any] = Field(
        default_factory=dict, description="Preferências do cliente"
    )
    loyalty_points: int = Field(default=0, description="Pontos de fidelidade")
    last_order_date: Optional[datetime] = Field(
        None, description="Data do último pedido"
    )
...
class APIUsage(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Data e hora do acesso"
    )
